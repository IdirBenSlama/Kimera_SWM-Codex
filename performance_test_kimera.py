...
class KimeraPerformanceTester:
    ...
    async def measure_request(self, session: aiohttp.ClientSession, method: str, 
                            endpoint: str, data: Dict = None) -> PerformanceMetrics:
    ...
    async def test_endpoint_load(self, endpoint: str, method: str = "GET", 
        ...
        endpoint_metrics = [m for m in self.metrics if m.endpoint == endpoint and m.method == method]
        response_times = [m.response_time_ms for m in endpoint_metrics if m.status_code != 0]
    ...
    async def capture_system_metrics(self, duration_seconds: int = 60, interval: float = 1.0):
    ...
    async def run_comprehensive_test(self):
        ...
        test_scenarios = [
            ...
            {"endpoint": "/system-metrics/", "method": "GET", "concurrent": 50, "total": 500},
            
            # Core functionality tests
            {"endpoint": "/kimera/system/status", "method": "GET", "concurrent": 20, "total": 200},
            {"endpoint": "/kimera/system/cycle", "method": "POST", "concurrent": 10, "total": 100},
            ...
            # Heavy load tests - Geoid creation
            {
                ...
                "method": "POST",
                "data": {
                    "content": "Performance test geoid content with substantial text to simulate real usage. " * 10,
            ...
            # Embedding generation load test
            {
                ...
                "method": "POST",
                "data": {
                    "text": "This is a test text for embedding generation. " * 20
                },
                "concurrent": 25,
                "total": 250
            },
        ...
        metrics_task = asyncio.create_task(self.capture_system_metrics(duration_seconds=300))
    ...
    def generate_report(self, test_results: List[Dict[str, Any]]):
        ...
        report = {
            ...
            "detailed_metrics": [asdict(m) for m in self.metrics[-1000:]],  # Last 1000 requests
    ...
    def _generate_recommendations(self, test_results: List[Dict[str, Any]]) -> List[str]:
        ...
        for result in test_results:
            if "avg_response_time_ms" in result:
                ...
                if result.get("failed_requests", 0) > result.get("total_requests", 1) * 0.05:
                    recommendations.append(
        ...
        if system_summary.get("cpu", {}).get("max_percent", 0) > 90:
            recommendations.append("⚠️ CPU usage peaked above 90%. Consider scaling or optimization.")
        
        if system_summary.get("memory", {}).get("max_percent", 0) > 85:
            recommendations.append("⚠️ Memory usage peaked above 85%. Monitor for memory leaks.")
    ...
    def _generate_visualizations(self, timestamp: str):
        ...
        endpoint_stats = endpoint_groups.agg(['mean', 'std']).sort_values('mean', ascending=False)
        ...
        df_sorted.set_index('timestamp')['response_time_ms'].rolling(window=10).mean().plot(ax=ax2)
        ...
        if self.system_metrics:
            ...
            sys_df.set_index('timestamp')[['cpu_percent', 'memory_percent']].plot(ax=ax3)
    ...
    def _print_summary(self, test_results: List[Dict[str, Any]]):
        ...
        for result in test_results:
            if "avg_response_time_ms" in result:
                ...
                print(f"   Successful: {result['successful_requests']} | Failed: {result['failed_requests']}")
        ...
        if system_summary:
            print(f"\n💻 SYSTEM METRICS")
            print(f"   CPU Usage - Avg: {system_summary['cpu']['avg_percent']:.1f}% | "
                  f"Max: {system_summary['cpu']['max_percent']:.1f}%")
            print(f"   Memory Usage - Avg: {system_summary['memory']['avg_percent']:.1f}% | "
...
async def main():
    ...
    print(f"   Success Rate: {(peak_results['successful_requests']/peak_results['total_requests']*100):.1f}%")
    print(f"   Avg Response Time: {peak_results.get('avg_response_time_ms', 'N/A'):.2f}ms")
    print(f"   Max Response Time: {peak_results.get('max_response_time_ms', 'N/A'):.2f}ms")
