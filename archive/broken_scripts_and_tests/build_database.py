...
def check_dependencies():
    ...
    try:
        ...
        logger.info("   ✅ Backend database modules")
    except ImportError as e:
        logger.error(f"   ❌ Backend modules: {e}")
...
def check_neo4j_connection() -> bool:
    ...
    try:
        ...
        if not neo4j_pass:
            logger.info("   ⚠️  NEO4J_PASS environment variable not set")
            logger.info("   Set with: export NEO4J_PASS='your_password'")
            return False
        
        logger.info(f"   Neo4j URI: {neo4j_uri}")
        logger.info(f"   Neo4j User: {neo4j_user}")
        logger.info("   Neo4j Password: [CONFIGURED]")
        ...
        if driver_liveness_check():
            ...
            logger.info("   ❌ Neo4j not responding")
            logger.info("   Start with: docker run -p 7687:7687 -p 7474:7474 -e NEO4J_AUTH=neo4j/password neo4j:5")
            return False
            
    except Exception as e:
        logger.info(f"   ❌ Neo4j connection error: {e}")
        return False
...
def initialize_relational_database(reset: bool = False, enhanced: bool = False):
    ...
    try:
        ...
        if enhanced:
            ...
            logger.info("   📊 Enhanced understanding schema loaded")
        ...
        if enhanced:
            ...
            logger.info("   ✅ Enhanced understanding tables created")
        ...
        with SessionLocal() as db:
            ...
            for table in core_tables:
                try:
                    ...
                    logger.info(f"   ❌ Table '{table}': {e}")
        
        logger.info("   ✅ Relational database initialized successfully")
        return True
        
    except Exception as e:
        logger.info(f"   ❌ Error initializing relational database: {e}")
        return False
...
def create_database_indexes():
    ...
    try:
        ...
        logger.info(f"   ❌ Error creating indexes: {e}")
        return False
...
def initialize_neo4j_database(reset: bool = False):
    ...
    try:
        ...
        with get_session() as session:
            ...
            for constraint in constraints:
                try:
                    ...
                    logger.info(f"   ✅ Constraint: {constraint_name}_unique")
                except Exception as e:
                    if "already exists" not in str(e).lower():
                        logger.info(f"   ⚠️  Constraint warning: {e}")
            ...
            for index in indexes:
                try:
                    ...
                    if "already exists" not in str(e).lower():
                        logger.info(f"   ⚠️  Index warning: {e}")
        ...
        logger.info(f"   ❌ Error initializing Neo4j database: {e}")
        return False
    ...
    try:
        ...
        for i, geoid in enumerate(sample_geoids):
            try:
                ...
                logger.info(f"   ✅ Sample {i+1}: SCAR {scar_db.scar_id[:12]}...")
            except Exception as e:
                logger.info(f"   ⚠️  Sample {i+1} warning: {e}")
        
        # Verify data was created
        total_scars_a = vault_manager.get_total_scar_count("vault_a")
        total_scars_b = vault_manager.get_total_scar_count("vault_b")
        
        logger.info(f"   📊 Vault A: {total_scars_a} SCARs")
        logger.info(f"   📊 Vault B: {total_scars_b} SCARs")
        logger.info(f"   📊 Total: {total_scars_a + total_scars_b} SCARs created")
...
def verify_database_setup():
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            logger.info(f"   📊 SQL Database: {scar_count} SCARs, {geoid_count} Geoids")
            
    except Exception as e:
        logger.info(f"   ❌ SQL Database error: {e}")
        success = False
        ...
        with get_session() as session:
            ...
            if neo4j_counts:
                for node_type, count in neo4j_counts.items():
                    logger.info(f"   🕸️  Neo4j: {count} {node_type} nodes")
            else:
                logger.info("   🕸️  Neo4j: No data (expected for fresh install)")
                
    except Exception as e:
        logger.info(f"   ❌ Neo4j Database error: {e}")
        success = False
...
def show_database_info():
    ...
    logger.info(f"📊 Relational DB: {database_url}")
    logger.info(f"🕸️  Graph DB: {neo4j_uri}")
    ...
    logger.info("\n🔧 Next Steps:")
    logger.info("1. Start KIMERA: python run_kimera.py")
    logger.info("2. Test API: curl http://localhost:8000/health")
    logger.info("3. View Neo4j: http://localhost:7474 (if Neo4j enabled)")
    logger.info("4. Run tests: python -m pytest tests/")
...
Examples:
  ...
  python build_database.py --reset            # Reset and rebuild
    ...
    logger.info("🚀 KIMERA SWM Database Builder")
    logger.info("=" * 50)
    ...
    if args.neo4j and not neo4j_available:
        ...
        logger.info("1. Start Neo4j: docker run -p 7687:7687 -p 7474:7474 -e NEO4J_AUTH=neo4j/password neo4j:5")
        logger.info("2. Set environment: export NEO4J_PASS='password'")
        logger.info("3. Re-run: python build_database.py --neo4j")
