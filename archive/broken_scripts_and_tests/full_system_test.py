...
def get_api_base_url():
    ...
    try:
        ...
        logger.info(f"🔗 Connecting to API server on port {port}")
        return f"http://localhost:{port}"
    except (IOError, ValueError):
        logger.warning("⚠️ Could not read .port.tmp, defaulting to port 8001. This may fail.")
...
class FullSystemTest:
    ...
    def test_database_operations(self):
        ...
        with self.engine.connect() as conn:
            ...
            logger.info(f"      ✅ JSONB query: {jsonb_time*1000:.1f}ms ({len(jsonb_result)
            ...
            logger.info(f"      ✅ Vector similarity: {vector_time:.2f}s ({len(vector_result)
            
            # 3. Test Materialized Views
            logger.info("   Testing materialized views...")
            for view in ['mv_scar_patterns', 'mv_geoid_complexity']:
                ...
                logger.info(f"      ✅ {view}: {view_time*1000:.1f}ms ({view_result} rows)
    ...
    def test_api_operations(self):
        ...
        try:
            ...
            logger.info(f"      ✅ /system/health: {response.status_code} ({api_time*1000:.0f}ms)
        except Exception as e:
            results['/system/health'] = {'status': 'FAIL', 'error': str(e)}
            logger.error(f"      ❌ /system/health: {str(e)
        ...
        for endpoint in ['/system/health/detailed', '/system/status', '/monitoring/status']:
            try:
                ...
                logger.info(f"      ✅ {endpoint}: {response.status_code} ({api_time*1000:.0f}ms)
            except Exception as e:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                logger.info(f"      ✅ Geoid created: {geoid_id} ({create_time:.2f}s)
                ...
                try:
                    ...
                    if response.status_code == 202:
                        ...
                        logger.info(f"      ✅ Contradiction processing task started: {response.status_code} ({api_time*1000:.0f}ms)
                    else:
                        ...
                        logger.error(f"      ❌ Contradiction processing failed to start: {response.status_code} ({api_time*1000:.0f}ms)

                except Exception as e:
                    results['contradiction_processing'] = {'status': 'FAIL', 'error': str(e)}
                    logger.error(f"      ❌ Contradiction processing: {str(e)
    ...
    def test_concurrent_load(self):
        ...
        logger.info("   Testing concurrent database queries...")
        def db_query():
            with self.engine.connect() as conn:
                start = time.time()
        ...
        logger.info(f"      ✅ Database: {qps:.1f} queries/second")
        ...
        logger.info(f"      ✅ API: {len(successful_requests)
    ...
    def test_data_integrity(self):
        ...
        with self.engine.connect() as conn:
            ...
            for row in vector_check:
                ...
                logger.info(f"      ✅ {table}: {correct}/{total} correct dimensions, {nulls} nulls")
            
            # Check JSONB validity
            logger.info("   Checking JSONB data integrity...")
            ...
            logger.info(f"      ✅ JSONB: {jsonb_check[1]}/{jsonb_check[0]} valid symbolic states")
    ...
    def generate_report(self):
        ...
        logger.info("\n" + "="*60)
        logger.info("KIMERA SWM FULL SYSTEM TEST REPORT")
        logger.info("="*60)
        ...
        if 'database' in self.test_results['tests']:
            ...
            for test, result in db_tests.items():
                if isinstance(result, dict) and 'status' in result:
                    ...
                    logger.info(f"      {status_icon} {test}: {time_str}")
        ...
        if 'api' in self.test_results['tests']:
            ...
            for test, result in api_tests.items():
                if isinstance(result, dict) and 'status' in result:
                    ...
                    logger.info(f"      {status_icon} {test}: {result.get('status', 'UNKNOWN')
        ...
        if 'concurrent' in self.test_results['tests']:
            ...
            if 'database_concurrent' in concurrent:
                ...
                logger.info(f"      ✅ Database: {db_conc.get('qps', 0)
            if 'api_concurrent' in concurrent:
                api_conc = concurrent['api_concurrent']
                logger.info(f"      ✅ API: {api_conc.get('successful', 0)
        ...
        if metrics.get('overall') == 'EXCELLENT':
            logger.info("🎉 SYSTEM TEST PASSED - All optimizations working perfectly!")
        elif metrics.get('overall') == 'GOOD':
            logger.info("✅ SYSTEM TEST PASSED - Minor issues detected")
        else:
            logger.warning("⚠️  SYSTEM TEST COMPLETED - Issues need attention")
        logger.info("="*60)
...
def main():
    ...
    logger.info("🚀 Starting Kimera SWM Full System Test")
    logger.info("This will test all optimizations and system components...\n")
