...
class AutomatedLearner:
    ...
    def learn_concept(self, concept_info):
        ...
        if grounding.confidence > 0.5:  # Only store confident groundings
            ...
            logger.info(f"  ✅ Grounded with confidence: {grounding.confidence:.2f}")
            ...
            logger.warning(f"  ⚠️ Low confidence: {grounding.confidence:.2f}")
            return False
    ...
    def _discover_relationships(self, concept, domain, grounding):
        ...
        for rel_type, target in concept_rels:
            if rel_type == "causes":
                ...
                logger.info(f"  🔗 Established: {concept} → {target}")
        ...
        if grounding.confidence > 0.7:
            ...
            for related_concept, similarity in related[:3]:  # Top 3
                logger.debug(f"  🔍 Related to: {related_concept} (similarity: {similarity:.2f})
    
    def run_learning_session(self, concepts_to_learn=10):
        """Run an automated learning session"""
        logger.info("\n" + "="*60)
        logger.info("🤖 AUTOMATED LEARNING SESSION")
        logger.info(f"Target: Learn {concepts_to_learn} new concepts")
        logger.info("="*60)
        ...
        logger.info("="*60)
        logger.info(f"  Concepts attempted: {attempted}")
        logger.info(f"  Concepts learned: {learned}")
        logger.info(f"  Success rate: {learned/attempted*100:.1f}%")
        logger.info(f"  New groundings: {final_groundings - initial_groundings}")
        logger.info(f"  New causal relations: {final_causal - initial_causal}")
        logger.info(f"  Remaining in queue: {len(self.learning_queue)
    ...
    def suggest_next_learning_goals(self):
        ...
        for i, suggestion in enumerate(suggestions, 1):
            logger.info(f"\n{i}. {suggestion['goal']}")
            logger.info(f"   Target: {suggestion['target']} more")
