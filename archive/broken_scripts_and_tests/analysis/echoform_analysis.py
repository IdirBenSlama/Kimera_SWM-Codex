...
def demonstrate_echoform_analysis():
    ...
    logger.debug("🔍 ECHOFORM ANALYSIS FOR KIMERA INSIGHTS")
    logger.info("="*60)
    logger.info("\nEchoForm is KIMERA's internal cognitive language that captures")
    logger.info("the deep structure of insights, revealing archetypal patterns,")
    logger.info("paradoxical tensions, and semantic relationships.\n")
    ...
    for i, example in enumerate(examples, 1):
        ...
        logger.info(f"📊 STRUCTURE: {analysis['structure_analysis']['completeness']}")
        logger.debug(f"🔍 CONCEPTS: {analysis['concept_analysis'].get('dominant_concept', 'N/A')
        logger.debug(f"🎭 ARCHETYPE: {analysis['archetype_analysis']['archetype_name']}")
        logger.info(f"⚡ PARADOX: {analysis['paradox_analysis']['paradox_statement']}")
        logger.info(f"🧩 COGNITIVE TYPE: {analysis['cognitive_signature']['cognitive_type']}")
        
        logger.info(f"\n💡 INTERPRETATION:")
        logger.info(analysis['interpretation_summary'])
        
        logger.debug(f"\n🔬 DETAILED ANALYSIS:")
        logger.info(f"   Archetype Meaning: {analysis['archetype_analysis']['archetype_meaning']}")
        logger.info(f"   Paradox Type: {analysis['paradox_analysis']['paradox_type']}")
        logger.info(f"   Cognitive Pattern: {analysis['archetype_analysis']['cognitive_pattern']}")
        logger.info(f"   Processing Style: {analysis['cognitive_signature']['processing_style']}")
        logger.info(f"   Abstraction Level: {analysis['cognitive_signature']['abstraction_level']}")
