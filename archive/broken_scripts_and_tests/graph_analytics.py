...
class KimeraGraphAnalytics:
    ...
    def generate_insights_report(self) -> Dict[str, Any]:
        ...
        logger.info(f"   Found {len(contradictions['contradiction_clusters'])
        logger.info(f"   Found {len(contradictions['contradiction_chains'])
        
        # Semantic communities
        logger.info("\n🌐 Discovering Semantic Communities...")
        communities = self.discover_semantic_communities()
        logger.info(f"   Discovered {len(communities)
        
        # Causal networks
        logger.info("\n🔗 Analyzing Causal Networks...")
        causal = self.analyze_causal_networks()
        logger.info(f"   Found {len(causal['causal_hubs'])
        logger.info(f"   Found {len(causal['causal_cycles'])
        ...
        logger.info(f"   Found {len(bridges)
    ...
    try:
        ...
        if report['contradictions']['contradiction_clusters']:
            ...
            for cluster in report['contradictions']['contradiction_clusters'][:3]:
                logger.info(f"   - {cluster['concept']}: {cluster['contradiction_count']} contradictions")
        
        # Largest communities
        if report['communities']:
            logger.info("\n   Largest Semantic Communities:")
            for comm in report['communities'][:3]:
                logger.info(f"   - Community {comm['communityId']}: {comm['size']} members")
                logger.info(f"     Members: {', '.join(comm['members'][:5])
        
        # Top causal hubs
        if report['causal_analysis']['causal_hubs']:
            logger.info("\n   Top Causal Hubs:")
            for hub in report['causal_analysis']['causal_hubs'][:3]:
                logger.info(f"   - {hub['concept']}: causes {hub['causes_count']}, caused by {hub['caused_by_count']}")
        ...
        if report['centrality']['pagerank']:
            ...
            for concept in report['centrality']['pagerank'][:5]:
                logger.info(f"   - {concept['concept']}: {concept['pagerank']:.4f}")
