...
class DatabaseMaintenance:
    ...
    def run_maintenance_cycle(self) -> None:
        ...
        try:
            ...
            logger.info(f"   Views refreshed: {sum(report['materialized_views'].values()
            logger.info(f"   Tables analyzed: {len(report['table_statistics'])
            logger.info(f"   Records cleaned: {sum(report['cleanup_results'].values()
            logger.info(f"   Active connections: {report['performance_monitoring']['database_stats']['active_connections']}")
            logger.info(f"   Database size: {report['performance_monitoring']['database_stats']['db_size']}")
...
def main():
    ...
    logger.debug("🔧 Kimera Database Maintenance")
    logger.info("=" * 40)
