        ...
        for table, column in conversions:
            try:
                ...
                if result and result[1] == 'json':
                    logger.info(f"   Converting {table}.{column} from JSON to JSONB...")
                    alter_query = text(f"ALTER TABLE {table} ALTER COLUMN {column} TYPE JSONB USING {column}::JSONB")
                    conn.execute(alter_query)
                    conn.commit()
                    logger.info(f"   ✅ Converted {table}.{column}")
                elif result and result[1] == 'jsonb':
                    logger.info(f"   ℹ️  {table}.{column} is already JSONB")
                else:
                    logger.warning(f"   ⚠️  {table}.{column} not found or not JSON type")
            except Exception as e:
                logger.error(f"   ❌ Error converting {table}.{column}: {e}")
                conn.rollback()
            ...
            for grounding_id, props in results:
                if props and 'Infinity' in str(props):
                    ...
                    try:
                        ...
                        logger.warning(f"   ⚠️  Could not fix JSON for {grounding_id}")
            
            conn.commit()
            logger.info(f"   ✅ Fixed {fixed_count} records with Infinity values")
            
        except Exception as e:
            logger.error(f"   ❌ Error cleaning Infinity values: {e}")
            conn.rollback()
    ...
    with engine.connect() as conn:
        ...
        for table in tables:
            try:
                ...
                logger.info(f"   ✅ Analyzed and vacuumed: {table}")
            except Exception as e:
...
def create_neo4j_sync_triggers(engine):
    ...
    with engine.connect() as conn:
        try:
            ...
            conn.commit()
            logger.info("   ✅ Created Neo4j sync triggers")
        except Exception as e:
...
def main():
    ...
    logger.info("🚀 Kimera SWM Database Optimization")
    logger.info("=" * 50)
    ...
    logger.info("\n📝 Next steps:")
    logger.info("1. Restart Kimera to use optimized database")
    logger.info("2. Monitor query performance")
    logger.info("3. Refresh materialized views periodically")
