...
def test_environment_setup():
    ...
    logger.debug("🔧 Test 1: Environment Setup")
    ...
    if missing_vars:
        ...
        logger.info("export NEO4J_PASS='password'")
        return False
    
    logger.info(f"✅ Neo4j URI: {os.getenv('NEO4J_URI')
    logger.info(f"✅ Neo4j User: {os.getenv('NEO4J_USER')
    logger.info("✅ Neo4j Password: [HIDDEN]")
    return True
...
def test_neo4j_connection():
    ...
    try:
        ...
        if driver_liveness_check():
            logger.info("✅ Neo4j database is reachable")
            return True
        else:
            logger.error("❌ Neo4j database is not responding")
            return False
...
def test_graph_models():
    ...
    try:
        ...
        logger.info(f"✅ Created test geoid: {test_geoid_id}")
        
        retrieved_geoid = get_geoid(test_geoid_id)
        if retrieved_geoid and retrieved_geoid.get("geoid_id") == test_geoid_id:
            logger.info("✅ Retrieved geoid successfully")
        else:
            logger.error("❌ Failed to retrieve geoid")
            return False
        ...
        logger.info(f"✅ Created test scar: {test_scar_id}")
        
        retrieved_scar = get_scar(test_scar_id)
        if retrieved_scar and retrieved_scar.get("scar_id") == test_scar_id:
            logger.info("✅ Retrieved scar successfully")
        else:
            logger.error("❌ Failed to retrieve scar")
            return False
        
        return True
        
    except Exception as e:
        logger.error(f"❌ Graph models test failed: {e}")
        return False
...
def test_cypher_queries():
    ...
    try:
        ...
        with get_session() as session:
            ...
            logger.info(f"✅ Test geoids found: {test_count}")
            
        return True
        
    except Exception as e:
        logger.error(f"❌ Cypher queries failed: {e}")
        return False
...
def test_vault_manager_integration():
    ...
    try:
        ...
        logger.info(f"✅ VaultManager created scar: {scar_db.scar_id}")
        
        # Verify it appears in Neo4j
        from backend.graph.models import get_scar
        neo4j_scar = get_scar(test_scar.scar_id)
        if neo4j_scar:
            logger.info("✅ Scar found in Neo4j via dual-write")
        else:
            logger.warning("⚠️  Scar not found in Neo4j (dual-write may have failed silently)
        
        return True
        
    except Exception as e:
        logger.error(f"❌ VaultManager integration test failed: {e}")
        return False
...
def main():
    ...
    logger.info("🚀 KIMERA Neo4j Integration Test Suite")
    logger.info("=" * 50)
    ...
    if failed == 0:
        ...
        logger.warning("⚠️  Some tests failed. Check Neo4j setup and configuration.")
        return 1
