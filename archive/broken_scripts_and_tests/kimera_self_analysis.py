...
class KimeraSelfAnalyzer:
    ...
    def check_system_status(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ KIMERA system is running")
                logger.info(f"   Active geoids: {status.get('active_geoids', 0)
                logger.info(f"   Total SCARs: {status.get('vault_a_scars', 0)
                return True
            else:
                logger.error(f"❌ KIMERA system error: {response.status_code}")
                return False
    ...
    def detect_self_contradictions(self) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"\n🔍 Detecting contradictions among {len(self.generated_geoids)
        ...
        for i, geoid_id in enumerate(self.generated_geoids[:5]):  # Limit to first 5 to avoid overload
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"   Geoid {i+1} ({geoid_id})
                    ...
                    logger.error(f"   ❌ Contradiction detection failed for {geoid_id}: {response.status_code}")
                    
            except Exception as e:
                logger.error(f"   ❌ Error detecting contradictions for {geoid_id}: {e}")
    ...
    def run_proactive_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"      Geoids scanned: {result.get('geoids_scanned', 0)
                logger.info(f"      Tensions found: {len(result.get('tensions_found', [])
...
def main():
    ...
    logger.info("🤖 KIMERA Self-Analysis: Having KIMERA Analyze Its Own Fine-Tuning Report")
    logger.info("=" * 80)
    ...
    if not analyzer.check_system_status():
        ...
        logger.info("   python -m uvicorn backend.api.main:app --host 0.0.0.0 --port 8000")
        return
    
    # Find the fine-tuning analysis report
    report_path = Path("FINE_TUNING_ANALYSIS.md")
    if not report_path.exists():
        logger.error(f"❌ Fine-tuning report not found: {report_path}")
        return
    ...
    if not geoids:
        logger.error("❌ No geoids created from the report")
        return
    ...
    logger.info(f"📄 Self-reflection report saved to: {output_path}")
    logger.info(f"\n📊 Summary:")
    logger.info(f"   • Geoids created from self-analysis: {len(geoids)
    logger.info(f"   • Self-contradictions detected: {len(contradictions)
    logger.info(f"   • Proactive tensions found: {len(proactive_results.get('tensions_found', [])
    logger.info(f"   • New SCARs from self-analysis: {proactive_results.get('scars_created', 0)
    ...
    if analyzer.analysis_results:
        ...
        logger.info(f"   • Performance Confidence: {avg_performance:.3f}/1.0")
        logger.info(f"   • Optimization Enthusiasm: {avg_optimization:.3f}/1.0")
        logger.info(f"   • Self-Contradiction Level: {'High' if contradictions else 'Low'}")
        logger.info(f"   • Meta-Cognitive Capability: {'Advanced' if len(geoids)
