...
def test_add_geoid(cfd_engine):
    ...
    field_embedding_np = field.embedding.cpu().numpy() if hasattr(field.embedding, 'cpu') else field.embedding
    expected_embedding = embedding / np.linalg.norm(embedding)
    assert np.allclose(field_embedding_np, expected_embedding)
