...
def main():
    logger.info("=== TESTING PHASE 4 IN ISOLATION ===\n")
    ...
    logger.info("Testing Phase 4 parameters: 16 threads, 256 features, depth 5")
    logger.info("Expected: 160 total operations (16 × 10)
    ...
    logger.info(f"Operations/sec: {result['ops_per_sec']:.2f}")
    ...
    if duration > 60:
        ...
        logger.info(f"Success rate: {result2['success_rate']:.1f}%")
        logger.info(f"Operations/sec: {result2['ops_per_sec']:.2f}")
