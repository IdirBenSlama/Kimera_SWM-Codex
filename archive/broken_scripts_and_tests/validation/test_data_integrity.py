...
from backend.vault.database import engine, Base, SessionLocal, ScarDB, GeoidDB
...
class TestDataIntegrity(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        if os.path.exists(TEST_DB_FILE):
            try:
                ...
                logger.warning(f"Warning: Could not remove test database {TEST_DB_FILE}. It may be locked.")
    ...
    def test_1_vault_balance_under_concurrent_load(self):
        ...
        logger.info("  Phase 1: Creating stable base geoids...")
        ...
        logger.info("  Phase 2: Creating conflicting geoids...")
        ...
        logger.info(f"  Phase 2: Complete. {len(new_geoid_ids)

        # Phase 3: Process contradictions for the new geoids against the stable base
        logger.info("  Phase 3: Processing contradictions...")
        ...
        logger.info(f"[RESULT] Total Scars Generated: {total_scars}")
        logger.info(f"[RESULT] Vault A: {vault_a_count} | Vault B: {vault_b_count}")
        ...
        logger.info("[SUCCESS] Vault balance maintained under concurrent load.")
    ...
    def test_2_acid_compliance_on_failure(self):
        ...
        # 1. Get initial state using SQLAlchemy to ensure proper connection
        with SessionLocal() as db:
            initial_geoid_count = db.query(GeoidDB).count()
        ...
        with patch('sqlalchemy.orm.Session.commit', side_effect=sqlite3.OperationalError("Simulated Commit Failure!")):
            ...
            try:
                ...
                # The API might return an error status instead of raising an exception
                self.assertNotEqual(response.status_code, 200)
            except Exception:
                # If the exception propagates, that's fine - it means the transaction was rolled back
                pass

        # 4. Verify that no data was written using SQLAlchemy
        with SessionLocal() as db:
            final_geoid_count = db.query(GeoidDB).count()

        logger.info(f"[RESULT] Initial Geoids: {initial_geoid_count} | Final Geoids: {final_geoid_count}")
        
        # Validation
        self.assertEqual(initial_geoid_count, final_geoid_count,
                         "A geoid was committed despite a transaction failure.")
        logger.error("[SUCCESS] Database correctly rolled back the failed transaction.")
