    ...
    def update_market_regime(self):
        ...
        if self.regime_duration > 10:  # Force regime change after 10 updates
            ...
            if random.random() < 0.3:  # 30% chance of regime change
                ...
                self.regime_duration = 0
                logger.info(f"🌪️  Market regime changed to: {self.current_regime.upper()
...
class HighVolatilityTester:
    ...
    def create_kimera_geoid(self, market_data: Dict) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ Failed to create geoid for {market_data['symbol']}: {response.status_code}")
                return None
                
        except Exception as e:
            logger.error(f"❌ Error creating geoid for {market_data['symbol']}: {e}")
            return None
    ...
    def process_market_data(self, market_data: Dict):
        ...
        if geoid_id:
            ...
            logger.info(f"{change_icon} {symbol}: ${price:.2f} ({change_pct:+.2f}%)
            
            # Detect contradictions for high volatility events
            if abs(change_pct) > 5.0 or event or market_data["volatility"] > 20:
                logger.debug(f"🔍 High volatility detected for {symbol}, checking contradictions...")
                ...
                if contradiction_result and contradiction_result.get("contradictions_detected", 0) > 0:
                    ...
                    logger.info(f"🚨 CONTRADICTION ALERT: {contradictions} contradictions, {scars} scars created!")
    ...
    def print_test_statistics(self):
        ...
        logger.info(f"📈 Symbols Tracked: {len(self.geoid_cache)
        logger.info(f"🧠 Geoids Created: {len(self.geoid_cache)
        logger.info(f"🔥 Total Contradictions: {self.contradiction_count}")
        logger.info(f"⚡ Total Scars: {self.scar_count}")
        logger.info(f"🌪️  Current Market Regime: {self.generator.current_regime.upper()
        ...
        if self.geoid_cache:
            ...
            for symbol, cache_data in self.geoid_cache.items():
                ...
                logger.info(f"   {symbol}: ${price:.2f} (Entropy: {entropy:.3f})
    ...
    def get_system_status(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"   Active Geoids: {status.get('active_geoids', 0)
                logger.info(f"   Vault A Scars: {status.get('vault_a_scars', 0)
                logger.info(f"   Vault B Scars: {status.get('vault_b_scars', 0)
                logger.info(f"   System Entropy: {status.get('system_entropy', 0)
                logger.info(f"   Cycle Count: {status.get('cycle_count', 0)
        except Exception as e:
    ...
    def run_test(self):
        ...
        logger.info("🌪️  KIMERA SWM HIGH VOLATILITY STREAM TEST")
        logger.info("=" * 60)
        logger.info("Simulating extreme market conditions with high volatility")
        logger.info("to stress test KIMERA's real-time contradiction detection")
        
        # Check KIMERA connection
        if not self.check_kimera_connection():
            logger.error("❌ KIMERA API not accessible. Please start with: python run_kimera.py")
            return
        
        logger.info("✅ KIMERA SWM API is accessible")
        logger.info(f"⏱️  Test Duration: {self.test_duration} seconds")
        ...
        logger.info(f"\n📈 Performance Metrics:")
        logger.info(f"   ⚡ Updates/Second: {updates_per_second:.2f}")
        logger.info(f"   🔥 Contradictions/Minute: {contradictions_per_minute:.2f}")
        logger.info(f"   🧠 Contradiction Rate: {(self.contradiction_count / max(len(self.geoid_cache)
        
        logger.info(f"\n🎯 Test Results:")
        if self.contradiction_count > 0:
            logger.info(f"   ✅ KIMERA successfully detected market contradictions")
            logger.info(f"   ✅ System handled high volatility streams")
            logger.info(f"   ✅ Real-time processing functional")
        else:
            logger.warning(f"   ⚠️  No contradictions detected (may indicate low volatility)
