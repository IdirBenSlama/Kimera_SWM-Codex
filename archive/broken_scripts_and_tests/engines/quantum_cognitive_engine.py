...
# KIMERA imports
from backend.utils.gpu_foundation import GPUFoundation, CognitiveStabilityMetrics, GPUValidationLevel
from backend.engines.geoid_mirror_portal_engine import GeoidMirrorPortalEngine, MirrorPortalState
    ...
    def _validate_quantum_gpu_integration(self) -> None:
        ...
        try:
            ...
            if "GPU" in str(e) and "not supported" in str(e):
                logger.warning("⚠️ GPU quantum simulation not supported - falling back to CPU")
                self.quantum_simulator = AerSimulator(method='statevector', device='CPU')
