...
from ..monitoring.thermodynamic_analyzer import ThermodynamicState, ThermodynamicCalculator
...
class CognitivePhaseTransitionDetector:
    ...
    def detect_phase_transition(self, geoids: List[GeoidState], 
                              temperature: float) -> Optional[PhaseTransitionSignature]:
        ...
        if (temperature > self.critical_temperature_threshold and 
            susceptibility > 10.0 and correlation_length > 5.0):
    ...
    def _calculate_susceptibility(self) -> float:
        ...
        susceptibility = np.mean(np.abs(order_diff[valid_indices] / temp_diff[valid_indices]))
        ...
        for i, geoid_a in enumerate(geoids):
            for geoid_b in geoids[i+1:]:
                ...
                if geoid_a.semantic_state and geoid_b.semantic_state:
                    common_keys = set(geoid_a.semantic_state.keys()) & set(geoid_b.semantic_state.keys())
                    if common_keys:
                        corr_sum = 0.0
    ...
    def _classify_transition_type(self, order_parameter: float, 
                                 susceptibility: float, 
                                 correlation_length: float) -> PhaseTransitionType:
    ...
    def _determine_universality_class(self, transition_type: PhaseTransitionType) -> str:
...
class AdvancedThermodynamicApplicationsEngine:
    ...
    async def run_phase_transition_analysis(self, 
        ...
        phase_transition = self.phase_detector.detect_phase_transition(geoids, temperature)
        ...
        results = {
            ...
            "critical_behavior": {
                "near_critical_point": temperature > self.phase_detector.critical_temperature_threshold,
    ...
    def create_information_heat_engine(self, demon_temperature: float, 
                                     reservoir_temperature: float) -> Dict[str, Any]:
        """Create Szilard engine for information-to-work conversion"""
        
        engine_id = f"SZILARD_{uuid.uuid4().hex[:8]}"
        
        # Calculate theoretical efficiency
        efficiency = 1 - (reservoir_temperature / demon_temperature) if demon_temperature > 0 else 0
        
        # Information-theoretic work extraction
        information_capacity = 1.0  # 1 bit
        max_work = information_capacity * BOLTZMANN_CONSTANT * demon_temperature * math.log(2)
    ...
    def simulate_thermodynamic_computing(self, computation_size: int) -> Dict[str, Any]:
        ...
        logger.info(f"   Best efficiency: {max(r['efficiency'] for r in results.values()):.3f}")
    ...
    def apply_fluctuation_dissipation_learning(self, geoids: List[GeoidState], 
                                             temperature: float = 2.0) -> Dict[str, Any]:
        ...
        avg_improvement = np.mean(learning_improvements) if learning_improvements else 0.0
    ...
    async def run_comprehensive_analysis(self, geoids: List[GeoidState], 
                                       temperature: float = 2.0) -> Dict[str, Any]:
        ...
        szilard_engine = self.create_information_heat_engine(
            demon_temperature=temperature * 1.5,
        ...
        learning_result = self.apply_fluctuation_dissipation_learning(geoids, temperature)
        ...
        computing_efficiency = max(r["efficiency"] for r in computing_result["modes"].values())
        learning_efficiency = min(learning_result["average_improvement"] * 10, 1.0)  # Scale and cap
        
        overall_efficiency = np.mean([phase_efficiency, heat_efficiency, computing_efficiency, learning_efficiency])
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            "advanced_applications": {
                "phase_detector": {
                    "transitions_detected": len(self.phase_detector.detected_transitions),
                    "order_parameter_history_size": len(self.phase_detector.order_parameter_history),
            ...
            "system_performance": {
                ...
                "average_efficiency": np.mean(self.efficiency_metrics["overall"]) if self.efficiency_metrics["overall"] else 0.0,
...
async def demonstrate_advanced_thermodynamic_applications():
    ...
    logger.info(f"Phase transitions: {'Detected' if results['applications']['phase_transitions']['phase_transition_detected'] else 'None'}")
    logger.info(f"Information work: {results['applications']['information_engines']['szilard_engine']['work_extracted']:.3f}")
    ...
    logger.info(f"\n🔧 SYSTEM STATUS:")
    logger.info(f"Applications run: {status['system_performance']['total_applications_run']}")
    logger.info(f"Average efficiency: {status['system_performance']['average_efficiency']:.3f}")
