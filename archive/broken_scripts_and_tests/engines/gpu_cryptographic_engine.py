...
class GPUCryptographicEngine:
    ...
    def generate_secure_key(self, key_size: int = 32) -> cp.ndarray:
        """Generate cryptographically secure key on GPU
        ...
        Returns:
            Secure random key
        """
        # Generate on CPU and transfer to GPU for security
        key = secrets.token_bytes(key_size)
        return cp.array(list(key), dtype=cp.uint8)
...
if __name__ == "__main__":
    ...
    logger.info(f"Encrypted {len(test_data)
    ...
    logger.info(f"Stream encrypted {len(stream_data)
