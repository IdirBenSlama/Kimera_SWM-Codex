...
class MetaCommentaryEliminator:
    ...
    def __init__(self):
        ...
        self.compiled_self_ref_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.self_reference_patterns
        ]
        
        logger.info("🛡️ Meta-Commentary Eliminator initialized")
        logger.info(f"   Tracking {sum(len(patterns) for patterns in self.pattern_categories.values())} dissociation patterns")
        logger.info(f"   Tracking {len(self.self_reference_patterns)} self-reference patterns")
    ...
    def analyze_text(self, text: str) -> DetectionResult:
        ...
        self_reference_score = min(1.0, self_reference_count / max(1, text_length * 0.1))
        direct_response_score = max(0.0, 1.0 - dissociation_score)
        
        # Generate suggestions
        suggestions = self._generate_improvement_suggestions(patterns_detected, dissociation_score)
    ...
    def _apply_pattern_correction(self, text: str, pattern_str: str, category: DissociationCategory) -> str:
    ...
    def _apply_general_improvements(self, text: str) -> str:
        ...
        text = re.sub(r'\b(it should be noted that|it is important to note that|it is worth noting that)\b', 
    ...
    def _generate_improvement_suggestions(self, patterns_detected: List[Tuple[str, DissociationCategory]], 
                                        dissociation_score: float) -> List[str]:
        """Generate specific suggestions for improvement"""
        
        suggestions = []
        
        if dissociation_score > 0.7:
            suggestions.append("High dissociation detected - consider rewriting from first-person perspective")
        elif dissociation_score > 0.4:
            suggestions.append("Moderate dissociation detected - reduce analytical language")
        ...
        if DissociationCategory.TECHNICAL_ANALYSIS in categories_found:
            suggestions.append("Replace technical analysis language with direct statements")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_patterns = sum(len(patterns) for patterns in self.pattern_categories.values())
