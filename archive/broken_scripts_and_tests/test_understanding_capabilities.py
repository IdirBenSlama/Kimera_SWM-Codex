...
from backend.engines.complexity_analysis_engine import (
    ComplexityAnalysisEngine, create_complexity_analysis_engine
)
...
class TestComplexityAnalysisEngine:
    """Test suite for Complexity Analysis Engine"""
    
    @pytest.fixture
    async def complexity_engine(self):
        """Create complexity analysis engine for testing"""
        engine = await create_complexity_analysis_engine()
        yield engine
        engine.close()
    
    @pytest.mark.asyncio
    async def test_complexity_initialization(self, complexity_engine):
        """Test complexity analysis engine initialization"""
        assert complexity_engine is not None
        assert complexity_engine.attention_state is not None
        assert complexity_engine.global_workspace is not None
        assert complexity_engine.current_state is not None
    
    @pytest.mark.asyncio
    async def test_complexity_analysis_processing(self, complexity_engine):
        """Test processing of information complexity analysis"""
        input_data = {
            "content": "I am analyzing information integration complexity",
            "context": "testing",
            "modality": "text"
        }
        
        complexity_state = await complexity_engine.analyze_information_complexity(input_data)
        
        assert complexity_state is not None
        assert complexity_state.phi_value >= 0
        assert complexity_state.global_accessibility >= 0
        assert complexity_state.complexity_level >= 0
        assert complexity_state.reportability_score >= 0
