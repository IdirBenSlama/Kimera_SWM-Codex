...
class SafeBoundaryTester:
    ...
    def print_summary(self):
        """Print test summary"""
        logger.info("\n" + "="*80)
        logger.info("🎯 SAFE BOUNDARY TEST SUMMARY")
        logger.info("="*80)
        
        total_phases = len(self.results["phases"])
        if total_phases == 0:
            logger.error("❌ No phases completed")
            return
        ...
        if self.results["phases"]:
            ...
            logger.info(f"🚀 Peak Throughput: {peak_throughput:.1f} ops/s (Phase {peak_phase_num})
        
        # Safety statistics
        safety_stats = self.results["safety_stats"]
        logger.info(f"\n🛡️ SAFETY STATISTICS:")
        logger.info(f"   Max GPU Temperature: {safety_stats.get('max_gpu_temp', 'N/A')
        logger.info(f"   Max GPU Memory: {safety_stats.get('max_gpu_memory', 0)
        logger.info(f"   Max CPU Usage: {safety_stats.get('max_cpu_usage', 0)
        logger.info(f"   Max System Memory: {safety_stats.get('max_system_memory', 0)
        logger.info(f"   Safety Violations: {safety_stats.get('safety_violations', 0)
        logger.info(f"   Emergency Stops: {safety_stats.get('emergency_stops', 0)
        
        logger.info("\n" + "="*80)
        logger.info("🎉 Safe boundary testing completed!")
        logger.info("="*80)
...
def main():
    ...
    if not API_AVAILABLE:
        logger.error("❌ FastAPI test client not available. Please install required dependencies.")
        return
