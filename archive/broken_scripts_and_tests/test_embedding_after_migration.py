...
def test_embedding_performance():
    ...
    logger.info("🧠 Testing BGE-M3 Embedding Performance...")
    ...
    # Initialize model
    logger.debug("🔧 Initializing BGE-M3 model...")
    ...
    for test_name, text in test_cases:
        ...
        logger.info(f"   ✅ Generated {len(embedding)
        logger.info(f"   ⏱️  Duration: {duration:.3f}s ({result['chars_per_sec']:.1f} chars/s)
    ...
    for result in results:
        logger.info(f"   {result['name']:10} | {result['duration']:6.3f}s | {result['chars_per_sec']:8.1f} chars/s | norm: {result['norm']:.3f}")
    
    logger.info("=" * 60)
    logger.info(f"   Average    | {avg_duration:6.3f}s | {avg_chars_per_sec:8.1f} chars/s")
    ...
    logger.info(f"   ✅ Processed {len(batch_texts)
    logger.info(f"   ⚡ Throughput: {len(batch_texts)
