...
class PerformanceSettings(BaseSettings):
    ...
    cache_size: int = Field(
        default=1000,
        env="KIMERA_CACHE_SIZE",
        ge=0,
        description="Default cache size"
    )
...
class ThresholdSettings(BaseSettings):
    ...
    memory_warning: float = Field(80.0, description="Memory usage warning threshold (%)")
    memory_critical: float = Field(95.0, description="Memory usage critical threshold (%)")
    gpu_memory_warning: float = Field(85.0, description="GPU memory usage warning threshold (%)")
    gpu_memory_critical: float = Field(95.0, description="GPU memory usage critical threshold (%)")
    ...
    response_time_warning: float = Field(2.0, description="Response time warning threshold (s)")
    response_time_critical: float = Field(5.0, description="Response time critical threshold (s)")
        ...
        if not self.is_production:
            ...
            if "api_keys" in data:
                for key in data["api_keys"]:
                    if isinstance(data["api_keys"][key], dict) and "secret_value" in data["api_keys"][key]:
