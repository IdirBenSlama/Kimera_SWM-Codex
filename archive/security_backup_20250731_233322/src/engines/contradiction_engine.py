...
class ContradictionEngine:
        ...
        try:
            ...
            for idx_pair in zip(i_indices, j_indices):
                ...
                semantic_conflict = self._fast_layer_conflict(
                    semantic_states[i_idx], semantic_states[j_idx],
            ...
            logger.debug(f"Vectorized processing: {comparison_count} comparisons, {len(tensions)} tensions found")
            return tensions
        ...
        for i in range(n):
            for j in range(i + 1, n):
                try:
                    ...
                    logger.warning(f"Failed to compute tension between {geoids[i].geoid_id} and {geoids[j].geoid_id}: {e}")
                    continue
        
        logger.debug(f"Pairwise processing: {n*(n-1)//2} comparisons, {len(tensions)} tensions found")
        return tensions
    ...
    def decide_collapse_or_surge(
        ...
        try:
            ...
            system_stability = (vault_pressure + semantic_cohesion + entropic_stability) / 3.0
    ...
    def check_insight_conflict(self, insight: InsightScar, existing_insights: List[InsightScar]) -> Optional[InsightScar]:
        ...
        try:
            ...
            for existing in existing_insights:
                ...
                if hasattr(insight, 'embedding_vector') and hasattr(existing, 'embedding_vector'):
