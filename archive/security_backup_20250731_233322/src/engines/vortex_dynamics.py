...
class Vortex:
    ...
    circulation: float        # Vortex strength (Î“)
...
class VortexField:
    ...
    vorticity_field: np.ndarray   # Vorticity Ï‰ = âˆ‡ Ã— v
    stream_function: np.ndarray   # Stream function Ïˆ
...
self.grid_size = grid_size
    ...
    def create_vortex(self,
        ...
        logger.info(f"Created {vortex_type.value} vortex at {position} with Î“={circulation}")
    ...
    def compute_vorticity(self, velocity_field: np.ndarray) -> np.ndarray:
        """
        Compute vorticity field Ï‰ = âˆ‚v/âˆ‚x - âˆ‚u/âˆ‚y
        ...
        while i < len(self.vortices):
            ...
            for j in range(i + 1, len(self.vortices)):
                ...
                if distance < merge_radius:
                    ...
                    merged.append(j)
                    logger.info(f"Merged vortices: Î“_total={total_circulation}")
