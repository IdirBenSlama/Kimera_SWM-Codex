...
class SelfTuningMonitor:  # noqa: D101
    ...
    def run_periodic_adjustment(self, recent_metrics: List[Dict[str, Any]]) -> None:  # noqa: D401
        ...
        if not recent_metrics:
            log.warning("No metrics supplied to SelfTuningMonitor â€“ skipping.")
            return
        ...
        if avg_entropy_delta > self.ENTROPY_DELTA_UPPER:
            ...
            log.info("Entropy growing quickly (Î”=%.2f). Tightening prune threshold to %.2f", avg_entropy_delta, prune_threshold)
        elif avg_entropy_delta < self.ENTROPY_DELTA_LOWER:
            prune_threshold = min(5.0, prune_threshold + 0.1)
            log.info("Entropy dropping (Î”=%.2f). Loosening prune threshold to %.2f", avg_entropy_delta, prune_threshold)
