...
def install_package(package):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.info(f"âœ— {package} failed: {result.stderr.split('ERROR:')[-1].strip()[:100]}...")
            return False
    except Exception as e:
        logger.info(f"âœ— {package} error: {str(e)}")
        return False
...
def main():
    ...
    logger.info("\n" + "=" * 60)
    logger.info(f"âœ“ Successfully installed: {len(succeeded)} packages")
    if failed:
        logger.info(f"âœ— Failed to install: {len(failed)} packages")
