...
class GPUManager:
                ...
                for cuda_path in possible_cuda_paths:
                    if os.path.exists(cuda_path):
                        ...
                        if versions:
                            ...
                            logger.info(f"🔧 Auto-detected CUDA path: {latest_version[1]}")
                            break
    ...
    def _detect_devices(self) -> None:
        ...
        try:
            ...
            for i in range(device_count):
                ...
                logger.info(f"📱 Device {i}: {device.name} ({device.capability_level.value})")
                logger.info(f"   Memory: {device.available_memory_gb:.1f}GB / {device.total_memory_gb:.1f}GB")
                logger.info(f"   Compute: {device.compute_capability[0]}.{device.compute_capability[1]}")
            ...
            if self.devices:
                ...
                logger.info(f"🎯 Selected device {self.current_device.device_id}: {self.current_device.name}")
    ...
    def _configure_optimal_settings(self) -> None:
        ...
        try:
            ...
            elif self.current_device.capability_level == GPUCapabilityLevel.HIGH_PERFORMANCE:
            ...
            logger.info(f"⚙️ Optimized for {self.current_device.capability_level.value} performance")
            
        except Exception as e:
            logger.warning(f"⚠️ GPU configuration optimization failed: {e}")
    
    def get_device_info(self, device_id: Optional[int] = None) -> Optional[Dict[str, Any]]:
    ...
    def optimize_for_task(self, task_type: str, **kwargs) -> Dict[str, Any]:
        ...
        if task_type == "geoid_processing":
            optimization_settings = {
                ...
                'precision': 'mixed' if self.capabilities.get('mixed_precision_supported') else 'float32',
