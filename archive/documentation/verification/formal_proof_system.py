...
class FormalProofSystem:
    ...
    def print_formal_proof(self, theorem_name: str):
        ...
        if theorem_name not in self.theorems:
            logger.info(f"Theorem '{theorem_name}' not found")
            return
        
        theorem = self.theorems[theorem_name]
        
        logger.info("=" * 80)
        logger.info(f"FORMAL PROOF: {theorem.statement}")
        logger.info("=" * 80)
        ...
        logger.info(f"VERIFICATION STATUS: {theorem.verification_status}")
        logger.info("=" * 80)

def main():
    """Main verification function"""
    logger.debug("🔬 FORMAL PROOF SYSTEM FOR KIMERA SET THEORY")
    logger.info("=" * 60)
    ...
    logger.info(f"Total Theorems: {report['total_theorems_verified']}")
    logger.info(f"Fully Verified: {report['verification_summary']['fully_verified']}")
    logger.info(f"Outline Verified: {report['verification_summary']['outline_verified']}")
    logger.info(f"Unverified: {report['verification_summary']['unverified']}")
