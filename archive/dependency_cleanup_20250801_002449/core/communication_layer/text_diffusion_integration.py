...
class TextDiffusionIntegration:
    ...
    async def generate_response(self, request: TextGenerationRequest) -> TextGenerationResult:
        """
        ...
        try:
            if self.diffusion_engine and hasattr(self.diffusion_engine, 'process_text_generation'):
    ...
    async def test_generation(self) -> bool:
        ...
        is_working = (
            result.generated_text is not None and
            len(result.generated_text.strip()) > 0 and
            result.confidence > 0
        )
