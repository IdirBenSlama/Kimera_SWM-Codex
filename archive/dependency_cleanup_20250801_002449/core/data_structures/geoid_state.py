...
class ProcessingMetadata:
    ...
    def add_processing_step(self, engine_name: str, operation: str, 
...
class GeoidState:
    ...
    def __post_init__(self):
        ...
        if self.semantic_state is None and self.symbolic_state is None:
            raise ValueError("GeoidState must have at least one of semantic_state or symbolic_state")
    ...
    def coherence_score(self) -> float:
        ...
        symbolic_coherence = len(self.symbolic_state.logical_predicates) / 10.0  # Normalize
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'semantic_state': {
                'embedding_vector': self.semantic_state.embedding_vector.tolist() if self.semantic_state else None,
                'confidence_scores': self.semantic_state.confidence_scores if self.semantic_state else None,
                'uncertainty_measure': self.semantic_state.uncertainty_measure if self.semantic_state else None,
                'semantic_entropy': self.semantic_state.semantic_entropy if self.semantic_state else None,
                'coherence_score': self.semantic_state.coherence_score if self.semantic_state else None,
            } if self.semantic_state else None,
            'symbolic_state': {
                'logical_predicates': self.symbolic_state.logical_predicates if self.symbolic_state else None,
                'symbolic_relations': self.symbolic_state.symbolic_relations if self.symbolic_state else None,
                'rule_activations': self.symbolic_state.rule_activations if self.symbolic_state else None,
                'symbolic_constraints': self.symbolic_state.symbolic_constraints if self.symbolic_state else None,
                'proof_chains': self.symbolic_state.proof_chains if self.symbolic_state else None,
            } if self.symbolic_state else None,
            'thermodynamic': {
                'cognitive_temperature': self.thermodynamic.cognitive_temperature if self.thermodynamic else None,
                'information_entropy': self.thermodynamic.information_entropy if self.thermodynamic else None,
                'free_energy': self.thermodynamic.free_energy if self.thermodynamic else None,
                'activation_energy': self.thermodynamic.activation_energy if self.thermodynamic else None,
                'dissipation_rate': self.thermodynamic.dissipation_rate if self.thermodynamic else None,
                'equilibrium_tendency': self.thermodynamic.equilibrium_tendency if self.thermodynamic else None,
            } if self.thermodynamic else None,
            'metadata': {
...
def create_relation_geoid(subject: str, predicate: str, object_: str) -> GeoidState:
    ...
    symbolic_state = SymbolicState(
        ...
        symbolic_relations={'subject': subject, 'predicate': predicate, 'object': object_},
        rule_activations={},
        symbolic_constraints=[],
        proof_chains=[]
    )
    ...
    return GeoidState(
        geoid_type=GeoidType.RELATION,
        symbolic_state=symbolic_state
    )


def create_hypothesis_geoid(hypothesis_text: str, confidence: float = 0.5) -> GeoidState:
