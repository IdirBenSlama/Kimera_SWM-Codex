...
class TaskManager:
    ...
    async def create_managed_task(
        ...
        async with self._lock:
            ...
            if running_tasks >= self.max_concurrent_tasks:
                raise RuntimeError(f"Maximum concurrent tasks ({self.max_concurrent_tasks}) exceeded")
    ...
    def get_running_tasks(self) -> Dict[str, TaskInfo]:
        ...
        return {
            name: info for name, info in self.tasks.items()
    ...
    async def shutdown(self, timeout: float = 30.0) -> None:
        ...
        if running_tasks:
            try:
                ...
                logger.warning(f"Shutdown timeout exceeded, {len(running_tasks)} tasks still running")
...
def get_task_manager() -> TaskManager:
    ...
    if _task_manager is None:
        raise RuntimeError(
            ...
            "Call initialize_task_manager() first."
        )
