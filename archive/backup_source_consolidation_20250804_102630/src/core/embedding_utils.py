...
def encode_text(text: str, device: Optional[str] = None) -> Union[np.ndarray, List[float]]:
    ...
    try:
        ...
        # Priority 3: Transformers model inference
            ...
            try:
                tokenizer = model['tokenizer']
                transformer_model = model['model'].to(target_device) # Ensure model is on correct device
...
def extract_semantic_features(text: str) -> Dict[str, float]:
    ...
    try:
        ...
        log.info(f"Raw embedding type: {type(embedding_raw)}, has cpu: {hasattr(embedding_raw, 'cpu')}, has numpy: {hasattr(embedding_raw, 'numpy')}")
