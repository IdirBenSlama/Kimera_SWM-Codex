...
from .background_job_manager import (
    BackgroundJobManager, get_job_manager,
    JobPriority, JobConfiguration
)
from .clip_service_integration import (
    CLIPServiceIntegration, get_clip_service
)
...
class ServicesIntegration:
    ...
    async def initialize(self, embedding_fn: Optional[Callable[[str], List[float]]] = None) -> bool:
        """
        ...
        with self._lock:
            ...
            try:
                ...
                service_status = ServiceStatus.HEALTHY if clip_status["available"] else ServiceStatus.DEGRADED
                ...
                if service_status == ServiceStatus.DEGRADED:
                    logger.warning("CLIP Service running in degraded mode (lightweight)")
                ...
                logger.info(f"✅ Services Integration initialized successfully in {self.initialization_time:.2f}s")
                return True
    ...
    def _add_system_maintenance_jobs(self):
        ...
        # Health check job
        self.job_manager.add_job(JobConfiguration(
    ...
    def _collect_metrics(self):
        ...
        if self.job_manager:
            try:
                metrics["services"]["job_manager"] = self.job_manager.get_metrics_summary()
            except Exception as e:
...
__all__ = ['ServicesIntegration', 'get_services_integration', 'ServiceStatus', 'ServiceHealth']