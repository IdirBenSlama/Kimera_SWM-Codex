...
from src.core.foundational_thermodynamic_engine import EpistemicTemperature, ThermodynamicMode
...
class EnhancedVortexBattery:
    ...
    def _find_optimal_evolution_vortex(self, energy_required: float) -> Optional[QuantumVortex]:
        """Find the best vortex to power signal evolution."""
        suitable_vortices = [
            v for v in self.active_vortices.values() 
            if v.stored_energy >= energy_required and v.state not in [VortexState.HEALING, VortexState.DEGRADED]
        ]
