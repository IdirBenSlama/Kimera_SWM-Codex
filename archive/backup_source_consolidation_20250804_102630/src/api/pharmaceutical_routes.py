...
from ..pharmaceutical.protocols.usp_protocols import USPProtocolEngine, DissolutionTestUSP711
...
class RawMaterialRequest(BaseModel):
    ...
    purity_percent: float = Field(..., ge=90.0, le=110.0, description="Purity percentage")
    moisture_content: float = Field(..., ge=0.0, le=10.0, description="Moisture content percentage")
    particle_size_d50: Optional[float] = Field(None, ge=0.0, description="Median particle size (μm)")
    bulk_density: Optional[float] = Field(None, ge=0.0, description="Bulk density (g/mL)")
    tapped_density: Optional[float] = Field(None, ge=0.0, description="Tapped density (g/mL)")
    potassium_confirmed: bool = Field(True, description="Potassium identification confirmed")
    chloride_confirmed: bool = Field(True, description="Chloride identification confirmed")
...
class FlowabilityRequest(BaseModel):
    ...
    angle_of_repose: float = Field(..., ge=0.0, le=90.0, description="Angle of repose (degrees)")

class FormulationRequest(BaseModel):
    """Formulation prototype creation request."""
    coating_thickness_percent: float = Field(..., ge=5.0, le=25.0, description="Coating thickness (%)")
    polymer_ratios: Dict[str, float] = Field(..., description="Polymer ratios")
    process_parameters: Dict[str, Any] = Field(default_factory=dict, description="Process parameters")

class DissolutionTestRequest(BaseModel):
    """Dissolution test request."""
    prototype_id: str = Field(..., description="Prototype ID to test")
    apparatus: int = Field(1, ge=1, le=2, description="USP apparatus (1=basket, 2=paddle)")
    ...
    reference_profile: Optional[Dict[str, List[float]]] = Field(None, description="Reference dissolution profile")
...
class BatchQualityRequest(BaseModel):
    ...
    specification_limits: Dict[str, Dict[str, float]] = Field(..., description="Specification limits")
...
def get_kcl_engine() -> KClTestingEngine:
    ...
    if kcl_engine is None:
        raise HTTPException(status_code=503, detail="KCl Testing Engine not initialized")
    return kcl_engine

def get_usp_engine() -> USPProtocolEngine:
    """Dependency to get USP protocol engine."""
    if usp_engine is None:
        raise HTTPException(status_code=503, detail="USP Protocol Engine not initialized")
    return usp_engine

def get_dissolution_analyzer() -> DissolutionAnalyzer:
    """Dependency to get dissolution analyzer."""
    if dissolution_analyzer is None:
        raise HTTPException(status_code=503, detail="Dissolution Analyzer not initialized")
...
def get_pharmaceutical_validator() -> PharmaceuticalValidator:
    ...
    if pharmaceutical_validator is None:
        raise HTTPException(status_code=503, detail="Pharmaceutical Validator not initialized")
...
async def characterize_raw_materials(
    ...
    engine: KClTestingEngine = Depends(get_kcl_engine)
) -> JSONResponse:
    """
        ...
        return JSONResponse(
            ...
            content={
                ...
                "data": {
                    ...
                    "quality_grade": "ACCEPTABLE" if specification.purity_percent >= 99.0 else "MARGINAL"
                },
...
async def analyze_flowability(
    ...
    try:
        ...
        result = engine.analyze_powder_flowability(
            ...
            request.tapped_density,
            request.angle_of_repose
        )
...
async def create_formulation_prototype(
    ...
    """
    try:
        logger.info(f"🧪 Creating formulation prototype with {request.coating_thickness_percent}% coating")
        ...
        return JSONResponse(
            ...
            content={
                ...
                "data": {
                    ...
                    "quality_assessment": {
                        "encapsulation_grade": (
                            "EXCELLENT" if prototype.encapsulation_efficiency >= 0.98
                            else "GOOD" if prototype.encapsulation_efficiency >= 0.95
                            else "ACCEPTABLE"
                        ),
                        "morphology_grade": (
                            "OPTIMAL" if "spherical" in prototype.particle_morphology.lower()
                            else "SUBOPTIMAL"
                        )
                    }
                },
...
async def perform_dissolution_test(
    request: DissolutionTestRequest,
    engine: KClTestingEngine = Depends(get_kcl_engine)
) -> JSONResponse:
    """
    ...
    """
    try:
        logger.info(f"🧪 Performing dissolution test for prototype {request.prototype_id}")
        ...
        if request.reference_profile:
            from ..pharmaceutical.analysis.dissolution_analyzer import DissolutionProfile
        ...
        return JSONResponse(
            ...
            content={
                ...
                "data": {
                    ...
                    "f2_similarity": usp_result.result_value if "f2" in str(usp_result.test_name).lower() else None
                },
                "timestamp": datetime.now().isoformat()
            }
        )
...
async def analyze_dissolution_kinetics(
    ...
    try:
        ...
        best_model = None
        if kinetic_models:
            best_model_name = min(kinetic_models.keys(), key=lambda k: kinetic_models[k].aic)
        ...
        return JSONResponse(
            ...
            content={
                ...
                "data": {
                    "fitted_models": {name: model.__dict__ for name, model in kinetic_models.items()},
                    "best_model": {
                    ...
                    "model_comparison": {
                        name: {
                            ...
                            "aic": model.aic,
                            "model_quality": (
                                "EXCELLENT" if model.r_squared >= 0.99
                                else "GOOD" if model.r_squared >= 0.95
                                else "FAIR" if model.r_squared >= 0.90
                                else "POOR"
                            )
                        }
            ...
            content={
                ...
                "data": {
                    ...
                    "executive_summary": {
                        ...
                        "critical_failures_count": len(validation_result.critical_failures),
                        "warnings_count": len(validation_result.warnings),
                        "regulatory_ready": validation_result.compliance_assessment.get('submission_readiness') == 'READY_FOR_SUBMISSION'
                    }
                },
                "timestamp": datetime.now().isoformat()
            }
        ...
        return JSONResponse(
            ...
            content={
                ...
                "data": {
                    ...
                    "batch_assessment": {
                        "overall_grade": (
                            "A" if quality_profile.quality_score >= 0.95
                            else "B" if quality_profile.quality_score >= 0.85
                            else "C" if quality_profile.quality_score >= 0.75
                            else "D"
                        ),
                        "release_recommendation": (
                            "APPROVED_FOR_RELEASE" if quality_profile.quality_score >= 0.80
                            else "HOLD_FOR_INVESTIGATION"
                        ),
                        "high_risk_attributes": [
                            attr for attr, risk in quality_profile.risk_assessment.items()
                            if "HIGH" in risk
                        ]
                    }
                },
    ...
    try:
        ...
        standards = {
            ...
            "stability_ich_q1a": {
                ...
                "accelerated": {"temperature": 40, "humidity": 75, "duration_months": 6},
                "intermediate": {"temperature": 30, "humidity": 65, "duration_months": 12}
            }
        }
