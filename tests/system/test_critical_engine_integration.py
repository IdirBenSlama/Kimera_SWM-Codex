...
async def test_critical_engine_integration():
    ...
    try:
        ...
        if understanding_engine is not None and understanding_engine != "initializing":
            ...
            understanding_result = await understanding_engine.understand_content(
                context
            )
            print(
                f"   - Understanding confidence: {understanding_result.confidence_score:.2f}"
            )
            print(
                f"   - Insights generated: {len(understanding_result.insights_generated)}"
            )
        ...
        if security_orchestrator is not None:
            ...
            print(
                f"   - Security policy active: {security_metrics.get('policy_active', False)}"
            )
            print(
                f"   - GPU cryptography available: {security_metrics.get('gpu_crypto_available', False)}"
            )
            print(
                f"   - Homomorphic encryption: {security_metrics.get('homomorphic_ready', False)}"
            )
        ...
        ready_engines = sum(
            1 for engine in critical_engines if final_status.get(engine, False)
        )
        ...
        if ready_engines >= 2:  # At least 2 critical engines working
            ...
            print(
                f"   {ready_engines}/{total_engines} critical engines successfully integrated"
            )
