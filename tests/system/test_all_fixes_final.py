...
async def test_all_fixes():
    ...
    try:
        from src.core.foundational_systems.cognitive_cycle_core import (
            WorkingMemorySystem, CognitiveContent
        )
    ...
    try:
        from src.core.foundational_systems.barenholtz_core import (
            AlignmentEngine, AlignmentMethod
        )
        ...
        if 0.0 <= result.alignment_score <= 1.0:
            print(f"   ✅ Barenholtz alignment working (score: {result.alignment_score:.3f})")
            passed += 1
        else:
            print(f"   ❌ Barenholtz alignment failed (score: {result.alignment_score})")
    except Exception as e:
    ...
    try:
        ...
        class MockContradiction:
            def detect_tension_gradients(self, geoids):
                class MockTension:
                    def __init__(self): self.geoid_a, self.geoid_b, self.tension_score = "a", "b", 0.6
                return [MockTension()]
        ...
        class MockGeoid:
            def __init__(self, gid): self.geoid_id, self.semantic_state = gid, {'concept': 0.5}
        ...
        test_system = {
            ...
            "vault_manager": MockVault(), "active_geoids": {"g1": MockGeoid("g1")}
        }
    ...
    try:
        ...
        cycle_core = CognitiveCycleCore(
            embedding_dim=32, num_attention_heads=1, working_memory_capacity=2, device="cpu"
        )
