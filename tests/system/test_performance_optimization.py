...
async def test_performance_optimization():
    ...
    try:
        ...
        for i in range(10):
            await put_cached(f"bench_key_{i}", {"data": i, "matrix": cpu_result[:10, :10].tolist()})
    ...
    try:
        ...
        print(f"  GPU Memory: {gpu_metrics.allocated_memory:.2f}GB / {gpu_metrics.total_memory:.2f}GB")
        
        # Cache metrics
        cache_stats = get_cache_stats()
        print(f"  Cache: {cache_stats.total_entries} entries, {cache_stats.hit_rate:.1%} hit rate")
        
        # Pipeline metrics
        pipeline_metrics = get_pipeline_metrics()
        print(f"  Pipeline: {pipeline_metrics.total_tasks} tasks, {pipeline_metrics.performance_score:.3f} score")
