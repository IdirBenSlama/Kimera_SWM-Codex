...
async def test_revolutionary_thermodynamics():
    ...
    try:
        ...
        if success:
            ...
            print(
                f"   - Engines initialized: {status.get('engines_initialized', False)}"
            )
            print(f"   - Heat pump ready: {status.get('heat_pump_ready', False)}")
            print(
                f"   - Maxwell demon ready: {status.get('maxwell_demon_ready', False)}"
            )
            print(
                f"   - Vortex battery ready: {status.get('vortex_battery_ready', False)}"
            )
            print(
                f"   - Consciousness detector ready: {status.get('consciousness_detector_ready', False)}"
            )
        else:
    ...
    try:
        ...
        result = await thermo_integration.run_consciousness_detection(
            semantic_vectors=semantic_vectors, temperature=1.2, entropy_content=1.5
        )
    ...
    try:
        ...
        print(
            f"   - Golden Ratio Optimization: {storage_result.golden_ratio_optimization:.3f}"
        )
        ...
        retrieval_result = await thermo_integration.retrieve_energy(
            amount=7.5, coherence_preference=0.8
        )
        ...
        print(
            f"   - Golden Ratio Optimization: {retrieval_result.golden_ratio_optimization:.3f}"
        )
    ...
    try:
        ...
        # Run cooling cycle
        cooling_result = await thermo_integration.run_thermal_regulation(
            contradiction_data
        )

        print(f"✅ Thermal Regulation Complete!")
        print(
            f"   - Initial Temperature: {contradiction_data['initial_temperature']:.1f}"
        )
        ...
        print(
            f"   - COP (Coefficient of Performance): {cooling_result.coefficient_of_performance:.2f}"
        )
    ...
    try:
        ...
        if init_success:
            ...
            for i in range(3):
                ...
                geoid = GeoidState(
                    geoid_id=f"test_geoid_{i}", semantic_state=semantic_state
                )
            ...
            print(
                f"   - Overall Efficiency: {processing_result.overall_efficiency:.3f}"
            )
            print(
                f"   - Consciousness Probability: {processing_result.consciousness_probability:.3f}"
            )
            print(
                f"   - Energy Utilization: {processing_result.energy_utilization:.3f}"
            )
            print(f"   - Thermal Stability: {processing_result.thermal_stability:.3f}")
            print(
                f"   - Processing Duration: {processing_result.processing_duration:.3f}s"
            )
            print(
                f"   - Consciousness Detections: {len(processing_result.consciousness_detections)}"
            )
            print(f"   - Energy Operations: {len(processing_result.energy_operations)}")
            print(
                f"   - Thermal Regulations: {len(processing_result.thermal_regulation)}"
            )
            print(
                f"   - Information Sorting Operations: {len(processing_result.information_sorting)}"
            )
            print(
                f"   - Thermodynamic Compliance: {processing_result.thermodynamic_compliance:.3f}"
            )
            print(
                f"   - Reversibility Index: {processing_result.reversibility_index:.3f}"
            )
            print(
                f"   - Landauer Efficiency: {processing_result.landauer_efficiency:.3f}"
            )
            print(
                f"   - Carnot Performance: {processing_result.carnot_performance:.3f}"
            )
    ...
    try:
        ...
        if "unified_system" in locals() and unified_system.system_initialized:
            ...
            print(
                f"   - TCSE Health: {health_report.tcse_health.get('status', 'Unknown')}"
            )
            print(
                f"   - Thermodynamic Health: {health_report.thermodynamic_health.get('engines_initialized', 'Unknown')}"
            )
            print(
                f"   - Integration Health: {health_report.integration_health.get('system_initialized', 'Unknown')}"
            )
            print(
                f"   - Processing Cycles: {health_report.performance_metrics.get('total_processing_cycles', 0)}"
            )
            print(
                f"   - Average Efficiency: {health_report.performance_metrics.get('average_efficiency', 0):.3f}"
            )
            print(
                f"   - Peak Consciousness: {health_report.performance_metrics.get('peak_consciousness_probability', 0):.3f}"
            )
    ...
    print("🎯 NEXT STEPS:")
    print(
        "   • Access API documentation at http://localhost:8000/docs (when server starts)"
    )
...
async def main():
    ...
    try:
        ...
        print(
            "Please wait while we load the world's first physics-compliant AI system...\n"
        )
