...
def test_understanding_engine_fix():
    ...
    try:
        ...
        if understanding_engine is not None:
            ...
            try:
                print(
                    f"   - Self-model available: {hasattr(understanding_engine, 'self_model')}"
                )
                if (
                    hasattr(understanding_engine, "self_model")
                    and understanding_engine.self_model
                ):
                ...
                print(
                    f"   - Understanding engine available (detailed test skipped: {type(e).__name__})"
                )
        ...
        ready_engines = sum(
            1 for engine in all_engines if system_status.get(engine, False)
        )
