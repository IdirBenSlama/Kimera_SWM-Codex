    ...
    async def execute_conservative_trade(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with BinanceConnector(
                api_key, secret_key, testnet=False
            ) as connector:
                ...
                if position_size < min_qty:
                    logger.warning(
                        f"⚠️ Position size {position_size:.8f} below minimum {min_qty:.8f}"
                    )
                    position_size = min_qty
                    actual_usd = position_size * current_price
                    logger.info(
                        f"   Adjusted position size to minimum: {position_size:.8f} BTC (${actual_usd:.2f})"
                    )

                # Format position size according to step size
                formatted_position_size = self._format_quantity(
                ...
                logger.info(
                    f"   Target price: ${target_price:,.2f} (+{self.config['target_profit_pct']}%)"
                )

                # Auto-confirm - user already confirmed they want to proceed
                print("\n🚨 REAL TRADE CONFIRMATION 🚨")
                print(
                    f"This will buy ${actual_position_value:.2f} worth of Bitcoin with REAL MONEY!"
                )
                ...
                order_status = await connector.get_order(
                    symbol=self.config["test_symbol"], order_id=buy_order["orderId"]
                )
    ...
    async def test_real_trading_connection(self) -> bool:
        ...
        try:
            ...
            if not secret_key:
                ...
                logger.info(
                    "   Set it with: export BINANCE_SECRET_KEY='your_secret_key'"
                )
                return False
            ...
            async with BinanceConnector(
                ...
                logger.info(
                    f"📊 Current {self.config['test_symbol']} price: ${current_price:,.2f}"
                )
                ...
                if usdt_balance < self.config["max_position_usd"]:
                    logger.warning(
                        f"⚠️ Low balance: Have ${usdt_balance:.2f}, need ${self.config['max_position_usd']}"
                    )
                    return False
