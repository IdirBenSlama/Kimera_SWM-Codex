...
class KimeraTestnetSystem:
    ...
    async def _run_component_orchestrator(self):
        ...
        while self.running:
            try:
                ...
                recent_data = (
                    list(self.market_data_buffer)[-10:]
    ...
    async def _run_performance_monitor(self):
        ...
        while self.running:
            try:
                ...
                logger.info(
                    f"   Market Events: {self.performance_metrics['total_events']:,}"
                )
                logger.info(
                    f"   Component Executions: {self.performance_metrics['component_executions']:,}"
                )
                logger.info(
    ...
    def _generate_trade_signal(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            recent_data = [
                md
                for md in list(self.market_data_buffer)[-20:]
                if md["symbol"] == symbol
            ]

            if len(recent_data) >= 10:
                price_change = (
                    recent_data[-1]["price"] - recent_data[-10]["price"]
                ) / recent_data[-10]["price"]
    ...
    async def _execute_trade(self, signal: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"📊 Trade: {signal['symbol']} {signal['side']} {signal['quantity']:.6f} @ {signal['price']:.2f}"
            )
