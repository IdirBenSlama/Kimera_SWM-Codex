...
class ConservativeTradeTest:
    ...
    async def initialize_system(self) -> bool:
        ...
        try:
            ...
            from src.trading.core.live_trading_manager import (
                LiveTradingConfig,
                LiveTradingManager,
                TradingMode,
                TradingPhase,
    ...
    async def verify_system_status(self) -> bool:
        ...
        try:
            ...
            if config.use_testnet:
                ...
                logger.info(
                    "To enable real trading, ensure KIMERA_USE_TESTNET is not set to 'true'"
                )
                return False
    ...
    async def execute_conservative_trade(self) -> Dict[str, Any]:
        ...
        try:
            ...
            from src.trading.core.trading_engine import MarketState, TradingDecision
            ...
            async with BinanceConnector(
                api_key, private_key_path, testnet=False
            ) as connector:
                ...
                logger.info(
                    f"📊 Current {self.test_config['test_symbol']} price: ${current_price:,.2f}"
                )
                ...
                logger.info(
                    f"💰 Conservative position size: {position_size} BTC (${max_usd})"
                )
                ...
                if usdt_balance < max_usd:
                    logger.error(
                        f"❌ Insufficient balance: ${usdt_balance:.2f} USDT available, need ${max_usd}"
                    )
                    return {
                        "success": False,
                ...
                target_price = current_price * (
                    1 + self.test_config["target_profit_pct"] / 100
                )
                stop_price = current_price * (
                    1 - self.test_config["stop_loss_pct"] / 100
                )

                logger.info(f"🎯 Trade targets:")
                logger.info(f"   Entry: ${current_price:,.2f}")
                logger.info(
                    f"   Target: ${target_price:,.2f} (+{self.test_config['target_profit_pct']}%)"
                )
                logger.info(
                    f"   Stop: ${stop_price:,.2f} (-{self.test_config['stop_loss_pct']}%)"
                )
    ...
    async def run_test(self) -> bool:
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.info(
                    f"   Target profit: {self.test_config['target_profit_pct']}%"
                )
                return True
            else:
