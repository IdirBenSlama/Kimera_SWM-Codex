...
-----END EC PRIVATE KEY-----"""
    ...
    def create_jwt_token(base_url: str, request_path: str, method: str = "GET") -> str:
        ...
        return jwt.encode(
            payload, private_key, algorithm="ES256", headers={"kid": API_KEY_ID}
        )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        data = response.json()
                        print(
                            f"      Data preview: {json.dumps(data, indent=2)[:150]}..."
                        )
                        successful_endpoints.append((path, description, data))
                    except Exception as e:
                        logger.error(
                            f"Error in test_alt_coinbase_apis.py: {e}", exc_info=True
                        )
