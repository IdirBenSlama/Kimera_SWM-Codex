...
async def test_vault_integration():
    ...
    try:
        ...
        patterns = await vault_interface.query_learned_patterns(
        ...
        logger.info(
            f"   ✅ Market insights retrieved: {len(market_insights.get('causal_chains', {}))}"
        )
        ...
        session_init = await vault_interface.initialize_trading_session(
            "test_session_init"
        )
        logger.info(
            f"   ✅ Session initialized: {session_init.get('session_id', 'unknown')}"
        )
        ...
        questions = await vault_interface.generate_market_questions(
            {"symbol": "BTC/USDT", "trend": "bullish", "volume": "increasing"}
        )
