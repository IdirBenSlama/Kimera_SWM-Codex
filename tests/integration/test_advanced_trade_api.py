    ...
    try:
        ...
        if trader.authenticate_and_test():
            ...
            if market_data:
                ...
                print(
                    f"   Horizontal analysis: {h_analysis['score']:.3f} ({h_analysis['action']})"
                )
                print(
                    f"   Vertical analysis: {v_analysis['score']:.3f} ({v_analysis['action']})"
                )
    ...
    try:
        ...
        for pair in ["BTC-EUR", "ETH-EUR", "SOL-EUR"]:
            ...
            if market_data:
                ...
                print(
                    f"   {pair}: H={h_score:.3f}, V={v_score:.3f}, Combined={h_score+v_score:.3f}"
                )
...
def main():
    ...
    if not test_credentials():
        print("\n❌ SETUP REQUIRED:")
        print(
            "1. Create Advanced Trade API keys at: https://www.coinbase.com/settings/api"
        )
    ...
    print(
        "Ready for live trading with: python kimera_omnidimensional_advanced_trade.py"
    )
