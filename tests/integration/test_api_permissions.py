...
class APIPermissionsTester:
            ...
            for balance in account_info.get("balances", []):
                ...
                if total > 0:
                    non_zero_balances.append(
                        {"asset": balance["asset"], "total": total}
                    )
        ...
        for test_name, test_func in tests:
            ...
            try:
                ...
                if test_name == "Account Access" and status == "PASSED":
                    ...
                    print(
                        f"      📊 Account Type: {account_result.get('account_type')}"
                    )
                    print(f"      🔄 Can Trade: {account_result.get('can_trade')}")
                    print(
                        f"      💰 Non-zero Balances: {account_result.get('non_zero_balances')}"
                    )
    ...
    def print_summary(self):
        ...
        passed_tests = sum(
            1 for result in self.results.values() if result.get("status") == "PASSED"
        )
