        ...
        try:
            ...
            if self.safety_level.value >= SafetyLevel.SIL_3.value:
                ...
                if np.std(values) > 0.1 * np.mean(values):
                    logger.warning(
                        f"Redundancy divergence in {name}: {values}"
                    )
            else:
            ...
            for name, metric in self.current_metrics.items():
                ...
                if metric.critical_threshold and metric.value >= metric.critical_threshold:
                    await self._trigger_safety_event(
                        name, SafetyLevel.SIL_3, "critical_threshold",
                    ...
                    await self._trigger_safety_event(
                        name, SafetyLevel.SIL_1, "warning_threshold",
                ...
                if metric.trend == "erratic":
                    await self._trigger_safety_event(
                        name, SafetyLevel.SIL_2, "erratic_behavior",
    ...
    async def _trigger_safety_event(
        ...
        for event in self.active_events.values():
            if (event.event_type == event_type and 
                metric_name in event.description and
                not event.resolved):
                ...
                if metric.trend == "rising" and metric.critical_threshold:
                    ...
                    if metric.rate_of_change > 0:
                        ...
                        if time_to_critical < 300:  # Less than 5 minutes
                            await self._trigger_safety_event(
                                name, SafetyLevel.SIL_2, "predictive_warning",
    ...
    def get_health_status(self) -> Tuple[HealthStatus, Dict[str, Any]]:
        ...
        with self._lock:
            ...
            report = {
                ...
                'severity_breakdown': {
                    k.name: v for k, v in severity_counts.items()
                },
                'metrics_summary': {}
            }
