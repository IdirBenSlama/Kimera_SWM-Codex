...
class TradingCycleRequest(BaseModel):
    ...
    max_risk_tolerance: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Maximum risk tolerance (0.0 to 1.0)"
    )
...
class RouteOptimizationRequest(BaseModel):
    ...
    strategy: Optional[str] = Field(default="best_price", description="Routing strategy")
...
class YieldOptimizationRequest(BaseModel):
    ...
    min_apy: Optional[float] = Field(default=0.03, ge=0.0, description="Minimum APY threshold")
    ...
    if _omnidimensional_engine is None:
        try:
            ...
            if ethical_governor is None:
                raise HTTPException(
                    status_code=503,
                    detail="Ethical governor not available"
                )
...
async def list_protocols(
    ...
    try:
        ...
        result = {
            ...
            "categories": {
                "spot_amm": len([p for p in engine.registry.protocols.values() if p.type.value == "spot_amm"]),
                "derivatives": len([p for p in engine.registry.protocols.values() if p.type.value == "derivatives"]),
                "cross_chain": len([p for p in engine.registry.protocols.values() if p.type.value == "cross_chain"]),
                "yield_optimizer": len([p for p in engine.registry.protocols.values() if p.type.value == "yield_optimizer"])
            }
        }
...
async def start_continuous_trading(
    ...
    try:
        logger.info(f"🚀 Starting continuous trading for {request.duration_hours} hours")
...
async def analyze_protocol_sentiment(
    ...
    try:
        ...
        if request.protocol not in engine.registry.protocols:
            raise HTTPException(
                status_code=404,
                detail=f"Protocol '{request.protocol}' not found"
            )
        ...
        logger.info(f"✅ Sentiment analysis completed for {request.protocol}: {sentiment_result['composite_score']:.3f}")
        return result
...
async def optimize_route(
    ...
    try:
        logger.info(f"🛣️ Optimizing route: {request.amount} {request.token_in} → {request.token_out}")
        ...
        logger.info(f"✅ Route optimization completed: {route_result['protocols_used']}")
        return result
...
async def scan_arbitrage_opportunities(
    ...
    try:
        ...
        result = {
            ...
            "total_estimated_profit": sum(opp.estimated_profit for opp in opportunities)
        }
...
async def optimize_yield(
    ...
    try:
        ...
        strategies = await engine.yield_optimizer.find_optimal_yield_strategies(request.assets)
        ...
        result = {
            ...
            "summary": {
                ...
                "average_apy": sum(s.get("apy", 0) for s in filtered_strategies) / max(len(filtered_strategies), 1),
    ...
    try:
        performance = {
            "overall_performance": {
                "total_profit": engine.total_profit,
                "total_trades": engine.total_trades,
                "average_profit_per_trade": engine.total_profit / max(engine.total_trades, 1)
            },
        ...
        for protocol in engine.registry.protocols.values():
            ...
            if protocol_type not in performance["protocol_statistics"]["protocols_by_type"]:
                performance["protocol_statistics"]["protocols_by_type"][protocol_type] = 0
