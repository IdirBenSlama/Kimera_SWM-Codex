...
class PerformanceMonitor:
    ...
    def __init__(self, sample_interval_seconds: float = 1.0):
        ...
        logger.info(f"PerformanceMonitor initialized (interval={sample_interval_seconds}s)")
            ...
            try:
                ...
                if last_disk_io:
                    metrics.disk_io_read_mb = (
                        ...
                        (1024 * 1024) / self.sample_interval
                    )
    ...
    def get_metrics_summary(self, window_seconds: int = 60) -> Dict[str, Any]:
        ...
        return {
            ...
            'disk_io': {
                ...
                'write_mb_per_sec': sum(m.disk_io_write_mb for m in recent) / len(recent)
            },
            'network': {
    ...
    def detect_bottlenecks(self) -> List[str]:
        ...
        if self.current_metrics.cpu_percent > 80:
            bottlenecks.append(f"High CPU usage: {self.current_metrics.cpu_percent:.1f}%")
        
        # Memory bottleneck
        if self.current_metrics.memory_percent > 85:
            bottlenecks.append(f"High memory usage: {self.current_metrics.memory_percent:.1f}%")
        
        # I/O bottleneck
        total_io = (self.current_metrics.disk_io_read_mb + 
        ...
        if self.current_metrics.active_threads > 100:
            bottlenecks.append(f"High thread count: {self.current_metrics.active_threads}")
