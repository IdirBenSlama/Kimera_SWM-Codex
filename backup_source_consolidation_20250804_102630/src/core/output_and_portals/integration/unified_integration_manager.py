...
from ..output_generation.multi_modal_output_generator import (
    MultiModalOutputGenerator, OutputModality, OutputQuality, OutputArtifact,
    get_multi_modal_output_generator
)
from ..portal_management.interdimensional_portal_manager import (
    InterdimensionalPortalManager, DimensionalSpace, PortalType, PortalStability,
...
class WorkflowType(Enum):
    ...
    OUTPUT_THEN_PORTAL = "output_then_portal"      # Generate output, then transfer via portal
    PORTAL_THEN_OUTPUT = "portal_then_output"      # Transfer via portal, then generate output
...
class ResourceScheduler:
    ...
    def schedule_task(self, task_id: str, component: str, resource_requirements: Dict[str, float]) -> bool:
        """
    ...
    def _can_allocate_resources(self, component: str, requirements: Dict[str, float]) -> bool:
        """Check if resources can be allocated"""
        current = self.current_allocations[component]

        total_cpu = sum(alloc["cpu"] for alloc in self.current_allocations.values())
        total_memory = sum(alloc["memory"] for alloc in self.current_allocations.values())
        total_gpu = sum(alloc["gpu"] for alloc in self.current_allocations.values())

        return (
            total_cpu + requirements.get("cpu", 0) <= self.resource_limits["cpu_percent"] and
            total_memory + requirements.get("memory", 0) <= self.resource_limits["memory_mb"] and
            total_gpu + requirements.get("gpu", 0) <= self.resource_limits["gpu_utilization"]
        )

    def _allocate_resources(self, task_id: str, component: str, requirements: Dict[str, float]) -> None:
    ...
    def _process_scheduling_queue(self) -> None:
        ...
        for queued_task in list(self.scheduling_queue):
            if self._can_allocate_resources(queued_task["component"], queued_task["requirements"]):
        ...
        for metric_name, threshold in self.alert_thresholds.items():
            if metric_name in metrics:
                ...
                if value > threshold:
                    ...
                    logger.warning(f"Performance alert: {metric_name}={value:.3f} > {threshold:.3f}")
...
class UnifiedIntegrationManager:
    ...
    def __init__(self,
        ...
        self.resource_scheduler = ResourceScheduler() if enable_resource_scheduling else None
    ...
    async def execute_workflow(self, request: IntegratedWorkflowRequest) -> IntegratedWorkflowResult:
        """
    ...
    async def _execute_workflow_by_type(self, request: IntegratedWorkflowRequest) -> IntegratedWorkflowResult:
        ...
        if request.workflow_type == WorkflowType.OUTPUT_ONLY:
            ...
            raise KimeraValidationError(f"Unsupported workflow type: {request.workflow_type}")

        return result

    async def _execute_output_only_workflow(self,
                                          request: IntegratedWorkflowRequest,
                                          result: IntegratedWorkflowResult) -> None:
        """Execute output-only workflow"""
        if not request.output_specification:
            raise KimeraValidationError("Output specification required for output-only workflow")
        ...
        try:
            ...
            if self.resource_scheduler:
                ...
                self.resource_scheduler.schedule_task(task_id, "output_generation", resource_requirements)

            # Generate output
            output_artifact = self.output_generator.generate_output(
                content_request=request.output_specification,
                modality=OutputModality(request.output_specification.get("modality", "text")),
                quality_level=OutputQuality(request.output_specification.get("quality", "standard")),
            ...
            logger.debug(f"Output-only workflow completed: {output_artifact.artifact_id}")
    ...
    async def _execute_portal_only_workflow(self,
                                          request: IntegratedWorkflowRequest,
                                          result: IntegratedWorkflowResult) -> None:
        """Execute portal-only workflow"""
        if not request.portal_specification:
            raise KimeraValidationError("Portal specification required for portal-only workflow")
        ...
        try:
            ...
            if self.resource_scheduler:
                ...
                self.resource_scheduler.schedule_task(task_id, "portal_management", resource_requirements)
    ...
    async def _execute_output_then_portal_workflow(self,
                                                 request: IntegratedWorkflowRequest,
                                                 result: IntegratedWorkflowResult) -> None:
        ...
        if result.success and result.output_artifacts:
            ...
            portal_spec["data_payload"] = {
                "output_artifacts": [artifact.artifact_id for artifact in result.output_artifacts],
    ...
    async def _execute_portal_then_output_workflow(self,
                                                 request: IntegratedWorkflowRequest,
                                                 result: IntegratedWorkflowResult) -> None:
    ...
    async def _execute_bidirectional_workflow(self,
                                            request: IntegratedWorkflowRequest,
                                            result: IntegratedWorkflowResult) -> None:
        ...
        if result.success:
            ...
            return_spec["source_dimension"], return_spec["target_dimension"] = \
                return_spec.get("target_dimension"), return_spec.get("source_dimension")
    ...
    async def _execute_multi_dimensional_workflow(self,
                                                request: IntegratedWorkflowRequest,
                                                result: IntegratedWorkflowResult) -> None:
        ...
        if not dimensions:
            raise KimeraValidationError("Multi-dimensional workflow requires dimension specification")
    ...
    def _monitoring_loop(self) -> None:
        ...
        while not self._stop_monitoring.is_set():
            try:
                ...
                # Check for alerts
                if health_report.overall_health in [SystemHealthStatus.CRITICAL, SystemHealthStatus.EMERGENCY]:
    ...
    def _collect_system_metrics(self) -> None:
        ...
        resource_status = self.resource_scheduler.get_resource_status() if self.resource_scheduler else {}

        # Combine metrics
        combined_metrics = {
            "output_generation_success_rate": output_stats.get("generation_stats", {}).get("successful_generations", 0) /
                                             max(1, output_stats.get("generation_stats", {}).get("total_generations", 1)),
            "portal_system_uptime": portal_status.get("system_info", {}).get("uptime_seconds", 0),
            "active_workflows": len(self.active_workflows),
            "resource_utilization_cpu": resource_status.get("utilization_ratios", {}).get("cpu", 0),
            "resource_utilization_memory": resource_status.get("utilization_ratios", {}).get("memory", 0),
            "resource_utilization_gpu": resource_status.get("utilization_ratios", {}).get("gpu", 0)
        }
    ...
    def _generate_health_report(self) -> SystemHealthReport:
        ...
        portal_status = self.portal_manager.get_system_status()
        resource_status = self.resource_scheduler.get_resource_status() if self.resource_scheduler else {}
        performance_report = self.performance_monitor.get_performance_report() if self.performance_monitor else {}

        # Determine overall health
        health_factors = []

        # Output generation health
        output_success_rate = output_stats.get("generation_stats", {}).get("successful_generations", 0) / \
                             max(1, output_stats.get("generation_stats", {}).get("total_generations", 1))
        ...
        return SystemHealthReport(
            ...
            component_health={
                "output_generation": {
                    ...
                    "active": True,
                    "cache_hit_rate": output_stats.get("cache_performance", {}).get("cache_hit_rate", 0)
                },
                "portal_management": {
                    "active_portals": active_portals,
                    "monitoring_active": portal_status.get("system_info", {}).get("monitoring_active", False),
            ...
            error_rates={
                ...
                "portal_operations": portal_status.get("operation_statistics", {}).get("safety_violations", 0) /
                                   max(1, portal_status.get("operation_statistics", {}).get("total_traversals", 1))
            },
            recommendations=self._generate_health_recommendations(overall_health),
            alerts=performance_report.get("alert_details", [])
        )

    def _generate_health_recommendations(self, health_status: SystemHealthStatus) -> List[str]:
        ...
        if health_status == SystemHealthStatus.OPTIMAL:
            ...
            recommendations.append("System operating well - monitor for potential improvements")
        elif health_status == SystemHealthStatus.DEGRADED:
            recommendations.append("Performance degradation detected - investigate resource allocation")
            recommendations.append("Consider reducing workflow complexity or increasing system resources")
    ...
    def _update_workflow_stats(self, result: IntegratedWorkflowResult, execution_time: float) -> None:
        ...
        self.integration_stats["uptime_seconds"] = (datetime.now() - self.start_time).total_seconds()
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "component_status": {
                ...
                "resource_scheduler": "active" if self.resource_scheduler else "disabled",
