...
class ScarState:
    ...
    # System Integration
    metrics: ScarMetrics = field(default_factory=lambda: ScarMetrics(
    ...
    def add_evidence(self, evidence_type: str, evidence_data: Dict[str, Any],
                    confidence: float, source_engine: str) -> None:
    ...
    def add_affected_geoid(self, geoid: GeoidState, relationship: str = "affected") -> None:
    ...
    def apply_resolution_action(self, action_type: str, description: str,
                               parameters: Dict[str, Any], applied_by: str) -> ResolutionAction:
    ...
    def mark_resolved(self, resolution_summary: str, effectiveness: float = 1.0) -> None:
    ...
    def calculate_impact_score(self) -> float:
        ...
        if self.evidence:
            evidence_confidence = sum(e.confidence_score for e in self.evidence) / len(self.evidence)
        else:
            evidence_confidence = 0.5
        
        # Combine factors
        impact_score = base_impact * (1 + geoid_impact * 0.3 + engine_impact * 0.2) * evidence_confidence
    ...
    def get_resolution_effectiveness(self) -> Optional[float]:
        ...
        successful_actions = sum(1 for action in self.resolution_actions if action.success)
    ...
    def __repr__(self) -> str:
        ...
        return (f"ScarState(id={self.scar_id[:8]}..., type={self.scar_type.value}, "
                f"severity={self.severity.value}, status={self.status.value})")


# Factory functions for common SCAR creation patterns
def create_logical_contradiction_scar(conflicting_geoids: List[GeoidState], 
                                     description: str) -> ScarState:
    ...
    for geoid in conflicting_geoids:
        ...
        scar.add_evidence(
            "logical_analysis",
            {"geoid_id": geoid.geoid_id, "predicates": geoid.symbolic_state.logical_predicates if geoid.symbolic_state else []},
            0.9,
...
def create_energy_violation_scar(geoid: GeoidState, expected_energy: float, 
                                actual_energy: float) -> ScarState:
...
def create_coherence_breakdown_scar(geoid: GeoidState, previous_coherence: float,
                                   current_coherence: float) -> ScarState:
...
def create_processing_error_scar(geoid: GeoidState, engine_name: str, 
                                error_message: str, error_context: Dict[str, Any]) -> ScarState:
...
def create_emergence_anomaly_scar(affected_geoids: List[GeoidState],
                                 anomaly_description: str,
                                 emergence_data: Dict[str, Any]) -> ScarState:
