...
class KClTestingEngine:
    ...
    def __init__(self, use_gpu: bool = True):
        ...
        if self.use_gpu:
            try:
                ...
                self.logger.info(f"   GPU Optimization: {self.gpu_optimization_enabled}")
    ...
    def _optimize_gpu_settings(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.debug("Memory fraction setting not available in this PyTorch version")
                pass
                
        except Exception as e:
            self.logger.warning(f"GPU optimization failed: {e}")
    
    def _track_performance(self, operation_type: str, gpu_time: float, 
                          cpu_time: float, memory_used: float, 
                          throughput: float, errors: int = 0, warnings: int = 0):
    ...
    def characterize_raw_materials_batch(self, 
                                       material_batches: List[Dict[str, Any]]) -> List[RawMaterialSpec]:
        """
        ...
        try:
            ...
            optimal_batch_size = self.batch_size_optimization['raw_material_characterization']
            results = []
            errors = 0
            warnings = 0
            ...
            self._track_performance(
                ...
                0, total_time, 0, throughput, errors, warnings
            )
            
            self.logger.info(f"✅ Batch characterization completed in {total_time:.2f}s")
    ...
    def _characterize_batch_gpu(self, batch_slice: List[Dict[str, Any]]) -> List[RawMaterialSpec]:
        ...
        try:
            ...
            purity_values = torch.tensor([b.get('purity_percent', 99.0) for b in batch_slice], 
                                       device=self.device, dtype=torch.float32)
            moisture_values = torch.tensor([b.get('moisture_content', 1.0) for b in batch_slice],
                                         device=self.device, dtype=torch.float32)
            ...
            usp_compliance_matrix[:, 2] = torch.ones(batch_size, device=self.device)  # Identity tests
            usp_compliance_matrix[:, 3] = torch.ones(batch_size, device=self.device)  # Impurity limits
    ...
    def _characterize_batch_cpu(self, batch_slice: List[Dict[str, Any]]) -> List[RawMaterialSpec]:
    ...
    def characterize_raw_materials(self, 
                                 material_batch: Dict[str, Any]) -> RawMaterialSpec:
        """
        ...
        try:
            ...
            if not isinstance(material_batch, dict):
                raise PharmaceuticalTestingException("Material batch must be a dictionary")
            
            required_fields = ['purity_percent', 'moisture_content']
            missing_fields = [field for field in required_fields if field not in material_batch]
            if missing_fields:
                raise PharmaceuticalTestingException(f"Missing required fields: {missing_fields}")
            ...
            if not isinstance(moisture, (int, float)) or moisture < 0:
                errors += 1
                raise PharmaceuticalTestingException(f"Invalid moisture content: {moisture}")
            ...
            if moisture > moisture_limit:
                ...
                warnings += 1
                self.logger.warning(f"Moisture content {moisture}% approaching USP limit")
            
            # Perform identification tests with error handling
            identification_tests = {}
            try:
                identification_tests['potassium_test'] = self._perform_potassium_identification(material_batch)
                identification_tests['chloride_test'] = self._perform_chloride_identification(material_batch)
            except Exception as e:
                errors += 1
            ...
            if bulk_density > 0 and tapped_density > 0 and bulk_density > tapped_density:
                warnings += 1
            ...
            self._track_performance(
                ...
                gpu_time, cpu_time, memory_used, throughput, errors, warnings
            )
            ...
            self._track_performance(
                "raw_material_characterization", 
                0, total_time, 0, 0, errors + 1, warnings
            )
            self.logger.error(f"❌ Raw material characterization failed: {e}")
            raise PharmaceuticalTestingException(f"Raw material characterization failed: {e}")
    ...
    def create_formulation_prototype(self,
        ...
        """
        try:
            self.logger.info(f"🧪 Creating formulation prototype with {coating_thickness}% coating...")
            ...
            morphology = self._assess_particle_morphology(coating_thickness, polymer_ratios)
    ...
    def perform_dissolution_test(self,
                               prototype: FormulationPrototype,
                               test_conditions: Dict[str, Any],
                               reference_profile: Optional[DissolutionProfile] = None) -> DissolutionProfile:
        """
        ...
        """
        try:
            self.logger.info(f"🧪 Performing dissolution test for {prototype.prototype_id}...")
            ...
            if reference_profile:
                f2_similarity = self._calculate_f2_similarity(profile, reference_profile)
            ...
            self.logger.info(f"✅ Dissolution test completed for {prototype.prototype_id}")
    ...
    def optimize_formulation(self,
                           target_profile: DissolutionProfile,
                           constraints: Dict[str, Any]) -> List[FormulationPrototype]:
        """
        ...
        try:
            ...
            for coating in coating_range:
                for ec_ratio in ethylcellulose_range:
                    ...
                    prototype = self.create_formulation_prototype(
                        coating, polymer_ratios, constraints.get('process_parameters', {})
                    )
                    
                    # Test dissolution
                    profile = self.perform_dissolution_test(
                        prototype, constraints.get('test_conditions', {}), target_profile
                    )
            ...
            candidates.sort(key=lambda x: x.dissolution_profile.f2_similarity or 0, reverse=True)
            ...
            raise PharmaceuticalTestingException(f"Formulation optimization failed: {e}")
    
    def validate_stability(self,
                         prototype: FormulationPrototype,
                         storage_conditions: List[Dict[str, Any]],
                         time_points: List[int]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for condition in storage_conditions:
                condition_name = f"{condition['temperature']}°C/{condition['humidity']}%RH"
                condition_results = {}
                
                for time_point in time_points:
                    # Simulate degradation over time
                    degradation_factor = self._calculate_degradation(condition, time_point)
            ...
            self.logger.info(f"✅ Stability validation completed for {prototype.prototype_id}")
    ...
    def _check_impurity_limits(self, material_batch: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _simulate_encapsulation_gpu(self, coating_thickness: float, 
                                  polymer_ratios: Dict[str, float],
                                  process_parameters: Dict[str, Any]) -> float:
        """Simulate encapsulation efficiency using GPU acceleration."""
        if not torch.cuda.is_available():
            return self._simulate_encapsulation_cpu(coating_thickness, polymer_ratios, process_parameters)
        
        try:
            with torch.amp.autocast('cuda', enabled=self.tensor_core_enabled):
                # Convert parameters to tensors for GPU computation
                coating_tensor = torch.tensor(coating_thickness, device=self.device, dtype=torch.float32)
                ...
                thickness_factor = torch.sigmoid((coating_tensor - optimal_thickness) / 3.0)
                ...
                ec_tensor = torch.tensor(ec_ratio, device=self.device, dtype=torch.float32)
                hpc_tensor = torch.tensor(hpc_ratio, device=self.device, dtype=torch.float32)
                ...
                spray_tensor = torch.tensor(spray_rate, device=self.device, dtype=torch.float32)
                ...
                # Combine all factors
                efficiency = (base_efficiency * thickness_factor * ec_factor * 
                            hpc_factor * temp_factor * spray_factor)
            ...
            self.logger.warning(f"GPU encapsulation simulation failed: {e}, falling back to CPU")
            return self._simulate_encapsulation_cpu(coating_thickness, polymer_ratios, process_parameters)
    
    def _simulate_encapsulation_cpu(self, coating_thickness: float,
                                  polymer_ratios: Dict[str, float], 
                                  process_parameters: Dict[str, Any]) -> float:
        ...
        thickness_factor = 1.0 / (1.0 + np.exp(-(coating_thickness - optimal_thickness) / 3.0))
    ...
    def _assess_particle_morphology(self, coating_thickness: float,
                                  polymer_ratios: Dict[str, float]) -> str:
    ...
    def _simulate_dissolution_kinetics(self, prototype: FormulationPrototype,
                                     test_conditions: Dict[str, Any],
    ...
    def _calculate_f2_similarity(self, profile1: DissolutionProfile,
                               profile2: DissolutionProfile) -> float:
        ...
        if len(profile1.release_percentages) != len(profile2.release_percentages):
            raise PharmaceuticalTestingException("Profiles must have same number of time points")
        
        # f2 = 50 * log{[1 + (1/n) * Σ(Rt - Tt)²]^(-0.5) * 100}
        n = len(profile1.release_percentages)
        sum_squared_diff = sum((r1 - r2)**2 for r1, r2 in 
                              zip(profile1.release_percentages, profile2.release_percentages))
    ...
    def _validate_dissolution_profile(self, profile: DissolutionProfile) -> None:
        ...
        for i, (time_point, release) in enumerate(zip(profile.time_points, profile.release_percentages)):
    ...
    def _calculate_degradation(self, condition: Dict[str, Any], time_months: int) -> float:
    ...
    def _simulate_degraded_dissolution(self, original_profile: DissolutionProfile,
                                     degradation_factor: float) -> DissolutionProfile:
    ...
    def generate_comprehensive_report(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info("📊 Generating comprehensive pharmaceutical testing report...")
    ...
    def _generate_recommendations(self, test_results: Dict[str, Any]) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("All tests passed - formulation ready for scale-up studies")
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        for metric in self.performance_metrics:
            ...
            operation_stats[op_type]['throughputs'].append(metric.throughput_ops_per_sec)
        ...
        for op_type, stats in operation_stats.items():
            summary_stats[op_type] = {
                ...
                'avg_gpu_time_ms': np.mean(stats['gpu_times']) if stats['gpu_times'] else 0,
                'avg_cpu_time_ms': np.mean(stats['cpu_times']) if stats['cpu_times'] else 0,
                ...
                'gpu_acceleration_ratio': (np.mean(stats['cpu_times']) / max(np.mean(stats['gpu_times']), 1e-6)) if stats['gpu_times'] and stats['cpu_times'] else 1.0
            }
        
        # Overall system performance
        total_operations = sum(len(stats['gpu_times']) for stats in operation_stats.values())
        total_errors = sum(sum(stats['error_counts']) for stats in operation_stats.values())
        total_warnings = sum(sum(stats['warning_counts']) for stats in operation_stats.values())
        ...
        system_health = {
            ...
            'overall_status': 'HEALTHY' if total_errors == 0 else 'DEGRADED' if total_errors < total_operations * 0.05 else 'CRITICAL'
        }
        ...
        return {
            ...
            'recommendations': self._generate_performance_recommendations(summary_stats, system_health)
        }
    
    def _generate_performance_recommendations(self, stats: Dict[str, Any], health: Dict[str, Any]) -> List[str]:
        ...
        if not self.gpu_optimization_enabled and torch.cuda.is_available():
            recommendations.append("GPU available but not optimized - enable GPU acceleration for better performance")
        
        # Throughput optimization
        for op_type, op_stats in stats.items():
            if op_stats['avg_throughput'] < 1.0:  # Less than 1 operation per second
                recommendations.append(f"Low throughput for {op_type} - consider batch processing optimization")
            
            if op_stats['gpu_acceleration_ratio'] < 2.0 and self.gpu_optimization_enabled:
                recommendations.append(f"Limited GPU acceleration for {op_type} - review GPU computation patterns")
        
        # Memory recommendations
        total_memory = sum(op_stats['avg_memory_mb'] for op_stats in stats.values())
        if total_memory > 1000:  # More than 1GB average
            recommendations.append("High memory usage detected - implement memory pooling and cleanup")
