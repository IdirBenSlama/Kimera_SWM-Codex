...
from ..config.settings import get_settings
    ...
    def calculate_thermodynamic_state(self, gpu_metrics: Dict[str, float], 
                                    performance_rate: float,
                                    field_count: int) -> ThermodynamicState:
        ...
        thermal_management_needed = gpu_metrics['temperature'] > self.optimal_temp_range[1]
    ...
    def _calculate_thermal_entropy(self, temp: float, util: float, power: float) -> float:
    ...
    def _calculate_computational_entropy(self, rate: float, count: int, memory: float) -> float:
    ...
    def _calculate_entropy_production_rate(self, thermal_S: float, comp_S: float, power: float) -> float:
    ...
    def _calculate_free_energy(self, thermal_S: float, comp_S: float, temp: float) -> float:
    ...
    def _calculate_thermodynamic_efficiency(self, rate: float, power: float, temp: float) -> float:
    ...
    def _determine_optimal_batch_size(self, thermal_S: float, reversibility: float, free_energy: float) -> int:
        ...
        if free_energy > 15.0:
            base_size = min(1000, int(base_size * 1.5))  # Larger batches when energy available
    ...
    def execute_thermodynamic_optimization(self, state: ThermodynamicState) -> OptimizationRecord:
        ...
        if state.should_optimize_reversibility:
            ...
            expected_improvement = (self.target_reversibility - state.reversibility_index) * 30.0  # 30% potential gain
        ...
        # Free energy exploitation
            ...
            expected_improvement = (state.free_energy - self.free_energy_threshold) * 2.0
    ...
    def run_optimized_cognitive_task(self, field_count: int) -> Dict[str, Any]:
        ...
        fields = self.field_engine.batch_create_fields(min(field_count, 100))  # Initial batch
        ...
        state = self.calculate_thermodynamic_state(gpu_metrics, initial_rate, len(fields))
        ...
        final_state = self.calculate_thermodynamic_state(final_gpu_metrics, final_rate, len(fields))
        
        # Calculate optimization effectiveness
        if optimization_record.decision_type != "none":
            improvement = ((final_state.thermodynamic_efficiency - state.thermodynamic_efficiency) / 
                          max(state.thermodynamic_efficiency, 0.001)) * 100.0
            optimization_record.actual_improvement = improvement
            optimization_record.reversibility_change = final_state.reversibility_index - state.reversibility_index
            optimization_record.efficiency_change = final_state.thermodynamic_efficiency - state.thermodynamic_efficiency
            optimization_record.validation_status = "success" if improvement > 0 else "neutral"
        ...
        return {
            ...
            "thermodynamic_improvement": final_state.thermodynamic_efficiency - state.thermodynamic_efficiency,
            "reversibility_improvement": final_state.reversibility_index - state.reversibility_index
        }
    ...
    def _monitoring_loop(self):
        ...
        while self.is_monitoring:
            try:
                ...
                state = self.calculate_thermodynamic_state(gpu_metrics, performance_rate, 1000)
                self.current_state = state
                self.thermodynamic_history.append(state)
                
                # Auto-optimization triggers
                if (state.reversibility_index < 0.4 or 
                    state.gpu_temperature > 50.0 or 
                    state.entropy_production_rate > 1.0):
                    
                    optimization = self.execute_thermodynamic_optimization(state)
                    self.logger.warning(f"🚨 Auto-optimization triggered: {optimization.decision_type}")
    ...
    def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        success_rate = (self.successful_optimizations / max(self.total_optimizations, 1)) * 100.0
        avg_efficiency_gain = np.mean(self.efficiency_gains) if self.efficiency_gains else 0.0
        
        recent_states = list(self.thermodynamic_history)[-10:] if self.thermodynamic_history else []
        ...
        return {
            ...
            "recent_reversibility_trend": [s.reversibility_index for s in recent_states],
            "recent_efficiency_trend": [s.thermodynamic_efficiency for s in recent_states],
            "recent_temperature_trend": [s.gpu_temperature for s in recent_states],
            "optimization_types": {
                record.decision_type: len([r for r in self.optimization_records if r.decision_type == record.decision_type])
                for record in self.optimization_records if record.decision_type != "none"
            }
        }
