...
from backend.engines.advanced_barenholtz_alignment_engine import AdvancedBarenholtzAlignmentEngine, AlignmentMethod
...
class AttentionMechanism:
    ...
    def __init__(self, embedding_dim: int = 768, num_heads: int = 8, device: str = "cpu"):
        ...
        # Attention matrices
        self.query_proj = torch.nn.Linear(embedding_dim, embedding_dim, device=self.device)
        self.key_proj = torch.nn.Linear(embedding_dim, embedding_dim, device=self.device)
        self.value_proj = torch.nn.Linear(embedding_dim, embedding_dim, device=self.device)
        self.output_proj = torch.nn.Linear(embedding_dim, embedding_dim, device=self.device)
    ...
    def compute_attention(self, 
        ...
        avg_attention = attention_weights.mean(dim=1).mean(dim=0)  # Average across heads and batch
...
class CognitiveCycleEngine:
    ...
    async def process_cognitive_cycle(self, 
        ...
        with self.processing_lock:
            ...
            for phase in self.cycle_phases:
                ...
                try:
                    ...
                    if asyncio.iscoroutinefunction(phase_processor):
                        phase_result = await phase_processor(current_content, cycle_context)
                    else:
            ...
            cycle_metrics = CycleMetrics(
                ...
                processing_efficiency=self._compute_processing_efficiency(phase_durations),
    ...
    async def _process_perception(self, content: CognitiveContent, context: Dict[str, Any]) -> CognitiveContent:
    ...
    async def _process_attention(self, content: CognitiveContent, context: Dict[str, Any]) -> CognitiveContent:
        """Process attention phase"""
        # Apply attention mechanism
        if content.data.dim() < 3:
            # Reshape for attention if needed
            data = content.data.unsqueeze(0) if content.data.dim() == 2 else content.data.unsqueeze(0).unsqueeze(0)
        else:
            data = content.data
        ...
        # Update content
        content.data = attended_output.squeeze(0) if attended_output.shape[0] == 1 else attended_output
        content.attention_weights = attention_weights.mean(dim=1).squeeze(0)  # Average across heads
        content.processing_depth += 1
        content.metadata['attention_processed'] = True
        
        return content
    
    async def _process_working_memory(self, content: CognitiveContent, context: Dict[str, Any]) -> CognitiveContent:
        ...
        if len(active_contents) > 1:
            ...
            all_embeddings = torch.stack([c.semantic_embedding for c in active_contents])
    ...
    async def _process_cognitive_processing(self, content: CognitiveContent, context: Dict[str, Any]) -> CognitiveContent:
        ...
        if processed_embedding.dim() > 0:
            ...
            processed_embedding = processed_embedding + 0.1 * torch.randn_like(processed_embedding)
    ...
    async def _process_integration(self, content: CognitiveContent, context: Dict[str, Any]) -> CognitiveContent:
        ...
        if semantic_network:
            ...
            for concept_id, concept_embedding in semantic_network.items():
                if concept_embedding.shape == content.semantic_embedding.shape:
                    ...
                    logger.info(f"[Kimera] Alignment method: {AlignmentMethod.ENSEMBLE_ALIGNMENT.value}, Score: {similarity:.4f}, Device: {'GPU' if torch.cuda.is_available() else 'CPU'}")
            content.metadata['similar_concepts'] = similarities
        content.processing_depth += 1
        content.metadata['integration_processed'] = True
        return content
    
    async def _process_consolidation(self, content: CognitiveContent, context: Dict[str, Any]) -> CognitiveContent:
        ...
        if content.priority > 0.5:
            ...
            self.cognitive_memory.semantic_network[concept_id] = content.semantic_embedding.clone()
    ...
    async def _process_output(self, content: CognitiveContent, context: Dict[str, Any]) -> CognitiveContent:
    ...
    def _compute_processing_efficiency(self, phase_durations: Dict[CyclePhase, float]) -> float:
    ...
    def _record_insight_event(self, cycle_id: str, content: CognitiveContent, metrics: CycleMetrics):
        ...
        logger.info(f"Insight generated in cycle {cycle_id} with score {metrics.insight_score:.3f}")
    ...
    def get_engine_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_insights": len([e for e in self.insight_events 
    ...
    def get_insights_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_insight_score": np.mean([e["insight_score"] for e in self.insight_events]),
            "top_insights": [
                {
