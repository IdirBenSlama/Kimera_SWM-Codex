...
class StatisticalProcessController:
    ...
    def _update_control_limits(self, metric_name: str):
        ...
        if STATSMODELS_AVAILABLE:
            self._update_model_based_control_limits(metric_name, metric_data)
        else:
            self._update_basic_control_limits(metric_name, metric_data)
    
    def _update_model_based_control_limits(self, metric_name: str, data: List[float]):
...
advanced_monitor = ForecastingAndControlMonitor()

def get_forecasting_and_control_monitor() -> ForecastingAndControlMonitor:
    """Get the global advanced statistical monitor instance"""
    return advanced_monitor

def initialize_forecasting_and_control_monitoring(entropy_monitor: Optional[EntropyMonitor] = None,
                                 system_observer: Optional[SystemObserver] = None) -> ForecastingAndControlMonitor:
    """Initialize advanced monitoring with existing monitors"""
    global advanced_monitor
    advanced_monitor = ForecastingAndControlMonitor(entropy_monitor, system_observer)
    return advanced_monitor