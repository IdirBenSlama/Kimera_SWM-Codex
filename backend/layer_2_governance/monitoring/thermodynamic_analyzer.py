...
from backend.utils.thermodynamic_utils import (
...
class ThermodynamicAnalyzer:
    ...
    def analyze_thermodynamic_state(self, geoids: List[GeoidState],
        ...
        energies = [sum(g.semantic_state.values()) for g in geoids if g.semantic_state] # Simplified energy for now
    ...
    def _calculate_free_energy(self, total_energy: float, entropy: float, temperature: float) -> float:
        return total_energy - temperature * entropy
    
    def _calculate_semantic_pressure(self, geoids: List[GeoidState], vault_capacity: int) -> float:
        if vault_capacity <= 0:
            return 0.0
        total_information = sum(len(g.semantic_state) + sum(g.semantic_state.values()) for g in geoids)
        return total_information / vault_capacity
    
    def _calculate_landauer_cost(self, bit_erasures: int, temperature: float) -> float:
        return bit_erasures * PHYSICS_CONSTANTS['normalized_kb'] * temperature * math.log(2)
    
    def _calculate_entropy_production_rate(self, entropy_before: float, entropy_after: float, time_delta: float) -> float:
        if time_delta <= 0: return 0.0
        return (entropy_after - entropy_before) / time_delta
    
    def _calculate_work_and_heat(self, energy_before: float, energy_after: float,
    ...
    def calculate_thermodynamic_efficiency(self, window_size: int = 100) -> Dict[str, float]:
        ...
        carnot_efficiency = calculate_theoretical_carnot_efficiency(avg_temp, self.thermal_reservoir_temp)
...
class ThermodynamicCalculator:
    ...
    def calculate_landauer_cost(bits_erased: int, temperature: float) -> float:
        ...
        """
        return bits_erased * PHYSICS_CONSTANTS['normalized_kb'] * temperature * math.log(2)
    
    @staticmethod
    def calculate_entropy_production_rate(entropy_before: float, entropy_after: float, time_delta: float) -> float:
        """
