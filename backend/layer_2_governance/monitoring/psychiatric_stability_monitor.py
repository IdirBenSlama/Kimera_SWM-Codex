...
class CognitiveCoherenceMonitor:
    ...
    def __init__(self, identity_coherence_threshold: float = 0.95,
...
class PersonaDriftDetector:
    ...
    def monitor_cognitive_stability(self, current_state: torch.Tensor) -> Dict[str, Any]:
        """
        ...
        if isinstance(current_state, dict):
            ...
            if 'processed_data' in current_state:
                ...
                import logging
                logging.warning("PersonaDriftDetector received dict without 'processed_data', creating simple representation")
        ...
        try:
            ...
            if min_size == 0:
                return {'drift_detected': False, 'stability_score': 1.0, 'note': 'empty_tensor'}
            ...
            current_norm = torch.nn.functional.normalize(current_windowed.unsqueeze(0), p=2, dim=1)
            baseline_norm = torch.nn.functional.normalize(baseline_windowed.unsqueeze(0), p=2, dim=1)
            cosine_similarity = torch.nn.functional.cosine_similarity(current_norm, baseline_norm, dim=1)
            ...
            def compute_entropy(tensor):
                hist = torch.histc(tensor, bins=20, min=torch.min(tensor).item(), max=torch.max(tensor).item())
            ...
            entropy_drift = abs(current_entropy - baseline_entropy) / (baseline_entropy + 1e-8)
            ...
            ensemble_drift = sum(score * weight for score, weight in zip(drift_scores, weights))
            
            # Adaptive threshold based on baseline variability
            adaptive_threshold = max(self.drift_threshold, self._compute_adaptive_threshold())
            ...
            drift_detected = (
                ensemble_drift > adaptive_threshold or
                p_value < 0.05 or
                any(score > self.drift_threshold * 2 for score in drift_scores)  # Any major algorithm detects
            )
            ...
            if drift_detected:
                ...
                results['severity'] = self._assess_drift_severity(ensemble_drift, adaptive_threshold)
    ...
    def _compute_spectral_drift(self, current_spectrum: torch.Tensor, baseline_spectrum: torch.Tensor) -> float:
        """
        ...
        spectral_diff = torch.mean((current_spectrum[:min_len] - baseline_spectrum[:min_len])**2)
    ...
    def _compute_moment_drift(self, current_moments: Dict[str, float], baseline_moments: Dict[str, float]) -> float:
        """
    ...
    def _compute_entropy_drift(self, current_state: torch.Tensor, baseline_state: torch.Tensor) -> float:
        """
    ...
    def _compute_wavelet_drift(self, current_state: torch.Tensor, baseline_state: torch.Tensor) -> float:
        """
    ...
    def _compute_drift_significance(self, drift_scores: list) -> float:
        ...
        if std_drift == 0 or n < 2:
            return 1.0 if mean_drift == 0 else 0.0 # No variance, significance is absolute
        ...
        p_value = 2 * (1 - torch.distributions.Normal(0, 1).cdf(torch.abs(t_statistic))).item()
...
class PsychoticFeaturePrevention:
    ...
    def __init__(self, reality_testing_threshold: float = 0.85,
    ...
    def assess_psychotic_risk(self, cognitive_output: torch.Tensor) -> Dict[str, Any]:
        ...
        try:
            if thought_coherence >= 0.95:
                ...
                adaptive_reality_threshold = max(0.70, self.reality_testing_threshold - 0.10)
