...
class GeoidState(Base):
    ...
    state_vector = Column(Vector(768), nullable=False)  # Changed from Text to Vector for pgvector compatibility
...
class CognitiveTransition(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    source_id = Column(UUID(as_uuid=True), ForeignKey("geoid_states.id"), nullable=False)
    target_id = Column(UUID(as_uuid=True), ForeignKey("geoid_states.id"), nullable=False)
...
class InsightDB(Base):
    ...
    application_domains = Column(ARRAY(Text))
...
class MultimodalGroundingDB(Base):
    ...
    visual_features = Column(JSONB, default={})
    auditory_features = Column(JSONB, default={})
    tactile_features = Column(JSONB, default={})
    temporal_context = Column(JSONB, default={})
    physical_properties = Column(JSONB, default={})
...
class SelfModelDB(Base):
    ...
    processing_capabilities = Column(JSONB, default={})
    knowledge_domains = Column(JSONB, default={})
    reasoning_patterns = Column(JSONB, default={})
    limitation_awareness = Column(JSONB, default={})
    self_assessment_accuracy = Column(Float, default=0.0)
    introspection_depth = Column(Integer, default=0)
    metacognitive_awareness = Column(JSONB, default={})
    created_at = Column(DateTime, default=datetime.utcnow)
...
class IntrospectionLogDB(Base):
    ...
    current_state_analysis = Column(JSONB, default={})
    predicted_state = Column(JSONB, default={})
    actual_state = Column(JSONB, default={})
    accuracy_score = Column(Float, default=0.0)
    processing_context = Column(JSONB, default={})
    created_at = Column(DateTime, default=datetime.utcnow)
...
class CompositionSemanticDB(Base):
    ...
    component_concepts = Column(ARRAY(Text))
    composition_rules = Column(JSONB, default={})
    emergent_meaning = Column(JSONB, default={})
    context_variations = Column(JSONB, default={})
...
class ConceptualAbstractionDB(Base):
    ...
    essential_properties = Column(JSONB, default={})
...
class ValueSystemDB(Base):
    ...
    learning_evidence = Column(JSONB, default={})
...
class GenuineOpinionDB(Base):
    ...
    supporting_values = Column(ARRAY(Text))
...
class EnhancedScarDB(Base):
    ...
    causal_understanding = Column(JSONB, default={})
    compositional_analysis = Column(JSONB, default={})
    contextual_factors = Column(JSONB, default={})
    introspective_accuracy = Column(Float, default=0.0)
    value_implications = Column(JSONB, default={})
    created_at = Column(DateTime, default=datetime.utcnow)
...
class EnhancedGeoidDB(Base):
    ...
    compositional_structure = Column(JSONB, default={})
    abstraction_level = Column(Integer, default=0)
    causal_relationships = Column(JSONB, default={})
    understanding_confidence = Column(Float, default=0.0)
    multimodal_groundings = Column(ARRAY(Text))
    created_at = Column(DateTime, default=datetime.utcnow)
...
class UnderstandingTestDB(Base):
    ...
    test_input = Column(JSONB, default={})
    expected_output = Column(JSONB, default={})
    actual_output = Column(JSONB, default={})
    understanding_accuracy = Column(Float, default=0.0)
    system_state = Column(JSONB, default={})
    created_at = Column(DateTime, default=datetime.utcnow)
...
class ComplexityIndicatorDB(Base):
    ...
    experience_report = Column(JSONB, default={})
    processing_context = Column(JSONB, default={})
