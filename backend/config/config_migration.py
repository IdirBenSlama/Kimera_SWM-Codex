...
class ConfigurationMigrator:
                    ...
                    for target in node.targets:
                        if isinstance(target, ast.Name):
                            ...
                            if any(keyword in target.id.lower() for keyword in 
                                   ["timeout", "port", "size", "limit", "max", "min"]):
    ...
    def _should_report_value(self, value: str, value_type: str) -> bool:
        ...
        skip_values = {
            "localhost", "127.0.0.1", "0.0.0.0",  # Common hosts
            "http://localhost", "https://localhost",  # Local URLs
            "__init__", "__main__", "__file__",  # Python internals
            "utf-8", "ascii",  # Common encodings
        }
    ...
    def _suggest_config_key(self, file_path: Path, value_type: str, context: str) -> str:
        ...
        for file_path, values in sorted(by_file.items()):
            ...
            for hv in sorted(values, key=lambda x: x.line_number):
                report_lines.extend([
                    ...
                    f"- Replacement:",
                    f"```python",
                    f"from backend.config import get_settings",
