...
import hashlib
import time
from typing import List, Any, Dict, Optional, Callable, TypeVar, Generic, Tuple
...
class DecisionVoter(Generic[T]):
        ...
        try:
            ...
            voter.average_execution_time_ms = (
                (voter.average_execution_time_ms * (voter.total_votes - 1) + 
                 execution_time) / voter.total_votes
            )
    ...
    def get_voter_health_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'health_percentage': healthy_count / len(self.voters) * 100 if self.voters else 0,
            'voters': []
        }
        ...
        if self.voting_history:
            ...
            report['recent_consensus_avg'] = np.mean([v.consensus_level for v in recent_votes])
            report['recent_confidence_avg'] = np.mean([v.confidence for v in recent_votes])
