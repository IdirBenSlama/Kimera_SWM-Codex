...
def to_state(row: GeoidDB) -> GeoidState:
    ...
    embedding_list = row.embedding.tolist() if hasattr(row.embedding, 'tolist') else row.embedding
    return GeoidState(
        geoid_id=row.geoid_id,
        semantic_features=row.semantic_features,
        symbolic_content=row.symbolic_content,
        created_at=row.created_at.isoformat(),
        metadata=row.metadata,
        embedding=embedding_list,
        echoform_text=row.echoform_text,
...
def create_scar_from_tension(
    ...
    embeddings = [np.array(g.embedding) for g in geoids_dict.values() if g.embedding is not None]
    if not embeddings:
...
def run_contradiction_processing_bg(
    ...
    if not contradiction_engine or not vault_manager:
        logger.error("Contradiction Engine or Vault Manager not available for background task.")
        return

    logger.info(f"Starting background contradiction processing for geoid: {body.trigger_geoid_id}")
    try:
        ...
        potential_matches_df = vault_manager.search_geoids_by_embedding(
            trigger_geoid_state.embedding, limit=body.search_limit + 1, include_distances=True
        )

        # Filter out the trigger geoid itself
        potential_matches_df = potential_matches_df[potential_matches_df['geoid_id'] != body.trigger_geoid_id]

        if potential_matches_df.empty:
            logger.info(f"No potential contradictions found for geoid {body.trigger_geoid_id}")
            return
        ...
        for _, row in potential_matches_df.iterrows():
            ...
            tension = contradiction_engine.check_contradiction(trigger_geoid_state, target_geoid_state)

            if contradiction_engine.is_significant(tension):
                logger.info(f"Significant tension {tension.tension_score:.3f} found between {trigger_geoid_state.geoid_id} and {target_geoid_state.geoid_id}")

                decision = "collapse" # Or some other logic
                scar, _ = create_scar_from_tension(
                    tension,
                    {trigger_geoid_state.geoid_id: trigger_geoid_state, target_geoid_state.geoid_id: target_geoid_state},
                    decision=decision
                )
        ...
        logger.error(f"Error during background contradiction processing: {e}", exc_info=True)
...
async def process_contradictions(
    ...
    """
    background_tasks.add_task(run_contradiction_processing_bg, body, contradiction_engine, vault_manager)
...
async def process_contradictions_sync(
    ...
    try:
        ...
        trigger_geoid_db = vault_manager.get_geoid(body.trigger_geoid_id)
        if not trigger_geoid_db:
            raise HTTPException(status_code=404, detail=f"Trigger geoid {body.trigger_geoid_id} not found.")
        ...
        for _, row in potential_matches_df.iterrows():
            ...
            if contradiction_engine.is_significant(tension):
                ...
                results.append({
                    ...
                    "scar_created": scar.to_dict()
                })

        return {"message": f"Processed {len(potential_matches_df)} potential contradictions, found {len(results)} significant.", "results": results}

    except Exception as e:
        logger.error(f"Error during synchronous contradiction processing: {e}", exc_info=True)
...
async def detect_contradictions(
    ...
    try:
        ...
        for pair in geoid_pairs:
            if len(pair) != 2:
                results.append({"pair": pair, "error": "Each pair must contain exactly 2 geoid IDs"})
                continue
            ...
            try:
                ...
                if not geoid1_db or not geoid2_db:
                    results.append({
                        "pair": pair,
                        "error": "One or both geoids not found"
                    })
                    continue
                ...
                # Check contradiction
                tension = contradiction_engine.check_contradiction(geoid1_state, geoid2_state)

                results.append({
                    "pair": pair,
                    "is_contradiction": contradiction_engine.is_significant(tension),
...
async def detect_tension(
    ...
    try:
        ...
        for pair in geoid_pairs:
            if len(pair) >= 2:
                # Create mock tension for testing
                tensions.append({
                    "geoid_pair": pair,
                    "tension_score": 0.5,
                    "significance": "medium",
