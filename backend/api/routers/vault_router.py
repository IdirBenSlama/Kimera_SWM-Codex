    ...
    if _vault_monitor is None:
        try:
            ...
            if vault_manager and contradiction_engine:
                _vault_monitor = RealTimeVaultMonitor(vault_manager, contradiction_engine)
                logger.info("✅ Vault monitor initialized")
            else:
                logger.warning("❌ Could not initialize vault monitor - missing dependencies")
...
async def query_vault(query: dict, vault: VaultManager = Depends(get_vault_manager)):
    ...
    try:
        ...
        if query_type == 'pattern_search':
            ...
            result = await cognitive_interface.query_market_insights(symbol, timeframe, context)
        ...
        logger.info(f"✅ VAULT QUERY COMPLETED: {query_type} - {len(str(result))} bytes returned")
        return {"status": "success", "query_type": query_type, "domain": domain, "result": result}
...
async def get_vault_contents(vault_id: str, limit: int = 10):
    ...
    try:
        ...
        if hasattr(vault_manager, 'get_vault_contents'):
            ...
            contents = [{"scar_id": s.scar_id, "timestamp": s.timestamp, "reason": s.reason} for s in contents]
...
async def get_recent_geoids(limit: int = 5):
    ...
    return {
        "recent_geoids": [],
        "count": 0,
        "message": "No recent geoids available"
    }
        ...
        for scar in scars:
            if hasattr(scar, 'to_dict'):
                ...
                result.append({
                    ...
                    "timestamp": getattr(scar, 'timestamp', datetime.now()).isoformat() if hasattr(scar, 'timestamp') else datetime.now().isoformat(),
...
async def rebalance_vaults(by_weight: bool = False):
    ...
    try:
        ...
        if hasattr(vault_manager, 'rebalance_vaults'):
            ...
            return {"status": "rebalance_not_implemented", "message": "Rebalancing functionality not yet available"}
    except Exception as e:
...
async def speak_geoid(geoid_id: str):
    ...
    try:
        ...
        if hasattr(vault_manager, 'get_geoid'):
            ...
            raise HTTPException(status_code=501, detail="Geoid retrieval not yet implemented")
...
async def store_in_vault(
    ...
    logger.info(f"/vault/store called with key={key}, value={value}, metadata={metadata}")
    try:
        vault_manager = get_vault_manager()
        if not vault_manager:
