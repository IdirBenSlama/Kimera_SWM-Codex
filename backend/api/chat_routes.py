...
class ChatRequest(BaseModel):
    ...
    cognitive_mode: Optional[str] = None  # "standard", "cognitive_enhanced", "persona_aware", "neurodivergent"
...
class SimpleChatService:
    ...
    @staticmethod
    def generate_response(message: str, cognitive_mode: str = "standard") -> Dict[str, Any]:
...
async def handle_chat(chat_request: ChatRequest, request: Request):
    ...
    try:
        ...
        logger.info(f"Using fallback chat service for session '{chat_request.session_id}' in mode '{chat_request.mode}'")
        
        # Generate response using simple chat service
        result = SimpleChatService.generate_response(
            chat_request.message, 
            chat_request.cognitive_mode or "standard"
        )
        ...
        logger.info(f"Confidence: {result['confidence']:.3f}, Coherence: {result['semantic_coherence']:.3f}")
        ...
        raise HTTPException(status_code=500, detail="Error processing your message in the cognitive engine.")
