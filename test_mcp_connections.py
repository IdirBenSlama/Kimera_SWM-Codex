...
def test_server_startup(command: List[str], server_name: str, timeout: int = 10) -> Dict[str, any]:
...
def main():
    ...
    print(f"Status: {'✅' if test_db_result['status'] == 'success' else '❌'} {test_db_result}")
    
    print(f"\n📊 Kimera Database: {kimera_db_path}")
    kimera_db_result = test_database_access(kimera_db_path)
    print(f"Status: {'✅' if kimera_db_result['status'] == 'success' else '❌'} {kimera_db_result}")
    ...
    with open(results_file, 'w') as f:
        json.dump({
            ...
            "servers": results,
            "databases": {
                "test_db": test_db_result,
                "kimera_db": kimera_db_result
            }
        }, f, indent=2)
