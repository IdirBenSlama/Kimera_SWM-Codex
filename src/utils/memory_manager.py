...
class MemoryManager:
    ...
    def allocate_tensor(
        ...
        try:
            ...
            if device.type == "cuda":
                ...
                required_memory = self._estimate_tensor_memory(
                    shape, dtype or torch.float32
                )

                if required_memory > available_memory:
                    logger.warning(
                        f"Insufficient GPU memory. Required: {required_memory / 1024**2:.1f}MB, Available: {available_memory / 1024**2:.1f}MB"
                    )
    ...
    def cleanup_gpu_cache(self):
        ...
        try:
            ...
            logger.info(
                f"   GPU Memory - Allocated: {allocated / 1024**2:.1f}MB, Cached: {cached / 1024**2:.1f}MB"
            )
    ...
    def emergency_gpu_cleanup(self):
        ...
        try:
            ...
            logger.warning(
                f"🚨 Emergency GPU cleanup completed - cleared {gpu_objects} GPU objects"
            )
    ...
    def check_for_leaks(self):
        ...
        for obj_type, count in current_counts.items():
            if obj_type in self.object_counts:
                ...
                self.growth_history[obj_type] = [
                    (time, growth)
                    for time, growth in self.growth_history[obj_type]
                    if time > cutoff_time
                ]
