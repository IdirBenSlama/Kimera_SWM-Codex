...
def BBANDS(
    close: np.ndarray, timeperiod: int = 20, nbdevup: float = 2.0, nbdevdn: float = 2.0
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Bollinger Bands
...
logger.info(
    f"[OK] TA-Lib Fallback v{__version__} loaded - {len(get_available_functions())} functions available"
)
