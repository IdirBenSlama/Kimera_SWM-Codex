...
class IntentionalProcessor:
    ...
    def __init__(self):
        ...
        self.goal_queue: List[Tuple[float, str]] = (
            []
        )  # Priority queue (priority, goal_id)
        ...
        self.novelty_memory: Dict[str, List[float]] = defaultdict(
            list
    ...
    def _goal_directed_attention(
        self, input_data: Any, goal_ids: List[str]
    ) -> Dict[str, Any]:
        ...
        for concept, attention_weight in top_concepts:
            if attention_weight > 0.1:  # Attention threshold
                ...
                for goal_id in goal_ids:
                    goal = self.goals.get(goal_id)
                    if goal:
                        connection = self._analyze_concept_for_goal(
                            concept, goal, input_data
                        )
                        if connection:
    ...
    def _analyze_concept_for_goal(
        self, concept: str, goal: Goal, context: Any
    ...
    def _calculate_novelty(self, concept: str) -> float:
        ...
        recency_factor = min(
            1.0, (datetime.now().timestamp() - recency) / 3600
        )  # Hours
    ...
    def _explore_curiosities(
        ...
        for signal in curiosity_signals[:3]:  # Explore top 3
            ...
            if signal.exploration_type == "feature":
                discovery["findings"]["new_features"] = self._discover_features(
                    signal.target
                )
            elif signal.exploration_type == "relation":
                discovery["findings"]["new_relations"] = self._discover_relations(
                    signal.target
                )
            elif signal.exploration_type == "pattern":
                discovery["findings"]["new_patterns"] = self._discover_patterns(
                    signal.target
                )
    ...
    def _assess_goal_relevance(
        self, focused_content: Dict[str, Any], goal_ids: List[str]
    ) -> Dict[str, float]:
    ...
    def _update_goal_progress(
        self, goal_ids: List[str], relevance_scores: Dict[str, float]
    ):
    ...
    def _determine_strategy(self, goal_ids: List[str], allow_exploration: bool) -> str:
    ...
    def _calculate_processing_confidence(
        ...
        if focused_content:
            avg_attention = np.mean(
                [item["attention_weight"] for item in focused_content.values()]
            )
