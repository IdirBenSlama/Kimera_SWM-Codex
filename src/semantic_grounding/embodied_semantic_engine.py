...
class EmbodiedSemanticEngine:
    ...
    def process_concept(
        ...
        logger.info(
            f"Concept '{concept}' grounded with confidence: {grounding.confidence}"
        )

        return grounding

    def _ground_visually(
        self, concept: str, context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Ground concept in visual experience"""
        try:
            visual_features = self.multimodal_processor.ground_visually(
                concept, context
            )
            return {
    ...
    def _ground_auditorily(
        self, concept: str, context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Ground concept in auditory experience"""
        try:
            audio_features = self.multimodal_processor.ground_auditorily(
                concept, context
            )
            return {
    ...
    def _ground_causally(
        self, concept: str, context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Ground concept in causal relationships"""
        try:
            causal_relations = self.causal_reasoner.identify_causes_effects(
                concept, context
            )
            return {
    ...
    def _ground_physically(
        self, concept: str, context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Ground concept in physical properties"""
        try:
            physical_properties = self.physical_grounding.map_properties(
                concept, context
            )
            return {
    ...
    def _update_grounding(
        self, existing: SemanticGrounding, new_context: Dict[str, Any]
    ) -> SemanticGrounding:
    ...
    def _persist_grounding(self, grounding: SemanticGrounding):
        ...
        try:
            ...
            if hasattr(self.vault_manager, "insert_geoid"):
                ...
                logger.debug(
                    "VaultManager doesn't support insert_geoid - skipping persistence"
                )
    ...
    def get_related_concepts(
        self, concept: str, modality: Optional[str] = None, threshold: float = 0.5
        ...
        for other_concept, other_grounding in self.semantic_memory.items():
            ...
            similarity = self._compute_similarity(
                base_grounding, other_grounding, modality
            )
    ...
    def explain_grounding(self, concept: str) -> str:
        ...
        if grounding.visual:
            ...
            explanation += (
                f"  - Confidence: {grounding.visual.get('confidence', 0):.2f}\n\n"
            )
        ...
        if grounding.causal:
            ...
            explanation += (
                f"  - Confidence: {grounding.causal.get('confidence', 0):.2f}\n\n"
            )
        ...
        if grounding.physical:
            ...
            explanation += (
                f"  - Confidence: {grounding.physical.get('confidence', 0):.2f}\n"
            )
