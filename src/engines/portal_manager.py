...
self.max_portals = max_portals
    ...
    def create_portal(self, 
                     ...
                     energy_cost: Optional[float] = None) -> str:
        """
        ...
        logger.info(f"Created {portal_type.value} portal {portal_id}: "
                   f"dim {source_dim} -> {target_dim}, stability={initial_stability:.2f}")
        
        return portal_id
    
    def traverse_portal(self, portal_id: str, 
                       cargo: Any,
                       energy_available: float) -> Tuple[bool, Any, Dict[str, Any]]:
        """
    ...
    def _transform_cargo(self, cargo: Any, portal: Portal) -> Any:
        ...
        if portal.portal_type == PortalType.SEMANTIC:
            ...
            if hasattr(cargo, 'embedding'):
                ...
                angle = (portal.target_dimension - portal.source_dimension) * np.pi / 180
            ...
            if hasattr(cargo, 'energy'):
                ...
                energy_scale = (portal.target_dimension + 1) / (portal.source_dimension + 1)
            ...
            if hasattr(cargo, 'timestamp'):
                ...
                time_shift = (portal.target_dimension - portal.source_dimension) * 3600  # hours
    ...
    def get_portal_status(self, portal_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "last_transit": portal.last_transit.isoformat() if portal.last_transit else None
        }
    ...
    def get_manager_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "portal_types": {
                ptype.value: sum(1 for p in self.portals.values() 
                               if p.portal_type == ptype)
                for ptype in PortalType
            },
            "average_stability": np.mean([p.stability for p in self.portals.values()])
                                if self.portals else 0.0
        }
