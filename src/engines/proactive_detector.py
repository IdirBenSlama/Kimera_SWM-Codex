...
class TimeSeriesData:
    ...
    def to_numpy(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        timestamps_numeric = [(ts - self.timestamps[0]).total_seconds() for ts in self.timestamps]
...
class AnomalyDetector:
    ...
    def add_data_point(self, value: float, timestamp: datetime = None) -> Optional[DetectionEvent]:
        """
            ...
            if anomaly_score > self.sensitivity:
                ...
                event = DetectionEvent(
                    ...
                    context={
                        ...
                        "z_score": abs(value - self.statistics["mean"]) / (self.statistics["std"] + 1e-8)
                    }
                )
...
class PatternRecognizer:
    ...
    def recognize_patterns(self, data: TimeSeriesData) -> Dict[str, Any]:
        ...
        result = {
            ...
            "pattern_confidence": np.mean([p["confidence"] for p in all_patterns]) if all_patterns else 0.0,
    ...
    def _predict_next_pattern(self, patterns: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        if best_pattern["type"] == "periodic":
            return {
                ...
                "confidence": best_pattern["confidence"] * 0.8,  # Slightly lower confidence for prediction
...
class ThresholdManager:
    ...
    def update_threshold(self, detection_type: DetectionType, performance_feedback: Dict[str, Any]):
        """
...
class ProactiveDetector:
    ...
    def update_data_stream(self, stream_id: str, new_value: float, timestamp: datetime = None) -> List[DetectionEvent]:
        """
        ...
        with self.detection_lock:
            ...
            anomaly_event = stream["anomaly_detector"].add_data_point(new_value, timestamp)
            if anomaly_event:
            ...
            if len(stream["data"].values) >= 20:
                pattern_result = self.pattern_recognizer.recognize_patterns(stream["data"])
            ...
            for event in events:
                ...
                if event.alert_level in [AlertLevel.HIGH, AlertLevel.CRITICAL, AlertLevel.EMERGENCY]:
                    self.active_alerts[event.event_id] = event
        
        return events
    
    def predict_future_values(self, stream_id: str, horizon: PredictionHorizon = PredictionHorizon.SHORT_TERM) -> Dict[str, Any]:
        """
        ...
        with self.detection_lock:
            ...
            for i in range(1, minutes_ahead + 1):
                ...
                if pattern_result["next_expected_pattern"]:
                    pattern_confidence = pattern_result["next_expected_pattern"]["confidence"]
    ...
    def get_active_alerts(self, alert_level: Optional[AlertLevel] = None) -> List[DetectionEvent]:
        """
    ...
    def get_detection_summary(self) -> Dict[str, Any]:
        ...
        with self.detection_lock:
            recent_events = [e for e in self.detection_events if (datetime.now() - e.timestamp).total_seconds() < 3600]
            ...
            return {
                ...
                "false_positive_rate": self.false_positive_count / max(self.total_detections, 1),
                "type_distribution": type_distribution,
                "alert_distribution": alert_distribution,
                "threshold_adaptations": len(self.threshold_manager.get_threshold_history())
            }
