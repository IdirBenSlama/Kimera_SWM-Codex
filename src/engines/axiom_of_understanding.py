...
self.candidate_axioms: List[MathematicalAxiom] = []
    ...
    async def discover_fundamental_axiom(self) -> MathematicalAxiom:
        ...
        for axiom in candidates:
            ...
            logger.info(f"   Statement: {axiom.statement}")
            ...
            logger.info(f"   ✓ Independence: {independence:.3f}")
        ...
        logger.info(f"   ID: {fundamental_axiom.axiom_id}")
        logger.info(f"   Statement: {fundamental_axiom.statement}")
        logger.info(f"   Formal: {fundamental_axiom.formal_notation}")
        logger.info(f"   Score: {results[0][1]:.3f}")
        
        # Derive consequences
        logger.info(f"\n🌟 Derivable Theorems:")
        for theorem in fundamental_axiom.derivable_theorems:
            logger.info(f"   - {theorem}")
...
async def main():
    ...
    logger.info("=" * 80)
    logger.info("KIMERA AXIOM OF UNDERSTANDING - MATHEMATICAL DISCOVERY")
    logger.info("=" * 80)
    ...
    for i, eigenval in enumerate(eigenvalues):
        logger.info(f"   λ_{i+1} = {eigenval:.6f}")
