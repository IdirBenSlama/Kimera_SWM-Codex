...
from .cognitive_field_dynamics import SemanticField # Assuming this is the base field class
...
class GPUBufferPool:
    ...
    def __init__(self, initial_size=10, buffer_size_bytes=1024*1024): # Default 1MB buffers
...
self.pool = deque(maxlen=initial_size)
        ...
        logger.info(f"GPUBufferPool initialized with {initial_size} buffers of {buffer_size_bytes} bytes.")
...
self.gpu_foundation = gpu_foundation
    ...
    def allocate_signal_enhanced_field(self, base_field: SemanticField) -> SignalEnhancedField:
        ...
        enhanced_field = SignalEnhancedField(
            ...
            signal_properties_buffer=signal_buffer
        )
        
        self.managed_fields[base_field.geoid_id] = enhanced_field
        self.total_memory_overhead += enhanced_field.memory_overhead_bytes
        
        logger.debug(f"Allocated signal-enhanced field for {base_field.geoid_id}. Overhead: {self.total_memory_overhead} bytes.")
    ...
    def deallocate_field(self, geoid_id: str):
        ...
        if geoid_id in self.managed_fields:
            ...
            self.total_memory_overhead -= enhanced_field.memory_overhead_bytes
            logger.debug(f"Deallocated signal-enhanced field for {geoid_id}. Overhead: {self.total_memory_overhead} bytes.")
