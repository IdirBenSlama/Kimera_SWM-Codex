                    ...
                    if geoids_to_process:
                        try:
                            ...
                            if consciousness_detector and len(geoids_to_process) >= 2:
                                complexity_state = foundational_engine.detect_complexity_threshold(geoids_to_process[:5])
                                cycle_stats["complexity_detected"] = complexity_state.complexity_probability >= 0.7
                                cycle_stats["complexity_probability"] = complexity_state.complexity_probability
                                cycle_stats["temperature_coherence"] = complexity_state.temperature_coherence
                                
                                if complexity_state.complexity_probability >= 0.7:
                                    logging.info(f"🔬 High complexity threshold detected! Probability: {complexity_state.complexity_probability:.3f}")
                            ...
                            if len(geoids_to_process) >= 4:
                                ...
                                cycle_stats["thermodynamic_work_extracted"] = carnot_result.work_extracted
                                cycle_stats["thermodynamic_efficiency"] = carnot_result.actual_efficiency
                                cycle_stats["physics_compliant"] = carnot_result.physics_compliant
                                
                                if not carnot_result.physics_compliant:
                                    logging.warning(f"⚠️ Physics violation detected in cycle {state['cycle_count']}")
                                    
                        except Exception as thermo_exc:
                            logging.warning(f"Thermodynamic processing failed: {thermo_exc}")
                            cycle_stats["thermodynamic_errors"] = cycle_stats.get("thermodynamic_errors", 0) + 1
                            
            except Exception as exc:
                # Log but never crash the cognitive cycle
                logging.getLogger(__name__).warning(
                    "Revolutionary Thermodynamic Integration failed: %s", exc, exc_info=True
                )
                cycle_stats["errors_encountered"] += 1
