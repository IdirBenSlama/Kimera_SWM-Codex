...
class ProactiveContradictionDetector:
        ...
        for row in geoid_rows:
            try:
                geoid = GeoidState(
                    ...
                    embedding_vector=(
                        row.semantic_vector if row.semantic_vector is not None else []
                    ),
    ...
    def _analyze_clusters(
        ...
        for cluster in clusters:
            ...
            cluster_tensions = self.contradiction_engine.detect_tension_gradients(
                cluster
            )
    ...
    def _calculate_similarity(self, geoid_a: GeoidState, geoid_b: GeoidState) -> float:
        ...
        if (
            geoid_a.embedding_vector is None
            or geoid_b.embedding_vector is None
            or len(geoid_a.embedding_vector) == 0
            or len(geoid_b.embedding_vector) == 0
        ):
            return 0.0

        try:
            # Use native cosine similarity (higher = more similar)
            return NativeMath.cosine_similarity(
                geoid_a.embedding_vector, geoid_b.embedding_vector
            )
        except Exception as e:
            # Log this error? For now, returning 0.0 is safe.
            return 0.0

    def _analyze_temporal_patterns(
        self, db: Session, geoids: List[GeoidState], results: Dict
    ) -> List[TensionGradient]:
        ...
        for window_geoids in time_windows:
            ...
            window_tensions = self.contradiction_engine.detect_tension_gradients(
                window_geoids
            )
    ...
    def _group_by_time_windows(
        self, geoids: List[GeoidState], window_hours: int = 24
    ) -> List[List[GeoidState]]:
        ...
        for geoid in geoids:
            ...
            elif "timestamp" in geoid.meta_data:
                timestamp_str = geoid.meta_data["timestamp"]
            ...
            for type_b in type_list[i + 1 :]:
                ...
                for geoid_a in sample_a:
                    for geoid_b in sample_b:
                        cross_tensions = (
                            self.contradiction_engine.detect_tension_gradients(
                                [geoid_a, geoid_b]
                            )
                        )
                        ...
                        if (
                            ...
                            > self.config.max_comparisons_per_run
                        ):
                            return tensions
    ...
    def _analyze_underutilized_geoids(
        ...
        if len(underutilized) >= 2:
            ...
            underutilized_tensions = self.contradiction_engine.detect_tension_gradients(
                underutilized[:20]
            )  # Limit batch size
