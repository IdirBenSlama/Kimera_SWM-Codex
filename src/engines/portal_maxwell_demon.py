...
class PortalMaxwellDemon:
    ...
    def __init__(self, 
        ...
        logger.info(f"👹 Portal Maxwell Demon initialized (T={temperature}, efficiency={landauer_efficiency})")
    ...
    def analyze_information_packets(self, packets: List[InformationPacket]) -> Dict[str, Any]:
        """
        ...
        if len(packets) > 1 and packets[0].semantic_vector is not None:
            vectors = np.array([p.semantic_vector for p in packets if p.semantic_vector is not None])
            if len(vectors) > 1:
        ...
        efficiency_factors = {
            ...
            SortingStrategy.HYBRID_OPTIMIZATION: (entropy_variance + mean_coherence + clustering_potential) / 3.0
        }
    ...
    def select_optimal_portal(self, packet: InformationPacket, strategy: SortingStrategy) -> Optional[QuantumPortal]:
        """
        ...
        """
        available_portals = [p for p in self.portals.values() if p.current_load < p.capacity]
        ...
        for portal in available_portals:
            ...
            if strategy == SortingStrategy.ENTROPY_MINIMIZATION:
                if packet.entropy_content > 1.5 and portal.portal_type == 'high_entropy':
                    score += 0.4
                elif packet.entropy_content <= 1.5 and portal.portal_type == 'low_entropy':
                    score += 0.4
                ...
                score += (portal.portal_efficiency + portal.quantum_coherence + portal.tunnel_probability) / 3.0 * 0.4
    ...
    def perform_sorting_operation(self, 
        ...
        for packet in packets:
            ...
            packet_bits = packet.entropy_content * len(packet.semantic_vector) if packet.semantic_vector is not None else packet.entropy_content
            ...
            entropy_reduction = max(0, packet.entropy_content - (packet.entropy_content * portal.portal_efficiency))
            ...
            if (packet.coherence_score > self.quantum_coherence_threshold and 
                portal.quantum_coherence > self.quantum_coherence_threshold):
        ...
        information_gain = entropy_reduction * self.boltzmann_constant * self.temperature
        
        # Calculate sorting efficiency
        theoretical_min_work = total_bits * self.boltzmann_constant * self.temperature * math.log(2)
        ...
        logger.info(f"👹 Sorting complete: {sorted_count} packets, efficiency={sorting_efficiency:.3f}")
    ...
    def get_portal_status(self) -> Dict[str, Any]:
        ...
        for portal_id, portal in self.portals.items():
            ...
            portal_status[portal_id] = {
                ...
                'operational_status': 'active' if utilization < 0.9 else 'near_capacity'
            }
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        avg_efficiency = np.mean([op.sorting_efficiency for op in self.operation_history])
        ...
        return {
            ...
            'information_packets_sorted': sum(len(packets) for packets in self.sorted_information.values()),
            'portal_utilization': {pid: p.current_load / p.capacity for pid, p in self.portals.items()}
        }
