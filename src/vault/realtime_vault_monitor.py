...
from ..vault.database import GeoidDB, ScarDB, get_db_status, get_engine
...
class RealTimeVaultMonitor:
    ...
    def __init__(
        self, vault_manager: VaultManager, contradiction_engine: ContradictionEngine
    ):
    ...
    def _monitor_cognitive_activity(self):
        ...
        try:
            ...
            if current_geoid_count > self.last_geoid_count:
                ...
                logger.info(
                    f"🧠 New geoid formations detected: +{delta} (total: {current_geoid_count})"
                )
            ...
            if current_scar_count > self.last_scar_count:
                ...
                self.activity_history.append(activity)
                logger.info(
                    f"⚡ New scar formations detected: +{delta} (total: {current_scar_count})"
                )
            ...
            if current_insight_count > self.last_insight_count:
                ...
                self.activity_history.append(activity)
                logger.info(
                    f"💡 New insights generated: +{delta} (total: {current_insight_count})"
                )
    ...
    def _perform_health_check(self):
        ...
        try:
            ...
            activity_rate = (
                len(recent_activities) / self.activity_window.total_seconds() * 60
            )

            # Memory efficiency (placeholder calculation)
            memory_efficiency = min(
                100.0,
                (geoid_count + scar_count) / max(1, geoid_count + scar_count + 1) * 100,
            )

            # Determine cognitive state
            cognitive_state = self._determine_cognitive_state(
                geoid_count, scar_count, activity_rate
            )
            ...
            # Log health status
            logger.info(
                f"🏥 Vault Health: {cognitive_state} | "
    ...
    def _determine_cognitive_state(
        self, geoid_count: int, scar_count: int, activity_rate: float
    ) -> str:
    ...
    def _detect_anomalies(
        ...
        if len(self.health_history) > 10:
            recent_latencies = [
                m.database_latency_ms for m in list(self.health_history)[-10:]
            ]
            if recent_latencies:
    ...
    def get_cognitive_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cognitive_patterns": {
                "formation_rate": len(
                    [a for a in activities if "formation" in a.activity_type]
                )
                / max(1, len(activities)),
                "creation_rate": len(
                    [a for a in activities if "creation" in a.activity_type]
                )
                / max(1, len(activities)),
                "insight_rate": len(
                    [a for a in activities if "insight" in a.activity_type]
                )
                / max(1, len(activities)),
            },
        }
