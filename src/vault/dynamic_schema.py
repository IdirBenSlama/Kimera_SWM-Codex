...
from sqlalchemy import (
    ...
    JSON, Boolean, func, Index, text, MetaData, Table
)
...
def create_dynamic_schema(engine):
    ...
    geoid_states = Table(
        'geoid_states', metadata,
        Column('id', String(36) if not is_postgresql else UUID(as_uuid=True), primary_key=True, default=lambda: str(uuid.uuid4()) if not is_postgresql else uuid.uuid4),
    ...
    cognitive_transitions = Table(
        'cognitive_transitions', metadata,
        Column('id', String(36) if not is_postgresql else UUID(as_uuid=True), primary_key=True, default=lambda: str(uuid.uuid4()) if not is_postgresql else uuid.uuid4),
        source_id_col,
        target_id_col,
