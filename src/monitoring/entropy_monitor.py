...
class EntropyEstimator:
    ...
    def miller_madow_entropy(
        probabilities: np.ndarray, sample_size: int, base: float = 2.0
    ) -> float:
...
class EntropyMonitor:
    ...
    def calculate_system_entropy(
        ...
        if total_activation == 0:
            ...
            probabilities = (
                np.array(list(feature_aggregation.values())) / total_activation
            )
        ...
        if self.estimation_method == "mle":
            ...
            shannon_entropy = EntropyEstimator.miller_madow_entropy(
        ...
        relative_entropy = 0.0
        if self.baseline_distribution is not None and len(probabilities) == len(
            self.baseline_distribution
        ):
            relative_entropy = EntropyEstimator.relative_entropy_kl(
                probabilities, self.baseline_distribution
            )

        # Conditional entropy and mutual information
        conditional_entropy, mutual_information = self._calculate_conditional_measures(
            geoids
        )
    ...
    def _calculate_conditional_measures(
        ...
        joint_counts = np.histogram2d(
            semantic_discrete,
            symbolic_discrete,
            bins=[len(semantic_bins) - 1, len(symbolic_bins) - 1],
        )[0]
        joint_probs = (
            joint_counts / np.sum(joint_counts)
        ...
        mutual_information = (
            h_semantic + h_symbolic - h_joint
        )  # I(X;Y) = H(X) + H(Y) - H(X,Y)

        return conditional_entropy, mutual_information

    def _calculate_system_complexity_adaptive(
        self,
        geoids: List[GeoidState],
        vault_info: Dict[str, Any],
        system_phase: str = "balanced",
    ) -> float:
        """
    ...
    def _calculate_vault_complexity(self, vault_info: Dict[str, Any]) -> float:
        ...
        return sum(
            vault_info.get(f"{vault}_scars", 0) for vault in ["vault_a", "vault_b"]
        )
    ...
    def detect_system_phase(self, recent_measurements: List[EntropyMeasurement]) -> str:
        ...
        # Calculate slopes
        entropy_slope = (
            np.polyfit(range(len(entropies)), entropies, 1)[0]
            if len(entropies) > 1
            else 0
        )
        complexity_slope = (
            np.polyfit(range(len(complexities)), complexities, 1)[0]
    ...
    def _calculate_system_complexity(
        ...
        interaction_complexity = sum(
            len(connections) for connections in feature_cooccurrence.values()
        )

        # Vault complexity
        vault_complexity = sum(
            vault_info.get(f"{vault}_scars", 0) for vault in ["vault_a", "vault_b"]
        )
    ...
    def _analyze_vault_distribution(self, vault_info: Dict[str, Any]) -> Dict[str, int]:
        ...
        return {
            ...
            "total_scars": vault_info.get("vault_a_scars", 0)
    ...
    def get_entropy_trends(self, window_size: int = 100) -> Dict[str, List[float]]:
        ...
        return {
            ...
            "thermodynamic_entropy": [
                m.thermodynamic_entropy for m in recent_measurements
            ],
    ...
    def detect_entropy_anomalies(
        ...
        for measurement in recent_measurements[-10:]:  # Check last 10
            if (
                abs(measurement.shannon_entropy - mean_entropy)
                > threshold_std * std_entropy
            ):
                anomalies.append(
                    {
                        "timestamp": measurement.timestamp.isoformat(),
                        "shannon_entropy": measurement.shannon_entropy,
                        "deviation": abs(measurement.shannon_entropy - mean_entropy)
                        / std_entropy,
                        "type": (
                            "high"
                            if measurement.shannon_entropy > mean_entropy
                            else "low"
                        ),
                    }
                )
