...
class SemanticAnalyzer:
    ...
    @staticmethod
    def measure_observer_dependency(
        measurements: List[SemanticMeasurement],
        observer_contexts: List[Dict[str, float]],
    ) -> float:
        """
    ...
    def calculate_syntax_to_semantics_ratio(text: str, semantic_score: float) -> float:
        ...
        avg_word_length = (
            np.mean([len(word) for word in re.findall(r"\b\w+\b", text)])
            if word_count > 0
            else 0
        )

        syntactic_complexity = (
            word_count * 0.4 + sentence_count * 0.3 + avg_word_length * 0.3
        )
...
class SemanticMetricsCollector:
    ...
    def collect_semantic_metrics(
        ...
        context_weights = self.observer_contexts.get(
            observer_context, self.observer_contexts["balanced"]
        )
        ...
        semantic_complexity = self._calculate_semantic_complexity(
            geoids, linguistic_geoids
        )

        # Calculate meaning density
        all_text = " ".join([lg.primary_statement for lg in linguistic_geoids])
        meaning_density = SemanticAnalyzer.measure_meaning_density(
            all_text, self.semantic_keywords
        )
        ...
        syntax_to_semantics_ratio = self._calculate_avg_syntax_semantics_ratio(
            linguistic_geoids
        )
    ...
    def _calculate_semantic_complexity(
        self, geoids: List[GeoidState], linguistic_geoids: List[LinguisticGeoid]
    ) -> float:
        ...
        if linguistic_geoids:
            total_ambiguities = sum(
                len(lg.potential_ambiguities) for lg in linguistic_geoids
            )
            total_scars = sum(len(lg.supporting_scars) for lg in linguistic_geoids)
            avg_confidence_variance = np.var(
                [lg.confidence_score for lg in linguistic_geoids]
            )
    ...
    def _calculate_avg_syntax_semantics_ratio(
        self, linguistic_geoids: List[LinguisticGeoid]
    ) -> float:
        ...
        ratios = []
        for lg in linguistic_geoids:
            semantic_score = lg.confidence_score * (
                1 - len(lg.potential_ambiguities) * 0.1
            )
    ...
    def _calculate_semantic_efficiency(
        self,
        semantic_entropy: float,
        semantic_complexity: float,
        meaning_density: float,
    ...
    def _calculate_ambiguity_index(
        self, linguistic_geoids: List[LinguisticGeoid]
    ) -> float:
        ...
        confidence_factor = 1 - np.mean(
            [lg.confidence_score for lg in linguistic_geoids]
        )

        base_ambiguity = (
            total_ambiguities / total_statements if total_statements > 0 else 0
        )

        return base_ambiguity * (1 + confidence_factor)

    def _calculate_information_utility(
        self,
        geoids: List[GeoidState],
        linguistic_geoids: List[LinguisticGeoid],
        ...
        speed_utility = 0.0
        if linguistic_geoids:
            avg_statement_length = np.mean(
                [len(lg.primary_statement) for lg in linguistic_geoids]
            )
    ...
    def detect_semantic_anomalies(
        ...
        for measurement in recent_measurements[-5:]:  # Check last 5
            if (
                abs(measurement.semantic_entropy - entropy_mean)
                > threshold_std * entropy_std
            ):
                anomalies.append(
                    {
                        ...
                        "deviation": abs(measurement.semantic_entropy - entropy_mean)
                        / entropy_std,
                        "severity": (
                            "high"
                            if measurement.semantic_entropy > entropy_mean
                            else "low"
                        ),
                    }
                )
        ...
        for measurement in recent_measurements[-5:]:
            if (
                abs(measurement.semantic_efficiency - efficiency_mean)
                > threshold_std * efficiency_std
            ):
                anomalies.append(
                    {
                        "timestamp": measurement.timestamp.isoformat(),
                        ...
                        "deviation": abs(
                            measurement.semantic_efficiency - efficiency_mean
                        )
                        / efficiency_std,
                        "severity": (
                            "low"
                            if measurement.semantic_efficiency < efficiency_mean
                            else "high"
                        ),
                    }
                )
