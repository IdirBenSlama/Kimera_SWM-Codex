...
from .entropy_monitor import EntropyMeasurement, EntropyMonitor
from .semantic_metrics import SemanticMeasurement, SemanticMetricsCollector
...
class SystemObserver:
    ...
    def _initialize_observer_profiles(self):
        ...
        self.active_observers["analytical"] = ObserverProfile(
            name="analytical",
            attention_weights={"entropy": 0.3, "thermodynamic": 0.4, "semantic": 0.3},
            temporal_focus="long",
            analysis_depth="deep",
            priority_metrics=[
                "entropy_stability",
        ...
        self.active_observers["operational"] = ObserverProfile(
            name="operational",
            attention_weights={"entropy": 0.2, "thermodynamic": 0.3, "semantic": 0.5},
        ...
        self.active_observers["research"] = ObserverProfile(
            name="research",
            attention_weights={"entropy": 0.4, "thermodynamic": 0.3, "semantic": 0.3},
    ...
    def _calculate_system_health(
        ...
        if len(self.snapshots) > 10:
            recent_entropies = [
                s.entropy_measurement.shannon_entropy
                for s in list(self.snapshots)[-10:]
            ]
        ...
        avg_observation_time = (
            np.mean(list(self.observation_times)) if self.observation_times else 1.0
        )
        ...
        memory_utilization = (
            total_geoids / vault_capacity if vault_capacity > 0 else 0.0
        )
    ...
    def _calculate_error_rate(self) -> float:
        ...
        entropy_anomalies = len(
            self.entropy_monitor.detect_entropy_anomalies(threshold_std=2.0)
        )
        semantic_anomalies = len(
            self.semantic_collector.detect_semantic_anomalies(threshold_std=2.0)
        )
        thermodynamic_anomalies = len(
            self.thermodynamic_analyzer.detect_thermodynamic_anomalies(
                threshold_std=2.0
            )
        )

        total_anomalies = (
            entropy_anomalies + semantic_anomalies + thermodynamic_anomalies
        )
        total_observations = len(recent_snapshots)

        return (
            min(total_anomalies / total_observations, 1.0)
            if total_observations > 0
            else 0.0
        )

    def _check_alerts(
        self, snapshot: SystemSnapshot, observer_profile: ObserverProfile
    ):
        ...
        if "semantic_degradation" in observer_profile.alert_thresholds:
            if (
                snapshot.semantic_measurement.semantic_efficiency
                < observer_profile.alert_thresholds.get("semantic_degradation", 0.5)
            ):
                alerts.append(
                    {
        ...
        if "efficiency_drop" in observer_profile.alert_thresholds:
            if (
                snapshot.thermodynamic_state.efficiency
                < observer_profile.alert_thresholds["efficiency_drop"]
            ):
                alerts.append(
                    {
        ...
        if "temperature_spike" in observer_profile.alert_thresholds:
            if (
                snapshot.thermodynamic_state.temperature
                > observer_profile.alert_thresholds["temperature_spike"]
            ):
                alerts.append(
                    {
    ...
    def get_system_summary(self, window_size: int = 100) -> Dict[str, Any]:
        ...
        thermodynamic_trends = self.thermodynamic_analyzer.get_thermodynamic_trends(
            window_size
        )

        # Calculate averages
        avg_entropy = np.mean(
            [s.entropy_measurement.shannon_entropy for s in recent_snapshots]
        )
        avg_semantic_efficiency = np.mean(
            [s.semantic_measurement.semantic_efficiency for s in recent_snapshots]
        )
        avg_thermodynamic_efficiency = np.mean(
            [s.thermodynamic_state.efficiency for s in recent_snapshots]
        )
        ...
        return {
            ...
            "avg_observation_time": (
                np.mean(list(self.observation_times)) if self.observation_times else 0.0
            ),
            "last_observation": (
...
"""
        ...
        if summary["recent_alerts"]:
            for alert in summary["recent_alerts"][-5:]:  # Last 5 alerts
                report += (
                    f"- {alert['type']}: {alert['message']} ({alert['timestamp']})\n"
                )
        else:
...
"""
    ...
    def _generate_executive_report(self, summary: Dict[str, Any]) -> str:
        ...
        health_status = (
            "HEALTHY"
            if summary["system_health"]["overall_health"] > 0.8
            else (
                "DEGRADED"
                if summary["system_health"]["overall_health"] > 0.6
                else "CRITICAL"
            )
        )
...
"""
    ...
    def _generate_recommendations(self, summary: Dict[str, Any]) -> str:
        ...
        if summary["system_health"]["memory_utilization"] > 0.8:
            recommendations.append(
                "- High memory utilization, consider vault rebalancing"
            )
        ...
        if summary["averages"]["thermodynamic_efficiency"] < 0.5:
            recommendations.append(
                "- Low thermodynamic efficiency, optimize processing algorithms"
            )
