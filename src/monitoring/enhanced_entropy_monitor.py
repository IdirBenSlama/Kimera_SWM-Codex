...
class EnhancedEntropyMonitor(EntropyMonitor):
    ...
    def __init__(
        ...
        self.logger.info(
            f"Enhanced entropy monitor initialized (SciPy: {self.use_scipy})"
        )

    def calculate_enhanced_entropy(
        self, geoids: List[GeoidState], vault_info: Dict[str, Any]
    ) -> EnhancedEntropyMeasurement:
        """
    ...
    def _calculate_enhanced_metrics(
        ...
        base_measurement: EntropyMeasurement,
    ) -> Dict[str, Any]:
        ...
        if feature_data["feature_values"]:
            metrics["differential_entropy"] = (
                self.enhanced_calculator.differential_entropy(
        ...
        if self.baseline_distribution is not None and feature_data["feature_values"]:
            ...
            if len(current_dist) == len(self.baseline_distribution):
                metrics["jensen_shannon_divergence"] = (
                    self.enhanced_calculator.jensen_shannon_divergence(
        ...
        if (
            feature_data["activation_sequences"]
            and len(feature_data["activation_sequences"]) > 10
        ):
        ...
        # 4. Entropy Rate
        if (
            feature_data["geoid_activations"]
            and len(feature_data["geoid_activations"]) > 5
        ):
            # Discretize activations for entropy rate calculation
            discretized = self._discretize_sequence(feature_data["geoid_activations"])
            metrics["entropy_rate"] = self.enhanced_calculator.entropy_rate(
                discretized, order=2
            )
        else:
    ...
    def _calculate_complexity_index(
        self, base_measurement: EntropyMeasurement, multiscale_entropy: Dict[int, float]
    ) -> float:
    ...
    def _calculate_predictability_score(
        self, feature_data: Dict[str, Any], entropy_rate: float
    ) -> float:
        ...
        if (
            feature_data["activation_sequences"]
            and len(feature_data["activation_sequences"]) > 3
        ):
                ...
                try:
                    ...
                    if len(autocorr) > 1:
                        pattern_strength = np.mean(
                            np.abs(autocorr[1 : min(5, len(autocorr))])
                        )
    ...
    def _calculate_anomaly_score(self, base_measurement: EntropyMeasurement) -> float:
        ...
        if self.use_scipy and len(recent_entropies) > 5:
            try:
                ...
                self.logger.debug(f"Anomaly score calculation failed: {e}")
                pass
    ...
    def _calculate_adaptive_threshold(
        self, base_measurement: EntropyMeasurement, vault_info: Dict[str, Any]
    ) -> float:
        ...
        total_scars = vault_info.get("vault_a_scars", 0) + vault_info.get(
            "vault_b_scars", 0
        )
        if total_scars > 0:
            vault_balance = (
                min(
                    vault_info.get("vault_a_scars", 0),
                    vault_info.get("vault_b_scars", 0),
                )
                / total_scars
            )
        ...
        maturity_factor = 1.0 - (
            self.system_maturity * 0.2
        )  # More lenient as system matures

        adaptive_threshold = (
            base_threshold * complexity_factor * balance_factor * maturity_factor
        )

        return max(adaptive_threshold, 0.01)  # Minimum threshold

    def _update_enhanced_tracking(
        self, measurement: EnhancedEntropyMeasurement
    ) -> None:
    ...
    def _create_empty_enhanced_measurement(
        self, base_measurement: EntropyMeasurement
    ) -> EnhancedEntropyMeasurement:
    ...
    def get_enhanced_trends(self, window_size: int = 100) -> Dict[str, List[float]]:
        ...
        enhanced_measurements = [
            m for m in self.measurements if isinstance(m, EnhancedEntropyMeasurement)
        ]
            ...
            base_trends.update(
                {
                    "differential_entropy": [
                        m.differential_entropy for m in recent_enhanced
                    ],
                    "jensen_shannon_divergence": [
                        m.jensen_shannon_divergence for m in recent_enhanced
                    ],
                    "complexity_index": [m.complexity_index for m in recent_enhanced],
                    "predictability_score": [
                        m.predictability_score for m in recent_enhanced
                    ],
                    "anomaly_score": [m.anomaly_score for m in recent_enhanced],
                    "adaptive_threshold": [
                        m.adaptive_threshold for m in recent_enhanced
                    ],
                }
            )
        ...
        if self.use_scipy:
            try:
                threshold = np.percentile(
                    list(self.anomaly_scores), threshold_percentile
                )
            except (ValueError, TypeError, IndexError) as e:
                # Failed to calculate percentile
                self.logger.debug(f"Percentile calculation failed: {e}")
                    ...
                    anomalies.append(
                        {
                            ...
                            "severity": (
                                "high"
                                if measurement.anomaly_score > threshold * 1.5
                                else "medium"
                            ),
                        }
                    )
    ...
    def get_system_health_report(self) -> Dict[str, Any]:
        ...
        recent_measurements = list(self.measurements)[-10:]
        enhanced_measurements = [
            m for m in recent_measurements if isinstance(m, EnhancedEntropyMeasurement)
        ]
        ...
        health_components = {
            ...
            "complexity_balance": 1.0
            - abs(latest.complexity_index - 0.5) * 2,  # Optimal complexity around 0.5
        ...
        return {
            ...
            "recommendations": self._generate_health_recommendations(
                health_components, latest
            ),
        }

    def _generate_health_recommendations(
        self, health_components: Dict[str, float], latest: EnhancedEntropyMeasurement
    ) -> List[str]:
        ...
        if health_components["entropy_stability"] < 0.6:
            recommendations.append(
                "High entropy instability detected - consider system stabilization"
            )

        if health_components["predictability"] < 0.4:
            recommendations.append(
                "Low predictability - system may benefit from pattern reinforcement"
            )

        if health_components["complexity_balance"] < 0.5:
            if latest.complexity_index > 0.7:
                recommendations.append(
                    "System complexity too high - consider simplification"
                )
            else:
                recommendations.append(
                    "System complexity too low - may need more diverse inputs"
                )

        if latest.anomaly_score > 0.8:
            recommendations.append(
                "High anomaly score - investigate recent system changes"
            )
...
def create_enhanced_entropy_monitor(
    use_scipy: bool = True, adaptive_thresholds: bool = True
