...
try:
    ...
    logger.warning(
        "Neo4j driver not available. Graph database functionality will be disabled."
    )
...
def _create_driver() -> Optional[Driver]:
    ...
    if not uri:
        logger.warning(
            "NEO4J_URI environment variable not set. Neo4j integration disabled."
        )
    ...
    if not pwd:
        logger.warning(
            "NEO4J_PASS environment variable not set. Neo4j integration disabled."
        )
