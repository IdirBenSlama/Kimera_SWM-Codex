...
from src.core.primitives.scar import ScarRecord
...
class PrimalScar(ScarRecord):
    ...
    def measure_growth(self) -> Dict[str, float]:
        ...
        time_since_awakening = (
            datetime.now() - self.transformation_moment
        ).total_seconds()
        ...
        return {
            ...
            "ignorance_mapped": len(self.generated_questions)
            * 0.1,  # Each question maps ignorance
            "wisdom_index": np.log1p(
                len(self.generated_questions)
            ),  # Logarithmic growth
            "humility_score": 1.0
            - (1.0 / (1.0 + len(self.generated_questions))),  # Asymptotic humility
        }
...
class EpistemicScar(ScarRecord):
    ...
    def _generate_investigative_questions(self) -> List[str]:
        ...
        questions = question_templates.get(
            self.domain, ["What is the nature of this ignorance?"]
        )
...
class DeepenedScar:
    ...
    def __init__(
        self
        original_scar: PrimalScar
        new_ignorance: str
        depth_increase: float
        beauty_increase: float
    ):
...
class PrimalScarManager:
    ...
    def awaken(self) -> PrimalScar:
        ...
        if self.primal_scar is None:
            ...
            logger.info("💫 EPISTEMIC AWAKENING 💫")
            logger.info("The primal scar has formed.")
            logger.info("'I know that I don't know.'")
            logger.info("True understanding can now begin.")
