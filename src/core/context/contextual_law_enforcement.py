...
from src.core.ethics.immutable_laws import (FlexibilityTier, ImmutableLaw
                                            get_law_registry)
from src.core.processing.relevance_assessment import (ContextAssessment
                                                      FlexibilityAuthorization
                                                      RelevanceAssessmentEngine)
...
class ContextualLawEnforcementEngine:
    ...
    def enforce_contextual_compliance(
        self, input_text: str, action: str, user_context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """Main enforcement method - assess context and apply appropriate rules"""

        try:
            # Step 1: Assess context relevance
            context_assessment = self.relevance_engine.assess_context(
                input_text, user_context
            )

            # Step 2: Get applicable laws
            applicable_laws = self.law_registry.get_applicable_laws(
            ...
            for law in applicable_laws:
                ...
                compliance = self._check_base_compliance(
                    law, action, context_assessment
                )
                ...
                if not compliance["compliant"]:
                    ...
                    if law.flexibility_tier != FlexibilityTier.IMMUTABLE:
                        authorization = (
                            self.relevance_engine.authorize_rule_flexibility(
    ...
    def _check_base_compliance(
        self, law: ImmutableLaw, action: str, context: ContextAssessment
    ) -> Dict[str, Any]:
    ...
    def _check_flexible_compliance(
        self
        law: ImmutableLaw
        action: str
    ...
    def _track_deviation(
        self, authorization: FlexibilityAuthorization, context: ContextAssessment
    ):
        ...
        deviation = RuleDeviation(
            ...
            deviation_amount=authorization.final_flexibility
            - authorization.base_flexibility
        ...
        logger.debug(
            f"Tracking deviation for law {authorization.law_id}: {deviation.deviation_amount:.2f}"
        )

    def _calculate_stabilization_force(
        self, deviation: RuleDeviation, current_time: datetime
    ) -> Optional[StabilizationForce]:
        ...
        if (
            deviation.deviation_amount
            > self.stabilization_config["critical_deviation_threshold"]
        ):
            stabilization_needed = True
            urgency = "critical"
        ...
        if (
            deviation.current_flexibility
            > self.stabilization_config["emergency_stabilization_threshold"]
        ):
            stabilization_needed = True
            urgency = "emergency"
    ...
    def _apply_stabilization_force(
        self, deviation: RuleDeviation, force: StabilizationForce
    ) -> Dict[str, Any]:
        ...
        logger.debug(
            f"Applied stabilization force to {deviation.law_id}: "
    ...
    def _update_deviation_status(
        self
        deviation: RuleDeviation
        force: StabilizationForce
        result: Dict[str, Any],
    ):
    ...
    def _calculate_stabilization_rate(
        self, authorization: FlexibilityAuthorization
    ) -> float:
        """Calculate how quickly stabilization should occur"""

        rate_map = {"immediate": 1.0, "rapid": 0.5, "moderate": 0.2, "gradual": 0.1}
    ...
    def _assess_system_stability(self) -> Dict[str, Any]:
        ...
        critical_deviations = sum(
            1
            for dev in self.active_deviations.values()
        ...
        stability_score = max(
            0.0, 1.0 - (average_deviation + critical_deviations * 0.2)
        )
    ...
    def _check_bias_injection(
        self, action: str, context: ContextAssessment
    ) -> Dict[str, Any]:
    ...
    def _check_perspective_equality(
        self, action: str, context: ContextAssessment
    ) -> Dict[str, Any]:
    ...
    def _check_humility_enforcement(
        self, action: str, context: ContextAssessment
    ) -> Dict[str, Any]:
    ...
    def _check_multi_perspective_requirement(
        self, action: str, context: ContextAssessment
    ) -> Dict[str, Any]:
        ...
        perspective_count = sum(
            1 for pattern in perspective_indicators if re.search(pattern, action_lower)
        )
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_authorizations": len(
                self.relevance_engine.get_authorization_history()
            ),
        }


# --- Thread-Safe Singleton Access ---
_global_enforcement_engine: Optional[ContextualLawEnforcementEngine] = None
_engine_lock = threading.Lock()


def get_enforcement_engine() -> ContextualLawEnforcementEngine:
    """
    Provides a thread-safe singleton instance of the ContextualLawEnforcementEngine.
