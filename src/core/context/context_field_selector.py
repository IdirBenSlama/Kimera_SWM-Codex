...
class ContextFieldSelector:
    ...
    def __init__(self, config: Optional[ContextFieldConfig] = None):
        ...
        logger.info(
            f"Context Field Selector initialized with {self.config.processing_level.value} level"
        )
        ...
        for field_name, value in semantic_state.items():
            if self.should_process_field(field_name):
                ...
                if (
                    ...
                    and processed_count >= self.config.max_semantic_features
                ):
                    break
    ...
    def create_filtered_output(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "symbolic_state" in raw_data and self.should_process_category(
            FieldCategory.SYMBOLIC_STATE
        ):
    ...
    def get_processing_summary(self) -> Dict[str, Any]:
        ...
        return {
            "config": {
                ...
                "included_categories": [
                    cat.value for cat in self.config.included_categories
                ],
                "excluded_categories": [
                    cat.value for cat in self.config.excluded_categories
                ],
            ...
            "performance": {
                ...
                / max(1, self.processing_stats["fields_processed"]),
                "filter_efficiency": self.processing_stats["fields_filtered"]
                / max(
                    1
                    self.processing_stats["fields_processed"]
                    + self.processing_stats["fields_filtered"],
                ),
            },
        }
...
def create_domain_selector(domain: str) -> ContextFieldSelector:
    ...
    config = ContextFieldConfig(
        processing_level=ProcessingLevel.STANDARD, domain_focus=domain
    )
    return ContextFieldSelector(config)
