...
class LazyInitializationManager:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=8, thread_name_prefix="KimeraLazy"
        )
    ...
    def register_component(self, config: ComponentConfig) -> None:
        ...
        with self._global_lock:
            if config.name in self.components:
                logger.warning(
                    f"Component {config.name} already registered, updating configuration"
                )
            ...
            logger.info(
                f"📦 Registered component: {config.name} (Priority: {config.priority.name})"
            )
        ...
        with self._locks[name]:
            ...
            if component.state == InitializationState.INITIALIZING:
                if component.initialization_future:
                    try:
                        component.initialization_future.result(
            ...
            try:
                ...
                if level == "basic" or level == "enhanced" or level == "full":
                    ...
                    if level == "basic":
                        ...
                        logger.info(
                            f"✅ {name} basic initialization complete ({init_time:.2f}s)"
                        )
                ...
                if level == "enhanced" or level == "full":
                    ...
                    if component.config.enhanced_initializer:
                        ...
                        enhanced_instance = component.config.enhanced_initializer(
                        ...
                        if level == "enhanced":
                            ...
                            self.stats["total_init_time"] += init_time
                            logger.info(
                                f"✅ {name} enhanced initialization complete ({init_time:.2f}s)"
                            )
                ...
                if level == "full":
                    ...
                    if component.config.full_initializer:
                        ...
                        self.stats["total_init_time"] += init_time
                        logger.info(
                            f"✅ {name} full initialization complete ({init_time:.2f}s)"
                        )
                ...
                return (
                    component.instance
                    or component.enhanced_instance
                    or component.basic_instance
                )
    ...
    def _create_fallback(self, component: ComponentInstance) -> Optional[Any]:
        ...
        if component.config.fallback_factory:
            try:
                ...
                logger.error(
                    f"❌ Fallback creation failed for {component.config.name}: {e}"
                )
    ...
    def initialize_critical_components(self) -> None:
        ...
        logger.info(
            f"🚀 Initializing {len(critical_components)} critical components..."
        )
    ...
    def start_background_enhancement(self) -> None:
        ...
        high_priority_components = [
            name
            for name, comp in self.components.items()
            if comp.config.priority == Priority.HIGH
            and comp.state == InitializationState.BASIC_READY
        ]
...
def lazy_init(
    manager: LazyInitializationManager, component_name: str, level: str = "basic"
):
