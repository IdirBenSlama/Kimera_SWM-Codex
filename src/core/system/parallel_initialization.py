...
from src.core.async_operations.async_performance_monitor import get_performance_monitor
from src.core.system.task_manager import get_task_manager, initialize_task_manager
...
class ParallelInitializer:
    ...
    def register_component(
        ...
        logger.debug(
            f"Registered component: {name} (dependencies: {component.dependencies})"
        )

    def add_progress_callback(
        self, callback: Callable[[InitializationProgress], None]
    ) -> None:
    ...
    async def initialize_all(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Starting parallel initialization of {len(self.components)} components"
        )
        ...
        for level_num, level_components in enumerate(init_levels):
            logger.info(
                f"Initializing level {level_num + 1}/{len(init_levels)}: {level_components}"
            )
            ...
            for component_name in level_components:
                ...
                if self._are_dependencies_satisfied(component):
                    task = asyncio.create_task(self._initialize_component(component))
    ...
    def _are_dependencies_satisfied(self, component: ComponentInfo) -> bool:
        ...
        for dep in component.dependencies:
            if dep not in self.components:
                logger.warning(
                    f"Unknown dependency '{dep}' for component '{component.name}'"
                )
                return False
    ...
    async def _initialize_component(self, component: ComponentInfo) -> None:
        ...
        async with self.performance_monitor.track_operation(f"init_{component.name}"):
            for attempt in range(component.retry_count):
                try:
                    logger.info(
                        f"Initializing component: {component.name} (attempt {attempt + 1}/{component.retry_count})"
                    )

                    # Run initialization with timeout
                    component.result = await asyncio.wait_for(
                        component.init_func(), timeout=component.timeout
                    )
                    ...
                    logger.info(
                        f"Component '{component.name}' initialized in {component.init_time:.2f}s"
                    )
                    self._update_progress()
                    return
                    ...
                    logger.error(
                        f"Component '{component.name}' initialization timeout (attempt {attempt + 1})"
                    )
                    ...
                    logger.error(
                        f"Component '{component.name}' initialization failed: {error_msg}"
                    )
    ...
    def _log_initialization_summary(self) -> None:
        ...
        logger.info(
            f"Components initialized: {self.progress.initialized}/{self.progress.total_components}"
        )
        ...
        for comp in sorted_components[:10]:  # Top 10 slowest
            status_icon = "✓" if comp.status == ComponentStatus.INITIALIZED else "✗"
