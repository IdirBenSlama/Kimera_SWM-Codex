...
from src.core.context.anthropomorphic_profiler import (AnthropomorphicProfiler
                                                       InteractionAnalysis)
from src.core.context.context_field_selector import ContextFieldSelector
...
class InterpreterLearningState:
    ...
    context_expertise: Dict[str, float] = field(
        default_factory=dict
    )  # Domain expertise levels
    pattern_effectiveness: Dict[str, List[float]] = field(
        default_factory=dict
    )  # Pattern success rates
    optimal_configurations: Dict[str, Dict[str, Any]] = field(
        default_factory=dict
    )  # Best settings per context
...
class SelectiveFeedbackInterpreter:
    ...
    def __init__(self, base_profiler: AnthropomorphicProfiler):
        ...
        logger.info(
            "🧠🔄 Selective Feedback Interpreter - Analysis learning enabled, behaviors protected"
        )
    ...
    def _enhance_analysis_only(
        ...
        if context_type in self.learned_analysis_patterns:
            ...
            enhanced_profiler.technical_patterns["high"].extend(
                learned_patterns.get("technical", [])
            )
        ...
        enhanced_profiler._analysis_context = (
            context  # Informs behavior without changing it
        )

        return enhanced_profiler


def create_selective_feedback_interpreter(
    domain_focus: str = "balanced",
) -> SelectiveFeedbackInterpreter:
    ...
    from src.core.context.context_field_selector import create_domain_selector

    base_profiler = create_default_profiler()
    context_selector = (
        create_domain_selector(domain_focus) if domain_focus != "balanced" else None
    )
