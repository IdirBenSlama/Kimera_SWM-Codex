...
class BackgroundJobManager:
    ...
    def _schedule_retry(self, job_id: str, job_config: JobConfiguration):
        ...
        retry_time = datetime.now(timezone.utc) + timedelta(
            seconds=job_config.retry_delay
        )
    ...
    def add_job(self, job_config: JobConfiguration) -> str:
        ...
        with self._lock:
            ...
            self.metrics[job_id] = JobMetrics(job_id=job_id, job_name=job_config.name)
        ...
        logger.info(
            f"Added job {job_id} ({job_config.name}) with {job_config.trigger} trigger"
        )
    ...
    def _crystallization_job(self):
        ...
        try:
            ...
            high_scars = (
                db.query(ScarDB)
                .filter(ScarDB.weight > self.CRYSTAL_WEIGHT_THRESHOLD)
                .all()
            )
            ...
            for scar in high_scars:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to generate embedding for SCAR {scar.scar_id}: {e}"
                    )
                    continue
            ...
            db.commit()
            logger.info(
                f"Crystallized {crystallized_count} high-weight SCARs into Geoids"
            )
