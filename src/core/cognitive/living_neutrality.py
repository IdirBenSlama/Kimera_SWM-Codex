...
class LivingNeutralityEngine:
    ...
    def _determine_neutrality_expression(
        self, context: Dict[str, Any]
    ) -> NeutralityExpression:
        ...
        return expression_map.get(
            context_type, NeutralityExpression.PASSIONATE_OBJECTIVITY
        )

    def _determine_required_tensions(
        self, context: Dict[str, Any]
    ) -> List[TensionType]:
    ...
    def _determine_emotional_permissions(
        self, context: Dict[str, Any]
    ) -> Dict[str, float]:
        ...
        if context_type == "creative_breakthrough":
            permissions.update({"excitement": 0.9, "passion": 0.8, "wonder": 0.95})
            ...
            permissions.update({"curiosity": 0.95, "wonder": 0.9, "excitement": 0.7})

        elif stakes == "high":
            permissions.update({"concern": 0.8, "determination": 0.8, "passion": 0.6})
    ...
    def _calculate_drift_authorization(self, context: Dict[str, Any]) -> float:
        ...
        drift_authorization = (
            base_drift + evidence_factor + innovation_factor
        ) * stakes_multiplier

        return min(0.95, max(0.1, drift_authorization))  # Keep in reasonable bounds

    def orchestrate_living_tension(
        self, context_assessment: Dict[str, Any]
    ) -> TensionField:
        ...
        for emotion, permission in context_assessment["emotional_permissions"].items():
            if (
                permission > 0.3
            ):  # Only create currents for sufficiently permitted emotions
                current = self._create_emotional_current(
                    emotion, permission, context_assessment
                )
        ...
        logger.info(
            f"Living tension field orchestrated: {len(self.active_tensions)} tensions, "
    ...
    def _create_cognitive_tension(
        self, tension_type: TensionType, context_assessment: Dict[str, Any]
    ) -> CognitiveTension:
        ...
        if tension_type == TensionType.PASSION_OBJECTIVITY:
            ...
            creativity_demand = context_assessment["context_analysis"][
                "creativity_demand"
            ]
    ...
    def _create_emotional_current(
        self, emotion: str, permission: float, context_assessment: Dict[str, Any]
    ) -> EmotionalCurrent:
        ...
        wisdom_data = emotional_wisdom.get(
            emotion
            {
    ...
    def _calculate_field_coherence(self) -> float:
        ...
        productive_tensions = sum(
            1
            for t in self.active_tensions.values()
            if t.tension_quality == "productive"
        )
        total_tensions = len(self.active_tensions)

        tension_coherence = (
            productive_tensions / total_tensions if total_tensions > 0 else 0
        )

        # Check emotional current harmony
        high_intensity_emotions = sum(
            1 for c in self.emotional_currents if c.intensity > 0.7
        )
        total_emotions = len(self.emotional_currents)

        # Too many high-intensity emotions can create chaos
        emotional_balance = (
            1.0 - (high_intensity_emotions / max(total_emotions, 1)) * 0.5
        )
    ...
    def _calculate_creative_pressure(self) -> float:
        ...
        total_potential = sum(
            t.creative_potential for t in self.active_tensions.values()
        )
        ...
        creative_current = sum(
            c.intensity
            for c in self.emotional_currents
        ...
        creative_pressure = (
            avg_potential * 0.7 + creative_current * 0.3
        ) * field_coherence
    ...
    def _assess_context_alignment(self, context_assessment: Dict[str, Any]) -> float:
        ...
        active_productive = sum(
            1
            for rt in required_tensions
            if rt.value in self.active_tensions
            and self.active_tensions[rt.value].tension_quality == "productive"
        )

        tension_alignment = (
            active_productive / len(required_tensions) if required_tensions else 1.0
        )

        # Check if emotional permissions are being respected
        emotional_permissions = context_assessment["emotional_permissions"]
        aligned_emotions = sum(
            1
            for ec in self.emotional_currents
            if ec.intensity <= emotional_permissions.get(ec.emotion_type, 0.5) + 0.1
        )

        emotional_alignment = (
            aligned_emotions / len(self.emotional_currents)
            if self.emotional_currents
            else 1.0
        )
    ...
    def _extract_context_wisdom(self, context: Dict[str, Any]) -> str:
        ...
        base_wisdom = wisdom_templates.get(
            context_type, "Every context teaches if we remain open"
        )
    ...
    def enable_genius_drift(
        self, tension_field: TensionField, contradiction: str, evidence_strength: float
    ) -> Dict[str, Any]:
        ...
        if (
            drift_potential > drift_threshold
            and drift_potential <= tension_field.drift_authorization
        ):
            return {
    ...
    def _generate_field_wisdom(self, field: TensionField) -> str:
        ...
        return (
            f"{coherence_wisdom}. {creative_wisdom}. Anchor: {field.stability_anchor}."
        )


# --- Thread-Safe Singleton Access ---
_global_living_neutrality: Optional[LivingNeutralityEngine] = None
_neutrality_lock = threading.Lock()


def get_living_neutrality_engine() -> LivingNeutralityEngine:
    """
    Provides a thread-safe singleton instance of the LivingNeutralityEngine.
