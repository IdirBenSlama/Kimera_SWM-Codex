...
class ResourcePool:
    ...
    def can_allocate(self, requirements: Dict[str, float]) -> bool:
        ...
        return (
            self.allocated_cpu + cpu_needed <= self.cpu_cores
            and self.allocated_memory + memory_needed <= self.memory_gb
            and self.allocated_gpu_memory + gpu_memory_needed <= self.gpu_memory_gb
    ...
    def validate_dependencies(self, all_tasks: Set[str]) -> bool:
        ...
        for task_id in all_tasks:
            ...
            if not dependencies.issubset(all_tasks):
                logger.error(
                    f"Task {task_id} has invalid dependencies: {dependencies - all_tasks}"
                )
                return False
...
class LoadBalancer:
    ...
    def __init__(self, resource_pools: List[ResourcePool]):
        ...
        self.pool_metrics = [
            {"tasks_completed": 0, "avg_duration": 0.0} for _ in resource_pools
        ]
    ...
    def select_optimal_pool(self, task: PipelineTask) -> Optional[int]:
        ...
        with self._lock:
            ...
            for i, pool in enumerate(self.resource_pools):
                ...
                efficiency = self.pool_metrics[i]["tasks_completed"] / max(
                    1, pool.active_tasks
                )
    ...
    def update_pool_metrics(self, pool_index: int, task_duration: float):
        ...
        with self._lock:
            ...
            metrics["avg_duration"] = (
                alpha * task_duration + (1 - alpha) * metrics["avg_duration"]
            )
...
class PipelineOptimizer:
    ...
    async def execute_task(self, task: PipelineTask, pool_index: int) -> Any:
        ...
        try:
            ...
            logger.debug(
                f"Task {task.task_id} completed in {task.actual_duration:.3f}s"
            )
            return result
    ...
    def get_pipeline_metrics(self) -> PipelineMetrics:
        ...
        self.metrics.resource_efficiency = (
            active_tasks / total_capacity if total_capacity > 0 else 0
        )
...
if __name__ == "__main__":
    ...
    async def test_pipeline():
        ...
        add_pipeline_task(
            "task3", test_task, "Task 3", 0.3, priority=TaskPriority.MEDIUM
        )
