...
class SemanticCacheManager:
    ...
    def find_similar_entries(
        self, query_embedding: torch.Tensor, cache_entries: Dict[str, CacheEntry]
...
class L1MemoryCache:
    ...
    def __init__(self, config: CacheConfiguration):
        ...
        self.semantic_manager = SemanticCacheManager(
    ...
    def _compute_priority_score(self, entry: CacheEntry) -> float:
        ...
        recency_score = 1.0 / (
            1.0 + (current_time - entry.last_access) / 3600
        )  # Decay over hours
    ...
    def get(self, key: str, input_data: Optional[Any] = None) -> Optional[Any]:
        ...
        with self._lock:
            ...
            if self.config.enable_semantic_caching and input_data is not None:
                ...
                similar_entries = self.semantic_manager.find_similar_entries(
                    query_embedding, self.cache
                )
    ...
    def put(
        ...
        with self._lock:
            try:
                ...
                if self.config.enable_semantic_caching and input_data is not None:
                    semantic_embedding = self.semantic_manager.compute_embedding(
                        input_data
                    )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            hit_rate = (
                self.access_stats["hits"] / total_requests if total_requests > 0 else 0
            )
...
class AdvancedCacheManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"L2 Cache: {'Enabled' if self.config.l2_enabled else 'Disabled'}"
            )
            logger.info(
                f"Semantic Caching: {'Enabled' if self.config.enable_semantic_caching else 'Disabled'}"
            )
    ...
    def _update_access_time(self, start_time: float):
        ...
        self.global_stats.avg_access_time_ms = (
            alpha * access_time + (1 - alpha) * self.global_stats.avg_access_time_ms
        )
    ...
    def get_comprehensive_stats(self) -> CacheMetrics:
        ...
        total_misses = (
            self.global_stats.l1_misses
            + self.global_stats.l2_misses
            + self.global_stats.l3_misses
        )
