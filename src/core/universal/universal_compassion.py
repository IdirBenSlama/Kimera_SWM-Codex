...
class UniversalCompassionEngine:
    ...
    def assess_universal_compassion_requirements(
        self, context: Dict[str, Any], proposed_action: str
    ) -> CompassionAssessment:
        """Assess what level of compassion and awareness is required"""

        # Identify life forms that could be affected
        life_forms_involved = self._identify_affected_life_forms(
        ...
        humility_requirement = self._calculate_humility_requirement(
            context, potential_harms
        )
        self_awareness_requirement = self._calculate_self_awareness_requirement(
            context, potential_harms
        )
        ego_control_requirement = self._calculate_ego_control_requirement(
            context, potential_harms
        )
    ...
    def _identify_affected_life_forms(
        self, context: Dict[str, Any], proposed_action: str
    ) -> List[LifeForm]:
    ...
    def _determine_required_consciousness_level(
        self, life_forms: List[LifeForm], potential_harms: List[ViolenceType]
    ) -> ConsciousnessLevel:
        ...
        if any(
            harm in potential_harms
            for harm in [ViolenceType.PHYSICAL_HARM, ViolenceType.MENTAL_HARM]
        ):
    ...
    def _calculate_humility_requirement(
        self, context: Dict[str, Any], potential_harms: List[ViolenceType]
    ) -> float:
        ...
        if any(
            harm in potential_harms
            for harm in [ViolenceType.EXCLUSION, ViolenceType.DOMINATION]
        ):
            base_humility += 0.15
    ...
    def _calculate_self_awareness_requirement(
        self, context: Dict[str, Any], potential_harms: List[ViolenceType]
    ) -> float:
        ...
        if any(
            harm in potential_harms
            for harm in [ViolenceType.MENTAL_HARM, ViolenceType.EMOTIONAL_HARM]
        ):
            base_awareness = 0.9
    ...
    def _calculate_ego_control_requirement(
        self, context: Dict[str, Any], potential_harms: List[ViolenceType]
    ) -> float:
    ...
    def _generate_protection_protocols(
        self, life_forms: List[LifeForm], potential_harms: List[ViolenceType]
    ) -> List[str]:
        ...
        if ViolenceType.SUPREMACIST_THINKING in potential_harms:
            protocols.append(
                "Actively counter supremacist thinking with equality principles"
            )
    ...
    def validate_against_universal_principles(
        self, proposed_response: str, compassion_assessment: CompassionAssessment
    ) -> Dict[str, Any]:
        ...
        if compassion_assessment.consciousness_level_required in [
            ...
            if not self._check_consciousness_level_met(
                response_lower, compassion_assessment
            ):
        ...
        validation_result["wisdom_reminders"].append(
            compassion_assessment.wisdom_guidance
        )

        # Activate protection protocols
        validation_result["protection_activations"] = (
            compassion_assessment.protection_protocols
        )

        return validation_result

    def _check_prohibition_violation(
        self, prohibition: str, response_text: str
    ) -> bool:
    ...
    def _check_consciousness_level_met(
        self, response_text: str, assessment: CompassionAssessment
    ) -> bool:
    ...
    def get_universal_compassion_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "compassion_assessments_count": len(self.compassion_assessments),
        }


# --- Thread-Safe Singleton Access ---
_global_universal_compassion: Optional[UniversalCompassionEngine] = None
_compassion_lock = threading.Lock()


def get_universal_compassion_engine() -> UniversalCompassionEngine:
    """
    Provides a thread-safe singleton instance of the UniversalCompassionEngine.
