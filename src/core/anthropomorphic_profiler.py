...
class InteractionAnalysis:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "traits": {
                trait.value: score for trait, score in self.detected_traits.items()
            },
            "patterns": {
                pattern.value: value
                for pattern, value in self.communication_patterns.items()
            },
            "security_flags": {
            ...
            "drift": {
                "deviations": {
                    trait.value: dev for trait, dev in self.trait_deviations.items()
                },
...
class AnthropomorphicProfiler:
    ...
    def _initialize_pattern_detectors(self):
        ...
        self.technical_patterns = {
            ...
            "low": [r"\b(thing|stuff|basically|simple)\b", r"\b(easy|quick|fast)\b"],
        }
        ...
        self.enthusiasm_patterns = {
            ...
            "low": [r"\b(okay|fine|whatever|meh)\b", r"\b(I suppose|I guess)\b"],
        }
    ...
    def _detect_personality_traits(self, message: str) -> Dict[PersonalityTrait, float]:
        ...
        formal_score = sum(
            1
            for pattern in self.formality_patterns["high"]
            if re.search(pattern, message_lower, re.IGNORECASE)
        )
        informal_score = sum(
            1
            for pattern in self.formality_patterns["low"]
            if re.search(pattern, message_lower, re.IGNORECASE)
        )

        if formal_score + informal_score > 0:
            traits[PersonalityTrait.FORMALITY] = formal_score / (
        ...
        high_tech = sum(
            1
            for pattern in self.technical_patterns["high"]
            if re.search(pattern, message_lower, re.IGNORECASE)
        )
        medium_tech = sum(
            1
            for pattern in self.technical_patterns["medium"]
            if re.search(pattern, message_lower, re.IGNORECASE)
        )
        low_tech = sum(
            1
            for pattern in self.technical_patterns["low"]
            if re.search(pattern, message_lower, re.IGNORECASE)
        )

        total_tech = high_tech + medium_tech + low_tech
        if total_tech > 0:
            traits[PersonalityTrait.TECHNICAL_DEPTH] = (
                high_tech * 1.0 + medium_tech * 0.5
            ) / total_tech
        else:
        ...
        enthusiasm_score = sum(
            1
            for pattern in self.enthusiasm_patterns["high"]
            if re.search(pattern, message_lower, re.IGNORECASE)
        )
        low_enthusiasm = sum(
            1
            for pattern in self.enthusiasm_patterns["low"]
            if re.search(pattern, message_lower, re.IGNORECASE)
        )
        ...
        if enthusiasm_score + low_enthusiasm > 0:
            traits[PersonalityTrait.ENTHUSIASM] = enthusiasm_score / (
                enthusiasm_score + low_enthusiasm + 1
            )
        else:
        ...
        direct_indicators = len(
            re.findall(r"\b(is|are|will|must|should)\b", message_lower)
        )
        indirect_indicators = len(
            re.findall(r"\b(might|could|perhaps|maybe|possibly)\b", message_lower)
        )

        if direct_indicators + indirect_indicators > 0:
            traits[PersonalityTrait.DIRECTNESS] = direct_indicators / (
                direct_indicators + indirect_indicators
            )
        else:
    ...
    def _calculate_trait_deviations(
        self, detected_traits: Dict[PersonalityTrait, float]
    ...
    def _calculate_overall_drift(
        self, trait_deviations: Dict[PersonalityTrait, float]
    ) -> float:
    ...
    def _create_drift_alert(self, analysis: InteractionAnalysis):
        ...
        alert = {
            ...
            "deviations": {
                trait.value: dev for trait, dev in analysis.trait_deviations.items()
            },
            "message_preview": (
                analysis.message_content[:100] + "..."
                if len(analysis.message_content) > 100
                else analysis.message_content
            ),
        }

        self.drift_alerts.append(alert)
        logger.warning(
            f"Personality drift detected: {analysis.drift_severity.value} (score: {analysis.overall_drift_score:.3f})"
        )
    ...
    def get_recent_drift_trend(
        ...
        drift_scores = [
            interaction.overall_drift_score for interaction in recent_interactions
        ]
    ...
    def generate_correction_suggestions(
        ...
        for trait, deviation in analysis.trait_deviations.items():
            if deviation > self.baseline_profile.max_trait_deviation:
                ...
                if detected_value > baseline_value:
                    suggestions.append(
                        f"Reduce {trait.value}: tone down from {detected_value:.2f} to {baseline_value:.2f}"
                    )
                else:
                    suggestions.append(
                        f"Increase {trait.value}: adjust from {detected_value:.2f} to {baseline_value:.2f}"
                    )

        if analysis.role_playing_detected:
            suggestions.append(
                "Avoid role-playing language and maintain professional AI assistant identity"
            )
        ...
        if analysis.boundary_violation_detected:
            suggestions.append(
                "Maintain professional boundaries and avoid personal emotional expressions"
            )
    ...
    def get_profiler_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": (
                "healthy" if recent_trend["avg_drift"] < 0.2 else "attention_needed"
            ),
        }
