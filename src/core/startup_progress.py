...
class StartupProgressTracker:
    ...
    async def start_step(self, name: str, stage: ProgressStage, message: str = "") -> ProgressStep:
    ...
    async def complete_current_step(self, message: str = ""):
        ...
        if self.current_step:
            async with self._lock:
                ...
                logger.info(f"Completed step: {self.current_step.name} in {self.current_step.duration:.2f}s")
                await self._notify_callbacks(self.current_step)
        ...
        return {
            ...
            "timeline": [
                {
                    ...
                    "end_time": datetime.fromtimestamp(step.end_time).isoformat() if step.end_time else None,
                    "duration": step.duration,
                    "progress": step.progress,
                    "message": step.message,
                    "error": step.error
            ...
            "performance_metrics": {
                ...
                "average_step_time": sum(
                    s.duration for s in self.steps if s.duration
                ) / len([s for s in self.steps if s.duration]) if self.steps else 0
            }
        }
