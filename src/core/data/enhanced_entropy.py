...
from ..core.native_math import NativeMath, NativeStats
...
try:
    ...
    if os.getenv("KIMERA_DEBUG", "").lower() in ("true", "1", "yes"):
        warnings.warn(
            "SciPy not available. Using native implementations only.", UserWarning
        )
class EnhancedEntropyCalculator:
    ...
    def shannon_entropy_enhanced(
        self, probabilities: List[float], base: float = 2.0, estimator: str = "mle"
    ) -> float:
        """
    ...
    def differential_entropy(
        self, data: List[float], distribution: str = "gaussian"
    ) -> float:
        """
        ...
        if distribution == "gaussian":
            ...
            except (ValueError, RuntimeError, OptimizeError) as e:
                logger.debug(f"Exponential distribution fitting failed: {e}")
                return self._differential_entropy_fallback(data)
    ...
    def cross_entropy(self, p: List[float], q: List[float], base: float = 2.0) -> float:
        """
    ...
    def kl_divergence_enhanced(
        self, p: List[float], q: List[float], base: float = 2.0
    ) -> float:
        """
        ...
        try:
            ...
            logger.debug(f"SciPy KL divergence calculation failed: {e}")
            return self._kl_divergence_native(p, q, base)
    ...
    def conditional_entropy_enhanced(
        self, xy_joint: List[List[float]], x_marginal: List[float], base: float = 2.0
    ) -> float:
        """
    ...
    def entropy_rate(self, sequence: List[Any], order: int = 1) -> float:
        """
    ...
    def _shannon_dirichlet(self, probs: np.ndarray, base: float) -> float:
        ...
        try:
            ...
            logger.debug(f"Dirichlet entropy estimation failed: {e}")
            return self._shannon_mle(probs, base)
    ...
    def _kl_divergence_native(
        self, p: List[float], q: List[float], base: float
    ) -> float:
    ...
    def _sample_entropy_scipy(
        self, data: np.ndarray, m: int, tolerance: float
    ) -> float:
    ...
    def _mutual_information_scipy(
        self, x_data: List[float], y_data: List[float], bins: int, base: float
    ) -> float:
                ...
                for j in range(len(p_y)):
                    if hist_2d[i, j] > 0 and p_x[i] > 0 and p_y[j] > 0:
                        if base == 2.0:
                            ...
                            mi += hist_2d[i, j] * math.log(
                                hist_2d[i, j] / (p_x[i] * p_y[j])
                            )
                        else:
                            mi += (
                                hist_2d[i, j]
                                * math.log(hist_2d[i, j] / (p_x[i] * p_y[j]))
                                / math.log(base)
                            )

            return mi
        except (ValueError, ZeroDivisionError, RuntimeError) as e:
            logger.debug(f"SciPy mutual information calculation failed: {e}")
            return self._mutual_information_native(x_data, y_data, bins, base)

    def _mutual_information_native(
        self, x_data: List[float], y_data: List[float], bins: int, base: float
    ) -> float:
...
def enhanced_shannon_entropy(
    probabilities: List[float], base: float = 2.0, estimator: str = "mle"
) -> float:
...
def differential_entropy(data: List[float], distribution: str = "gaussian") -> float:
...
def jensen_shannon_divergence(
    p: List[float], q: List[float], base: float = 2.0
) -> float:
...
def multiscale_entropy(data: List[float], scales: List[int] = None) -> Dict[int, float]:
...
def mutual_information_enhanced(
    x_data: List[float], y_data: List[float], bins: int = 10
) -> float:
