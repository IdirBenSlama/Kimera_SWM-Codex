...
class CoherenceService:
    ...
    def _calculate_thematic_alignment(self, geoids: List[GeoidState]) -> float:
        ...
        all_features = set(
            key for geoid in geoids for key in geoid.semantic_state.keys()
        )
        if not all_features:
            return 1.0
    ...
    def compute_global_coherence(
        ...
        support_score = 1.0 - (
            1 / (1 + support_links)
        )  # Approaches 1 as links increase
        ...
        coherence = (
            base_coherence + (1 - base_coherence) * support_score * 0.5
        )  # Bonus fills 50% of the gap to 1.0

        # Clamp the result to [0, 1]
        final_coherence = np.clip(coherence, 0, 1)
        log.info(
            f"Computed global coherence: {final_coherence:.4f} (Alignment: {thematic_alignment:.2f}, Penalty: {contradiction_penalty:.2f})"
        )
    ...
    def temperature(self) -> float:
        ...
        raise NotImplementedError(
            "Temperature must be calculated from a coherence score. Call compute_global_coherence first."
        )
