...
from ..vault.database import GeoidDB, ScarDB
...
class AxisStabilityMonitor:
    ...
    def get_stability_metrics(self) -> dict:
        ...
        recent_scars_count = (
            ...
            .filter(ScarDB.timestamp > datetime.now(timezone.utc) - timedelta(hours=1))
            .count()
        )
        ...
        recent_geoids = (
            self.db.query(GeoidDB).order_by(GeoidDB.geoid_id.desc()).limit(20).all()
        )
        ...
        if len(recent_geoids) > 1:
            try:
                ...
                if len(vectors) > 1:
                    ...
                    distances = NativeDistance.condensed_distances(
                        norm_vectors, metric="cosine"
                    )
                    if distances:
        ...
        recent_scars = (
            self.db.query(ScarDB).order_by(ScarDB.timestamp.desc()).limit(20).all()
        )
