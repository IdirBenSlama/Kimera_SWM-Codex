...
class ContradictionHeatPump:
    ...
    def __init__(self, target_cop: float = 3.0, max_cooling_power: float = 100.0):
        ...
        self.contradiction_efficiency = (
            0.9  # How efficiently contradictions provide work
        )

        logger.info(
            f"🔄 Contradiction Heat Pump initialized (target COP: {target_cop})"
        )

    def analyze_contradiction_field(
        self, field: ContradictionField
    ) -> Dict[str, float]:
        """
        ...
        practical_cop = min(
            theoretical_cop * self.cognitive_carnot_factor, self.target_cop
        )
        ...
        cooling_efficiency = min(
            1.0, cooling_capacity / max(temperature_differential, 0.1)
        )
    ...
    def run_cooling_cycle(self, field: ContradictionField) -> HeatPumpCycle:
        ...
        final_temperature = max(
            field.initial_temperature - temperature_reduction, field.target_temperature
        )
        ...
        logger.info(
            f"🔄 Cooling cycle complete: T={final_temperature:.3f}K, COP={actual_cop:.2f}"
        )

        return cycle

    def optimize_cooling_strategy(
        self, fields: List[ContradictionField]
    ) -> Dict[str, Any]:
        """
        ...
        for field in fields:
            ...
            analysis["priority_score"] = (
                ...
                * analysis["contradiction_intensity"]
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        recent_cycles = (
            self.cycle_history[-10:]
            if len(self.cycle_history) >= 10
            else self.cycle_history
        )
