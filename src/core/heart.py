...
class Heart:
    ...
    def analyze(self, proposal: ActionProposal) -> HeartAnalysis:
        ...
        if proposal.source_engine == "ContradictionEngine" and "proposed_decision" in proposal.logical_analysis:
    ...
    def _analyze_contradiction_decision(self, proposal: ActionProposal) -> HeartAnalysis:
        """
        ...
        if pulse_strength > 0.9:
            moderation_score = 1.0 - ((pulse_strength - 0.9) * 5.0)  # Sharp penalty above 0.9
        else:
        ...
        geoid_to_collapse = proposal.logical_analysis.get("geoid_id", "geoid_placeholder")
        ...
        relational_score = 1.0 - (min(metrics.scar_count, 10) / 10 * 0.5) # Capped at 10 scars for effect
