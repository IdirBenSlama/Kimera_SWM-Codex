...
from src.core.processing.native_math import NativeMath, NativeStats

try:
    from src.core.data.enhanced_entropy import EntropyCalculator
...
try:
    ...
    logging.warning(
        "Statsmodels not available. Statistical modeling features will be limited."
    )
...
class SemanticTimeSeriesAnalyzer:
    ...
    def analyze_entropy_series(
        self, entropy_data: List[float], timestamps: Optional[List[datetime]] = None
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"ARIMA modeling failed: {e}")
                arima_model = None
                arima_forecast = None
            ...
            return StatisticalModelResult(
                ...
                predictions=(
                    arima_forecast.tolist() if arima_forecast is not None else None
                ),
    ...
    def _fallback_entropy_analysis(
...
class ContradictionRegressionAnalyzer:
    ...
    def analyze_contradiction_factors(
        self
        ...
        """
        if not STATSMODELS_AVAILABLE:
            return self._fallback_regression_analysis(
            ...
            diagnostics = {
                ...
                "outliers_detected": len(
                    influence.summary_frame()[
                        influence.summary_frame()["cooks_d"] > 0.5
                    ]
                ),
            }
    ...
    def _fallback_regression_analysis(
        self
        contradiction_scores: List[float],
        semantic_features: Dict[str, List[float]],
    ) -> StatisticalModelResult:
...
class SemanticEconometrics:
    ...
    def analyze_semantic_market(
        ...
        """
        if not STATSMODELS_AVAILABLE:
            return self._fallback_market_analysis(
    ...
    def _fallback_market_analysis(
        ...
        market_balance = abs(supply_mean - demand_mean) / max(
            supply_mean, demand_mean, 1.0
        )
    ...
    def comprehensive_analysis(
        ...
        if "entropy_history" in system_data:
            results["entropy_analysis"] = (
                self.time_series_analyzer.analyze_entropy_series(
                    system_data["entropy_history"], system_data.get("timestamps")
                )
            )
        ...
        if "contradiction_scores" in system_data and "semantic_features" in system_data:
            results["contradiction_analysis"] = (
                self.regression_analyzer.analyze_contradiction_factors(
        ...
        if all(
            key in system_data
            for key in ["semantic_supply", "semantic_demand", "entropy_prices"]
        ):
        ...
        if "entropy_history" in system_data:
            ...
            results["regime_analysis"] = StatisticalModelResult(
                ...
                statistics={
                    "stability_score": regime_analysis.get("stability_score", 1.0)
                },
    ...
    def get_model_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_models": (
                ...
                else ["Basic Statistics", "Native Correlation"]
            ),
        }

    def get_model_capabilities(self) -> Dict[str, Any]:
        """Get model capabilities (alias for get_model_summary for API compatibility)"""
        return self.get_model_summary()

    def run_model(self, model_name: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """Run a specific statistical model on the provided data"""
        if model_name == "descriptive_stats":
            values = data.get("values", [])
            if not values:
                return {"mean": 0, "std_dev": 0, "count": 0}
            return {
                "mean": NativeStats.mean(values),
                "std_dev": NativeStats.std(values),
                "count": len(values),
                "min": min(values),
                "max": max(values),
            }
...
def analyze_entropy_time_series(
    entropy_data: List[float], timestamps: Optional[List[datetime]] = None
) -> StatisticalModelResult:
    """Convenience function for entropy time series analysis"""
    return statistical_engine.time_series_analyzer.analyze_entropy_series(
        entropy_data, timestamps
    )


def analyze_contradiction_factors(
    contradiction_scores: List[float], semantic_features: Dict[str, List[float]]
) -> StatisticalModelResult:
