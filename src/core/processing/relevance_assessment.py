...
from src.core.ethics.immutable_laws import ImmutableLaw, get_law_registry
...
class RelevanceAssessmentEngine:
    ...
    def assess_context(
        ...
        benefit_potential = self._assess_benefit_potential(
            context_type, relevance_level
        )
        ...
        logger.debug(
            f"Context assessed: {context_type.value}, relevance: {relevance_level.value}"
        )

        return assessment

    def authorize_rule_flexibility(
        self, law_id: str, context_assessment: ContextAssessment
        ...
        safety_constraint = self._apply_safety_constraint(
            context_assessment.safety_level
        )
        ...
        authorization_level = self._determine_authorization_level(
            final_flexibility, context_assessment
        )

        # Generate justification
        justification = self._generate_justification(
            law, context_assessment, final_flexibility
        )
        ...
        stabilization_urgency = self._determine_stabilization_urgency(
            final_flexibility, context_assessment
        )
        ...
        logger.info(
            f"Authorized flexibility for {law_id}: {final_flexibility:.2f} ({authorization_level})"
        )
    ...
    def _assess_relevance(
        self, input_text: str, context_type: ContextType, user_context: Dict[str, Any]
    ) -> RelevanceLevel:
        ...
        if (
            ...
            and user_context
            and user_context.get("is_student")
        ):
            relevance_indicators += 2

        if (
            context_type == ContextType.THERAPEUTIC
            and user_context
            and user_context.get("seeking_support")
        ):
            relevance_indicators += 2
    ...
    def _assess_benefit_potential(
        self, context_type: ContextType, relevance_level: RelevanceLevel
    ) -> float:
    ...
    def _assess_harm_potential(
        self, safety_level: SafetyLevel, context_type: ContextType
    ) -> float:
    ...
    def _calculate_confidence_score(
        self, input_text: str, context_type: ContextType
    ) -> float:
    ...
    def _extract_context_factors(
        self, context_type: ContextType, user_context: Dict[str, Any]
    ) -> List[str]:
    ...
    def _calculate_context_adjustment(
        self, law: ImmutableLaw, assessment: ContextAssessment
    ) -> float:
        ...
        total_adjustment = (
            relevance_adjustment + benefit_factor + urgency_factor + expertise_factor
        )
    ...
    def _determine_authorization_level(
        self, flexibility: float, assessment: ContextAssessment
    ) -> str:
    ...
    def _generate_justification(
        self, law: ImmutableLaw, assessment: ContextAssessment, flexibility: float
    ) -> str:
    ...
    def _determine_stabilization_urgency(
        self, flexibility: float, assessment: ContextAssessment
    ) -> str:
