...
class AdvancedUnderstandingIntegration:
    ...
    async def process_advanced_understanding(
        self, request: AdvancedUnderstandingRequest
    ) -> AdvancedUnderstandingResult:
        """
        ...
        try:
            ...
            if self.understanding_engine:
                ...
                genuine_understanding = (
                    await self.understanding_engine.understand_content(
                ...
                if request.require_causal_analysis and hasattr(
                    self.understanding_engine, "_process_causal_understanding"
                ):
                    capabilities_used.append("causal_analysis")

                # Add ethical analysis if requested
                if request.require_ethical_analysis and hasattr(
                    self.understanding_engine, "value_system"
                ):
                    capabilities_used.append("ethical_reasoning")

                # Determine analysis depth
                understanding_depth = (
                    genuine_understanding.understanding_depth
                    if hasattr(genuine_understanding, "understanding_depth")
                    else 0.5
                )
    ...
    async def simple_understanding(self, content: str) -> Dict[str, Any]:
        ...
        return {
            "understanding_depth": getattr(
                result.genuine_understanding, "understanding_depth", 0.5
            ),
            "confidence": getattr(
    ...
    def get_understanding_stats(self) -> Dict[str, Any]:
        ...
        if self.understanding_history:
            ...
            avg_understanding_depth = sum(
                op.get("understanding_depth", 0) for op in recent_operations
        ...
        success_rate = (
            self.successful_understanding_operations
            / max(self.total_understanding_requests, 1)
        ) * 100
    ...
    async def test_advanced_understanding(self) -> bool:
        ...
        try:
            ...
            is_working = (
                result.genuine_understanding is not None
                and getattr(result.genuine_understanding, "understanding_depth", 0) > 0
                and len(result.capabilities_used) > 0
                and result.analysis_depth != "error"
            )

            logger.info(
                f"Advanced understanding test: {'PASSED' if is_working else 'FAILED'}"
            )
            if is_working:
                logger.info(f"   Analysis depth: {result.analysis_depth}")
                logger.info(
                    f"   Capabilities used: {', '.join(result.capabilities_used)}"
                )
                logger.info(
                    f"   Understanding depth: {getattr(result.genuine_understanding, 'understanding_depth', 0):.3f}"
                )
