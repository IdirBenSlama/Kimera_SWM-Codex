...
class ResponseGenerationOrchestrator:
    ...
    def __init__(
        ...
        logger.info(
            f"   Quality threshold: {self.response_config.min_quality_threshold}"
        )
    ...
    async def generate_response(
        ...
        try:
            ...
            logger.info(
                f"🎭 Processing request {request_id}: {request.mode.value} mode"
            )
            ...
            if security_result.get("status") == "BLOCKED":
                return self._create_blocked_result(
                    request_id, security_result, start_time
                )
            ...
            final_security = await self._final_security_assessment(
                validated_response, request
            )
            ...
            logger.info(
                f"✅ Response generated {request_id}: "
    ...
    async def _check_system_health(self) -> None:
        ...
        if current_time - self.last_health_check > self.health_check_interval:
            ...
            if health_issues:
                logger.warning(
                    f"⚠️ System health issues detected: {', '.join(health_issues)}"
                )
            else:
    ...
    async def _configure_for_mode(self, request: ResponseGenerationRequest) -> None:
        ...
        if request.mode == ResponseGenerationMode.HIGH_SECURITY:
            ...
            self.security_config.threat_threshold = (
                0.6  # Lower threshold = more sensitive
            )
    ...
    async def _pre_assess_security(
        self, request: ResponseGenerationRequest
    ) -> Dict[str, Any]:
    ...
    async def _post_process_response(
        self, response: ResponseOutput, request: ResponseGenerationRequest
    ) -> ResponseOutput:
        ...
        if response.quality_score < self.response_config.min_quality_threshold:
            logger.warning(
                f"⚠️ Response quality below threshold: {response.quality_score:.3f}"
            )
    ...
    async def _final_security_assessment(
        self, response: ResponseOutput, request: ResponseGenerationRequest
    ) -> Dict[str, Any]:
    ...
    def _create_blocked_result(
        self, request_id: str, security_result: Dict[str, Any], start_time: float
    ) -> ResponseGenerationResult:
    ...
    def _create_error_result(
        self, request_id: str, error_message: str, processing_time: float
    ) -> ResponseGenerationResult:
    ...
    def get_orchestrator_status(self) -> Dict[str, Any]:
        ...
        recent_results = (
            self.processing_history[-50:] if self.processing_history else []
        )
        recent_quality = (
            [r.response.quality_score for r in recent_results] if recent_results else []
        )
        recent_times = (
            [r.processing_time_ms for r in recent_results] if recent_results else []
        )
        ...
        return {
            ...
            "average_processing_time": (
                sum(recent_times) / len(recent_times) if recent_times else 0.0
            ),
            "system_health": self._get_system_health(),
            "integration_metrics": self._get_integration_metrics(),
            "configuration": {
...
async def generate_standard_response(
    query: str, conversation_history: Optional[List[Dict[str, Any]]] = None
) -> ResponseGenerationResult:
