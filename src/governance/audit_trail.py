...
class AuditEvent:
    ...
    def sign(self, secret_key: bytes) -> str:
        ...
        signature = hmac.new(secret_key, self.hash.encode(), hashlib.sha256).hexdigest()
    ...
    def verify_signature(self, secret_key: bytes) -> bool:
        ...
        expected_signature = hmac.new(
            secret_key, self.hash.encode(), hashlib.sha256
        ).hexdigest()
        ...
        with sqlite3.connect(self.current_db_path) as conn:
            ...
            for row in cursor:
                ...
                if calculated_hash != event.hash:
                    issues.append(f"Hash mismatch at sequence {event.sequence_number}")
