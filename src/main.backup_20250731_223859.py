...
from src.core.kimera_system import KimeraSystem, get_kimera_system, kimera_singleton
...
from src.api.foundational_thermodynamic_routes import (
    router as foundational_thermodynamic_routes,
)
...
from src.api.routers.cognitive_architecture_router import (
    router as cognitive_architecture_router,
)
...
from src.api.routers.unified_thermodynamic_router import (
    router as unified_thermodynamic_router,
)
from src.api.routers.vault_router import router as vault_router

# Import pharmaceutical router if available (optional addon module)
try:
    from src.api.cognitive_pharmaceutical_routes import (
        router as cognitive_pharmaceutical_routes,
    )

    PHARMACEUTICAL_ROUTER_AVAILABLE = True
except ImportError:
    cognitive_pharmaceutical_routes = None
    PHARMACEUTICAL_ROUTER_AVAILABLE = False
from src.api.auth_routes import router as auth_router
from src.api.auth_routes import token_router
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error(
            f"❌ Failed to initialize enhanced cognitive control services: {e}",
            exc_info=True,
        )
    ...
    try:
        ...
        logger.warning(
            f"Metrics collection will be disabled due to error: {metric_error}"
        )
...
app.include_router(
    contradiction_router, prefix="/kimera", tags=["Contradiction Engine"]
)
...
app.include_router(
    thermodynamic_router, prefix="/kimera", tags=["Thermodynamic Engine"]
)
app.include_router(
    unified_thermodynamic_router,
    prefix="/kimera",
    tags=["Unified Thermodynamic + TCSE"],
)
app.include_router(gpu_router, prefix="/kimera", tags=["GPU Acceleration"])
app.include_router(
    linguistic_router, prefix="/kimera", tags=["Linguistic Intelligence"]
)
app.include_router(
    cognitive_architecture_router, prefix="/kimera", tags=["Cognitive Architecture"]
)
app.include_router(
    cognitive_control_routes, prefix="/kimera", tags=["Cognitive Control"]
)
...
app.include_router(
    foundational_router, prefix="/kimera", tags=["Foundational Thermodynamics"]
)
...
if PHARMACEUTICAL_ROUTER_AVAILABLE and cognitive_pharmaceutical_routes:
    app.include_router(
        cognitive_pharmaceutical_routes,
        prefix="/kimera",
        tags=["Cognitive Pharmaceutical"],
    )
    logger.info("Pharmaceutical router included successfully")
...
async def prometheus_metrics():
    ...
    try:
        from src.layer_2_governance.monitoring.kimera_prometheus_metrics import (
            get_kimera_metrics,
        )
...
if __name__ == "__main__":
    ...
    if port is None:
        logger.error(
            "❌ No available ports found. Please free up one of: " + str(ports_to_try)
        )
        sys.exit(1)
