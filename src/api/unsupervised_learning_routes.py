...
class LearningConfigRequest(BaseModel):
    ...
    learning_sensitivity: float = Field(0.15, ge=0.01, le=1.0, description="Learning sensitivity")
    emergence_threshold: float = Field(0.7, ge=0.1, le=1.0, description="Pattern emergence threshold")
    insight_threshold: float = Field(0.85, ge=0.1, le=1.0, description="Insight generation threshold")
    enable_pharmaceutical_integration: bool = Field(True, description="Enable pharmaceutical data integration")

class PharmaceuticalDataRequest(BaseModel):
    """Pharmaceutical data for unsupervised learning."""
    dissolution_profiles: List[Dict[str, Any]] = Field(..., description="Dissolution test profiles")
    formulation_parameters: List[Dict[str, Any]] = Field(..., description="Formulation parameters")
    quality_attributes: Optional[List[Dict[str, Any]]] = Field(None, description="Quality attributes")
    enable_pattern_discovery: bool = Field(True, description="Enable automatic pattern discovery")

class LearningInsightRequest(BaseModel):
    """Learning insight generation request."""
    data_context: Dict[str, Any] = Field(..., description="Data context for insight generation")
    insight_type: str = Field("general", description="Type of insight to generate")
    min_confidence: float = Field(0.7, ge=0.1, le=1.0, description="Minimum confidence threshold")
...
def get_learning_engine() -> UnsupervisedCognitiveLearningEngine:
    ...
    if learning_engine is None:
        raise HTTPException(status_code=503, detail="Unsupervised Learning Engine not initialized")
    return learning_engine

def get_cognitive_field() -> CognitiveFieldDynamics:
    """Dependency to get cognitive field engine."""
    if cognitive_field is None:
        raise HTTPException(status_code=503, detail="Cognitive Field Engine not initialized")
    return cognitive_field

@router.post("/initialize")
async def initialize_learning_engine(
    config: LearningConfigRequest,
            ...
            content={
                ...
                "data": {
                    ...
                    "pattern_discovery_metrics": {
                        "total_patterns_discovered": len(dissolution_patterns) + len(formulation_patterns) + len(quality_patterns),
                        "high_confidence_patterns": len([p for p in dissolution_patterns + formulation_patterns + quality_patterns if p.get('confidence', 0) > 0.8]),
                        "insight_generation_rate": len(insights) / max(1, len(dissolution_patterns) + len(formulation_patterns) + len(quality_patterns))
                    }
                },
                "timestamp": datetime.now().isoformat()
            }
...
async def generate_learning_insights(
    ...
    try:
        ...
        high_confidence_insights = [
            insight for insight in current_insights
            ...
            content={
                ...
                "data": {
                    ...
                    "insight_statistics": {
                        ...
                        "high_confidence_count": len([i for i in all_insights if i.confidence >= 0.8]),
                        "average_confidence": sum(i.confidence for i in all_insights) / max(1, len(all_insights)),
                        "average_resonance": sum(i.resonance_strength for i in all_insights) / max(1, len(all_insights)),
                        "insight_diversity": len(set(i.event_type for i in all_insights))
                    }
                },
                "timestamp": datetime.now().isoformat()
            }
            ...
            content={
                ...
                "data": {
                    ...
                    "pattern_summary": {
                        ...
                        "pattern_types": list(set(p.get('pattern_type', 'unknown') for p in patterns.values())),
                        "average_pattern_strength": sum(p.get('strength', 0) for p in patterns.values()) / max(1, len(patterns))
                    },
                    "insight_summary": {
                        "total_insights": len(insights),
                        "recent_insights": len([i for i in insights if (datetime.now() - i.discovery_timestamp).days < 1]),
                        "average_confidence": sum(i.confidence for i in insights) / max(1, len(insights)),
                        "breakthrough_insights": len([i for i in insights if i.event_type == LearningEvent.INSIGHT_FLASH])
                    },
                    "cognitive_field_status": {
                        ...
                        "evolution_trajectory_length": len(engine.cognitive_evolution_trajectory)
                    }
                },
                "timestamp": datetime.now().isoformat()
            }
            ...
            content={
                ...
                "data": {
                    ...
                    "learning_summary": {
                        ...
                        "learning_phases_completed": len(set(i.phase for i in final_insights)),
                        "highest_confidence_insight": max((i.confidence for i in final_insights), default=0.0)
                    },
                    "preservation_status": {
