...
async def assess_context(request: ContextAssessmentRequest):
    ...
    try:
        ...
        assessment = enforcement_engine.relevance_engine.assess_context(
            input_text=request.input_text, user_context=request.user_context
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Context assessment failed: {str(e)}"
        )


@router.post("/apply_stabilization")
async def apply_stabilization(
    request: StabilizationRequest, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        if request.force_immediate:
            ...
            result = {
                "status": "scheduled",
                "message": "Stabilization scheduled in background",
            }

        return {"status": "success", "stabilization_result": result}
...
async def get_all_laws():
    ...
    try:
        ...
        return {"status": "success", "laws": laws, "total_count": len(laws)}
...
async def get_law_details(law_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Law detail retrieval failed: {str(e)}"
        )
...
async def get_active_deviations():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Deviation retrieval failed: {str(e)}"
        )
...
async def get_assessment_history(limit: int = 20):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Assessment history retrieval failed: {str(e)}"
        )
...
async def get_authorization_history(limit: int = 20):
    ...
    try:
        ...
        authorizations = enforcement_engine.relevance_engine.get_authorization_history(
            limit
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Authorization history retrieval failed: {str(e)}"
        )
...
async def verify_law_integrity_endpoint():
    ...
    try:
        ...
        return {
            ...
            "message": (
                "Law registry integrity verified"
                if integrity_valid
                else "Law registry integrity compromised"
            ),
        }
...
async def get_stability_metrics():
    ...
    try:
        ...
        stability["recent_stabilization_forces"] = (
            len(
                [
                    force
                    for force in enforcement_engine.stabilization_forces
                    if (
                        enforcement_engine.stabilization_forces[-1].applied_at
                        - force.applied_at
                    ).seconds
                    < 3600
                ]
            )
            if enforcement_engine.stabilization_forces
            else 0
        )

        return {"status": "success", "stability_metrics": stability}

    except Exception as e:
        logger.error(f"Stability metrics retrieval failed: {e}")
        raise HTTPException(
            status_code=500, detail=f"Stability metrics retrieval failed: {str(e)}"
        )
...
async def test_enforcement():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Enforcement test failed: {str(e)}"
        )
