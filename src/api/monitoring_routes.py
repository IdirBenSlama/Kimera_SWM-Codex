...
async def health_check():
    ...
    components = {
        ...
        "anomaly_detection": (
            "healthy" if monitoring_core.enable_anomaly_detection else "disabled"
        ),
        "alerting": "healthy" if monitoring_core.alert_channels else "no_channels",
        "integration": (
            "healthy" if integration_manager.is_initialized else "not_initialized"
        ),
    }
    ...
    try:
        ...
        # Add engine summary
        components["engines_summary"] = (
            f"{operational_engines}/{total_engines}_operational"
        )
    ...
    overall_status = (
        "healthy" if engine_health and not critical_monitoring_errors else "unhealthy"
    )
...
async def get_gpu_metrics(
    ...
    return {
        "timestamp": datetime.now().isoformat(),
        "time_range_minutes": last_minutes,
        "gpu_available": monitoring_core.monitoring_core.get(
...
async def get_alerts_summary():
    ...
    for period_name, cutoff_time in time_periods.items():
        period_alerts = [
            alert for alert in monitoring_core.alerts if alert.timestamp >= cutoff_time
        ]
        ...
        summary[period_name] = {
            ...
            "by_severity": {
                severity.value: len(
                    [alert for alert in period_alerts if alert.severity == severity]
                )
...
async def get_performance_metrics(
    ...
    for metric_name, history in monitoring_core.metrics_history.items():
        if any(
            metric_name.startswith(prefix) or metric_name == prefix
            ...
            performance_metrics[metric_name] = {
                ...
                "statistics": {
                    ...
                    "average": (
                        sum(recent_data) / len(recent_data) if recent_data else 0
                    ),
...
async def get_anomaly_detection_results(
    ...
    anomaly_alerts = [
        alert
        for alert in monitoring_core.alerts
        if (alert.metric_name == "anomaly_score" and alert.timestamp >= cutoff_time)
    ]
...
async def prometheus_metrics(request: Request):
    ...
    return Response(generate_latest(metrics), media_type=get_metrics_content_type())
...
async def get_monitoring_config():
    ...
    return {
        ...
        "integration_config": (
            integration_manager.config.__dict__
    ...
    try:
        ...
        try:
            ...
            with SessionLocal() as db:
                ...
                entropy_measurement = entropy_monitor.calculate_system_entropy(
                    geoids, vault_info
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get entropy data: {str(e)}"
        )
...
async def get_entropy_trends(
    window_size: int = Query(
        default=100, description="Number of recent measurements to include"
    )
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get entropy trends: {str(e)}"
        )


@router.get("/entropy/anomalies")
async def get_entropy_anomalies(
    threshold_std: float = Query(
        default=2.0, description="Standard deviation threshold for anomaly detection"
    )
):
    """
...
async def get_engines_status():
    ...
    try:
        ...
        engines_status = {"timestamp": datetime.now().isoformat(), "engines": {}}
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get engines status: {str(e)}"
        )
...
async def get_revolutionary_intelligence_metrics():
    ...
    try:
        ...
        revolutionary_intelligence = (
            kimera_system.get_revolutionary_intelligence_engine()
        )
        ...
        metrics = {
            ...
            "intelligence_assessments": engine_status.get(
...
async def get_spde_engine_metrics():
    ...
    try:
        ...
        metrics = {
            ...
            "total_solutions": engine_status.get("metrics", {}).get(
                "total_solutions", 0
            ),
            "average_solve_time": engine_status.get("metrics", {}).get(
                "average_solve_time", 0.0
            ),
            "last_solve_time": engine_status.get("metrics", {}).get(
                "last_solve_time", 0.0
            ),
...
async def get_meta_insight_engine_metrics():
    ...
    try:
        ...
        metrics = {
            ...
            "total_insights_processed": engine_status.get(
                "total_insights_processed", 0
            ),
            "total_meta_insights_generated": engine_status.get(
                "total_meta_insights_generated", 0
            ),
            "average_processing_time": engine_status.get(
                "average_processing_time", 0.0
            ),
            "insights_in_repository": engine_status.get("insights_in_repository", 0),
            "meta_insights_in_repository": engine_status.get(
                "meta_insights_in_repository", 0
            ),
            "processing_history_size": engine_status.get("processing_history_size", 0),
            "quality_assessor_history": engine_status.get(
                "quality_assessor_history", 0
            ),
            "metacognitive_processor_history": engine_status.get(
                "metacognitive_processor_history", 0
            ),
            "insights_summary": insights_summary,
...
async def get_proactive_detector_metrics():
    ...
    try:
        ...
        metrics = {
            ...
            "detection_events_history": engine_status.get(
                "detection_events_history", 0
            ),
...
async def get_revolutionary_thermodynamics_metrics():
    ...
    try:
        ...
        metrics = {
            ...
            "engine_mode": (
                getattr(foundational_engine, "mode", {}).value
                if hasattr(getattr(foundational_engine, "mode", {}), "value")
                else "unknown"
            ),
        }
        ...
        try:
            ...
            if total_cycles > 0:
                compliant_cycles = sum(
                    1
                    for cycle in carnot_cycles
                    if getattr(cycle, "physics_compliant", True)
                )
                physics_compliance_rate = compliant_cycles / total_cycles

                # Calculate average efficiency
                efficiencies = [
                    getattr(cycle, "actual_efficiency", 0.0)
                    for cycle in carnot_cycles[-10:]
                ]
                average_efficiency = (
        ...
        try:
            if consciousness_detector:
                if hasattr(consciousness_detector, "get_consciousness_statistics"):
                    consciousness_stats = (
                        consciousness_detector.get_consciousness_statistics()
                    )
                    metrics["consciousness_detection"] = {
                        "detector_active": True,
                        "total_detections": consciousness_stats.get(
                            "total_detections", 0
                        ),
                        "average_probability": consciousness_stats.get(
                            "average_probability", 0.0
                        ),
                        "last_detection": consciousness_stats.get(
                            "last_detection", None
                        ),
                    }
                else:
        ...
        try:
            ...
            metrics["thermodynamic_performance"] = {
                ...
                "recent_violations": (
                    len(physics_violations[-5:]) if physics_violations else 0
                ),
                "violation_types": (
                    list(
                        set(
                            [
                                ...
                                for v in physics_violations[-10:]
                            ]
                        )
                    )
                ...
                if hasattr(recent_cycle, "hot_temperature"):
                    ...
                    if hasattr(hot_temp, "semantic_temperature") and hasattr(
                        ...
                        if semantic > 0 and physical > 0:
                            coherence = 1.0 - abs(semantic - physical) / max(
                                semantic, physical
                            )
                            metrics["thermodynamic_performance"][
...
async def get_prometheus_metrics_endpoint():
    ...
    """
    report = get_kimera_metrics()
    return Response(content=report, media_type=get_metrics_content_type())


@router.get(
    "/system/report", summary="Get a detailed system report", tags=["Monitoring"]
)
...
@router.get(
    "/threads", summary="Get information about active threads", tags=["Monitoring"]
)
...
@router.post(
    "/shutdown", summary="Gracefully shut down the Kimera system", tags=["Monitoring"]
)
...
async def test_metrics_usage(request: Request):
    ...
    metrics.record_api_request(
        method="GET", endpoint="/test/metrics", status_code=200, duration=0.1
    )
