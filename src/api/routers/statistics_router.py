...
async def analyze_entropy_series_endpoint(
    entropy_data: List[float], timestamps: List[str] = None
):
...
async def comprehensive_statistical_analysis(request: StatisticalAnalysisRequest):
    ...
    if request.entropy_history:
        results["entropy_analysis"] = analyze_entropy_time_series(
            request.entropy_history, request.timestamps
        )
    if request.contradiction_scores and request.semantic_features:
        results["contradiction_analysis"] = analyze_contradiction_factors(
            request.contradiction_scores, request.semantic_features
        )
    if request.semantic_supply and request.semantic_demand and request.entropy_prices:
        results["semantic_market_analysis"] = analyze_semantic_market(
            request.semantic_supply, request.semantic_demand, request.entropy_prices
        )
    return results
...
async def get_statistical_alerts(severity: str = None, hours: int = 24):
    ...
    return forecasting_and_control_monitor.get_alerts(
        severity_filter=severity, hours=hours
    )
...
async def get_metric_forecast(metric_name: str):
    ...
    if forecast is None:
        raise HTTPException(
            status_code=404, detail=f"No forecast available for metric: {metric_name}"
        )
    return forecast
...
async def get_system_entropy_analysis():
    ...
    return statistical_engine.run_model(
        "descriptive_stats", data={"values": entropy_values}
    )
    ...
    try:
        if analysis_type == "basic":
            ...
            return {
                ...
                "skewness": float(
                    np.mean(
                        ((data_array - np.mean(data_array)) / np.std(data_array)) ** 3
                    )
                ),
                "kurtosis": float(
                    np.mean(
                        ((data_array - np.mean(data_array)) / np.std(data_array)) ** 4
                    )
                    - 3
                ),
            }
