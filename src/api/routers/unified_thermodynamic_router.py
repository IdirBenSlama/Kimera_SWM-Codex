...
class ProcessingRequest(BaseModel):
    ...
    geoid_data: List[Dict[str, Any]] = Field(
        ..., description="List of geoid states to process"
    )
    enable_consciousness_detection: bool = Field(
        True, description="Enable consciousness detection"
    )
    enable_thermal_regulation: bool = Field(
...
async def get_system_status():
    ...
    try:
        ...
        monitoring_active = (
            unified_system
            and hasattr(unified_system, "monitoring_active")
            and unified_system.monitoring_active
        )

        tcse_operational = (
            unified_system
            and hasattr(unified_system, "tcse_pipeline")
            and unified_system.tcse_pipeline is not None
        )

        thermodynamic_engines_ready = (
            thermo_integration
            and hasattr(thermo_integration, "engines_initialized")
            and thermo_integration.engines_initialized
        )

        consciousness_detection_active = (
            unified_system
            and hasattr(unified_system, "consciousness_detector")
            and unified_system.consciousness_detector is not None
        )
...
async def get_engines_status():
    ...
    try:
        ...
        if not thermo_integration:
            raise HTTPException(
                status_code=503, detail="Thermodynamic integration not available"
            )
...
async def detect_consciousness(
    ...
    try:
        ...
        if not thermo_integration or not thermo_integration.engines_initialized:
            raise HTTPException(
                status_code=503, detail="Thermodynamic engines not initialized"
            )
...
async def retrieve_energy(amount: float, coherence_preference: float = 0.5):
    ...
    try:
        ...
        result = await thermo_integration.retrieve_energy(
            amount=amount, coherence_preference=coherence_preference
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Energy retrieval failed: {str(e)}"
        )
...
async def get_monitoring_report():
    ...
    try:
        ...
        if not unified_system or not unified_system.system_initialized:
            raise HTTPException(
                status_code=503, detail="Unified system not initialized"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get monitoring report: {str(e)}"
        )
...
async def demo_consciousness_detection():
    ...
    try:
        ...
        result = await detect_consciousness(
            semantic_vectors=sample_vectors, temperature=1.2, entropy_content=1.5
        )

        result["demo"] = True
        result["description"] = (
            "Demo consciousness detection with random semantic vectors"
        )
...
async def demo_energy_cycle():
    ...
    try:
        ...
        retrieve_result = await retrieve_energy(amount=5.0, coherence_preference=0.8)
        ...
        return {
            ...
            "cycle_efficiency": retrieve_result["energy_retrieved"]
            / store_result["energy_stored"],
        }
