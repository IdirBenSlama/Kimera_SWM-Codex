...
async def create_geoid(request: CreateGeoidRequest):
    ...
    if not semantic_features and not request.symbolic_content:
        raise HTTPException(
            status_code=400,
            detail="Either semantic_features or symbolic_content must be provided",
        )
...
async def create_geoid_from_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Ensure text_features is a dictionary
        if not isinstance(text_features, dict):
            text_features = {"analysis": text_features}
...
async def search_geoids(query: str, limit: int = 5):
    ...
    try:
        ...
        if hasattr(query_embedding_raw, "cpu"):  # PyTorch tensor on GPU
            ...
            query_embedding_raw, "numpy"
        ):  # PyTorch tensor on CPU (alternative)
...
async def search_scars(query: str, limit: int = 3):
    ...
    try:
        ...
        search_result = vault_manager.search_scars_by_embedding(
        ...
        return {"query": query, "results": scars, "status": status, "count": len(scars)}
    except Exception as e:
