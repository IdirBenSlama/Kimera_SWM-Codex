...
class TextAnalysisRequest(BaseModel):
    ...
    processing_level: Optional[str] = Field(
        default="standard", 
        description="Processing level: minimal, standard, enhanced"
    )
...
async def get_linguistic_engine_dependency() -> LinguisticIntelligenceEngine:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Linguistic engine unavailable: {str(e)}")
...
def validate_processing_level(level: str) -> ProcessingLevel:
    ...
    if level.lower() not in level_map:
        raise HTTPException(
            status_code=400, 
            detail=f"Invalid processing level. Must be one of: {list(level_map.keys())}"
        )
...
def validate_capabilities(capabilities: Optional[List[str]]) -> Optional[List[LinguisticCapability]]:
    ...
    for cap in capabilities:
        if cap not in valid_cap_values:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid capability '{cap}'. Must be one of: {valid_cap_values}"
            )
...
@router.post("/translate", response_model=TranslationResponse, summary="Universal Translation")
async def translate_text(
    ...
    try:
        ...
        translation_dict = {
            "source_content": result.source_content if hasattr(result, 'source_content') else request.text,
            "translated_content": result.translated_content if hasattr(result, 'translated_content') else None,
            ...
            "processing_metadata": result.processing_metadata if hasattr(result, 'processing_metadata') else {}
        }
...
@router.get("/capabilities", response_model=LinguisticCapabilitiesResponse, summary="List Capabilities")
        ...
        return {
            ...
            "configuration": {
                ...
                "enabled_capabilities": [
                    k for k, v in config.__dict__.items() 
    ...
    return {
        "processing_levels": [
            ...
            },
            {
                ...
                "features": ["Semantic embedding", "Grammar analysis", "Context processing", "Meta-commentary elimination"]
            },
            {
                "value": "enhanced",
                "description": "Comprehensive analysis with all advanced features",
                "features": ["All standard features", "Entropy analysis", "Universal translation", "Advanced context assessment"]
            }
        ],
        "default": "standard",
        "note": "Processing level affects both capability availability and resource usage"
    }
