...
async def calculate_epistemic_temperature(fields: List[Any]):
    ...
    try:
        ...
        if not thermo_engine:
            raise HTTPException(
                status_code=503, detail="Thermodynamic Engine not available."
            )

        logger.info(
            f"Received {len(fields)} fields for epistemic temperature calculation."
        )

        temperature = thermo_engine.calculate_epistemic_temperature(fields)

        logger.info(
            f"Calculated epistemic temperature: {temperature.physical_temperature:.4f} K (Physical)"
        )
        return temperature
