...
async def create_geoid_optimized(
    content: str, metadata: Dict[str, Any], vault_manager
) -> GeoidState:
    ...
    try:
        ...
        return {
            ...
            "_performance": {
                ...
                "avg_time_per_geoid_ms": ((time.perf_counter() - start_time) * 1000)
                / len(geoids),
            },
        }
...
async def search_geoids_fast(query: str, limit: int = 5):
    ...
    try:
        ...
        results = await loop.run_in_executor(
            executor, vault_manager.search_geoids_by_embedding, query_embedding, limit
        )
...
@router.post(
    "/geoids/from_image", response_class=ORJSONResponse, tags=["Geoids-Optimized"]
)
...
async def process_image_geoid(
    geoid_id: str, contents: bytes, filename: str, content_type: str, vault_manager
):
    ...
    try:
        ...
        # Process image
        image_features, text_features = await clip_service.process_image_from_bytes(
            contents
        )

        embedding = (
            image_features.tolist()
            if hasattr(image_features, "tolist")
            else image_features
        )
...
@router.get(
    "/geoids/{geoid_id}/status",
    response_class=ORJSONResponse,
    tags=["Geoids-Optimized"],
)
async def get_geoid_status(geoid_id: str):
    ...
    try:
        ...
        if geoid:
            ...
            return {"geoid_id": geoid_id, "status": status, "exists": True}
        else:
            return {"geoid_id": geoid_id, "status": "not_found", "exists": False}
    except Exception as e:
