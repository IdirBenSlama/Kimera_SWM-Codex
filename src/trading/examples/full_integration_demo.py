...
async def test_full_integration():
    ...
    logger.info("\n📊 Processing market event through KIMERA...")
    ...
    if 'analysis' in result:
        ...
        if analysis.contradiction_map:
            logger.info(f"\n   ⚡ Contradictions Detected: {len(analysis.contradiction_map)}")
    ...
    if rsi_value:
        ...
        if (news_sentiment_score > 0.3 and rsi_value > 70) or (news_sentiment_score < -0.3 and rsi_value < 30):
            logger.info("  ⚡ CONTRADICTION: News sentiment contradicts technical indicators!")
...
async def main():
    ...
    try:
        ...
        logger.info("\n" + "="*80)
        logger.info("✅ Full integration demo completed successfully!")
        logger.info("="*80)
